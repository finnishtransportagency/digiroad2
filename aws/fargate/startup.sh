...
if ! [ "$batchMode" = "true" ]; then
  ...
  echo "Batch mode"
    if  [[ -z "$batchRunType" ]]; then
      ...
      elif [ "$batchRunType" = "ReplaceAutoGeneratedValues" ]; then
        echo "ReplaceAutoGeneratedValues"
          java $javaParameter -cp /digiroad2.jar fi.liikennevirasto.digiroad2.util.ReplaceAutoGeneratedValues
      else
        echo "Wrong mode"
        exit 1
      fi
