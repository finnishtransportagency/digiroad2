...
Oskari.clazz.define('Oskari.mapframework.bundle.infobox.plugin.mapmodule.OpenlayersPopupPlugin',
     ...
     */
        ...
        init: function () {
            ...
            this._templateTitle = _.template('<div class="popupInfoTitle">{{title}}</div>');
        },
        ...
        popup: function (id, title, contentData, lonlat) {
            ...
            var htmlTitle = this._templateTitle({ title : title});

            contentDiv.append(closeButton);
            contentDiv.append(htmlTitle);
        ...
        _adaptPopupSize: function ($) {
            ...
            var content = popup.find('.arrow_box').css({
                'margin-left': '0',
         ...
         */
        close: function (id) {
            eventbus.trigger('asset:unselected', id && parseInt(id, 10));
