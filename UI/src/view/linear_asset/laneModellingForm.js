(function(root) {
  var DynamicField = function (fieldSettings, isDisabled) {
    ...
    me.viewModeRender = function (field, propertyValues) {
      ...
      var _value = value ? value.value + unit: field.defaultValue ? field.defaultValue : '-';
  ...
  var NumericalField = function(assetTypeConfiguration, field, isDisabled){
    ...
    me.editModeRender = function (fieldValue, sideCode, setValue, getValue) {
      ...
      me.element = $('' +
        '<div class="form-group">' +
        '   <label class="control-label">' + field.label + '</label>' +
        '   <input type="text" name="' + field.publicId + '" fieldType = "' + field.type + '" ' + me.required() + ' class="form-control' + unitClass + '" value="' + _value + '"  id="' + className + field.publicId + '" ' + me.disabled() + '>' +
        unit +
        '</div>');
  ...
  root.LaneModellingForm = function (formStructure) {
    ...
    me.initialize = function(assetTypeConfiguration, feedbackModel){
      ...
      function setInitialForm() {
        ...
        lanesAssets.setCurrentLane(parseInt(_.min(_.map(_assetTypeConfiguration.selectedLinearAsset.get(), function (lane) {
    ...
    var createHeaderElement = function(selectedAsset) {
      var title = function () {
        ...
        return asset.id !== 0 && !selectedAsset.isSplit() ?
            ...
            '<span>' + _assetTypeConfiguration.title + '</span>';
      };
    ...
    var SaveButton = function(assetTypeConfiguration) {
      ...
      var updateStatus = function(element) {
          element.prop('disabled', !(me.isSaveable() && selectedLinearAsset.isDirty() && !selectedLinearAsset.lanesCutAreEqual()));
      };
