(function (root) {
  root.CreatedLinearAssetWorkList = function() {
    ...
    this.title = 'Linear assets created';
    var backend;
    ...
    this.bindEvents = function () {
      eventbus.on('createdLinearAssets:select', function(assetsCreated) {
        $('.container').hide();
        $('#work-list').show();
        $('body').addClass('scrollable');
        assetsList = assetsCreated;
        me.generateWorkList(assetsCreated);
      });
    };
    ...
    this.createVerificationForm = function(assetType) {
      ...
      assetTypeName = renameAssetLink(assetType.typeId);
    ...
    this.generateWorkList = function(assetsList) {
      ...
      assetsList.then(function(createdLinearAssets){
        ...
        element.html($('<div class="linear-asset-list">').append(me.assetTypesTable(createdLinearAssets.result)));
