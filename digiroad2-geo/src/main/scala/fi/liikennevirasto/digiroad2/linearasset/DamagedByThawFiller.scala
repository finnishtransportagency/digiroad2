...
class DamagedByThawFiller extends AssetFiller {
  ...
  override protected def updateValues(roadLink: RoadLink, assets: Seq[PersistedLinearAsset], changeSet: ChangeSet): (Seq[PersistedLinearAsset], ChangeSet) = {
    ...
    def inPeriod(value: DynamicPropertyValue): Boolean = {
      val period = value.value.asInstanceOf[DatePeriod]

      val startDate = period.startDate.get
      val endDate = period.endDate.get

      val todaysDay = today.getDayOfMonth
      val todaysMonth = today.getMonthOfYear
      val thisYear = today.getYear
      ...
      val parsedStartDate = monthDay.parseDateTime(period.startDate.get.toString(dateFormat))
      val parsedEndDate = monthDay.parseDateTime(period.endDate.get.toString(dateFormat))



      startDate.isBefore(today) && endDate.isAfter(today) //add enddate is before and year is before
    }
    ...
    def needsUpdate(value: DynamicPropertyValue): Boolean = {
      ...
      val parsedEndDate = monthDay.parseDateTime(period.endDate.get.toString(dateFormat))
//
//      thisYear - endDateYear > 0 && parsedCurrentDate

      startDate.isBefore(today) && endDate.isAfter(today) //add enddate is before and year is before
    }


//    def annualCorrection(value: DynamicPropertyValue): DynamicPropertyValue = {
//      toCurrentYear(value.value.asInstanceOf[DatePeriod])
//    }
//
//    def convertStringToDate(str: String): Option[DateTime] = {
