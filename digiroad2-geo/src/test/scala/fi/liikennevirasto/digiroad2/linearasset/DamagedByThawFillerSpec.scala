...
class DamagedByThawFillerSpec extends FunSuite with Matchers {
  ...
  test("ongoing period is not updated") {
    ...
    val linearAssets = Map(linkId -> damagedByThawFiller.toLinearAsset(Seq(PersistedLinearAsset(1l, linkId, 2, Some(assetValue),
      0.0, 10.0, None, None, None, None, false, 110, 0, None, linkSource = NormalLinkInterface, None, None, None)), topology.map(damagedByThawFiller.toRoadLinkForFilTopology).head))
    val (filledTopology, changeSet) = damagedByThawFiller.fillTopology(topology.map(damagedByThawFiller.toRoadLinkForFilTopology), linearAssets, DamagedByThaw.typeId)
