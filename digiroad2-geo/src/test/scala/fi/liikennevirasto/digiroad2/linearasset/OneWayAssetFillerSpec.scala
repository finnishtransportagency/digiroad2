...
class OneWayAssetFillerSpec extends FunSuite with Matchers {
  ...
  test("generate one-sided asset when two-way road link only has asset on the other side") {
    ...
    val filledTopology = oneWayAssetFiller.generateUnknowns(topology.map(oneWayAssetFiller.toRoadLinkForFillTopology), linearAssets, 110, Some(changeSet), geometryChanged = false)._1
