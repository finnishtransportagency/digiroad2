...
class NumericalLimitFillerSpec extends FunSuite with Matchers {
  ...
  test("expire assets that fall completely outside topology") {
    ...
    val linearAssets = Map(
      1l -> Seq(PersistedLinearAsset(1l, 1l, 1, Some(NumericValue(1)), 10.0, 15.0, None, None, None, None, false, 110, 0, None, None, linkSource = NormalLinkInterface)))
  ...
  test("cap assets that go over roadlink geometry") {
    val topology = Seq(
      RoadLink(1, Seq(Point(0.0, 0.0), Point(10.0, 0.0)), 10.0, Municipality,
        1, TrafficDirection.BothDirections, Motorway, None, None))
    val linearAssets = Map(
      1l -> Seq(PersistedLinearAsset(1l, 1l, 1, Some(NumericValue(1)), 0.0, 15.0, None, None, None, None, false, 110, 0, None, None, linkSource = NormalLinkInterface)))
  ...
  test("transform one-sided asset to two-sided when its defined on one-way road link") {
    ...
    val linearAssets = Map(
      1l -> Seq(PersistedLinearAsset(1l, 1l, 2, Some(NumericValue(1)), 0.0, 10.0, None, None, None, None, false, 110, 0, None, None, linkSource = NormalLinkInterface)),
      2l -> Seq(
        PersistedLinearAsset(2l, 2l, 2, Some(NumericValue(1)), 0.0, 5.0, None, None, None, None, false, 110, 0, None, None, linkSource = NormalLinkInterface),
        PersistedLinearAsset(3l, 2l, 3, Some(NumericValue(1)), 7.0, 10.0, None, None, None, None, false, 110, 0, None, None, linkSource = NormalLinkInterface),
        PersistedLinearAsset(4l, 2l, SideCode.BothDirections.value, Some(NumericValue(1)), 5.0, 7.0, None, None, None, None, false, 110, 0, None, None, linkSource = NormalLinkInterface)
      )
    )
  ...
  test("project road lights to new geometry") {
    ...
    val assets = Seq(
      ...
      None, None, None, expired = false, 100, 0, None, None, linkSource = NormalLinkInterface)
        )
  ...
  test("project paved road to new geometry, one side paved, should switch the last turned link segment") {
    ...
    val assets = Seq(
      PersistedLinearAsset(1, 1, SideCode.TowardsDigitizing.value, None, 0.0, 10.0, Some("guy"),
        None, None, None, expired = false, 110, 0, None, None, linkSource = NormalLinkInterface),
  ...
  test("project thawing asset to new geometry, cuts short") {
    ...
    val assets = Seq(
      PersistedLinearAsset(1, 1, SideCode.TowardsDigitizing.value, None, 0.0, 9.0, Some("guy"),
        None, None, None, expired = false, 130, 0, None, None, linkSource = NormalLinkInterface)
    )
  ...
  test("drop segments less than 2 meters"){
    ...
    val assets = Seq(
      PersistedLinearAsset(1, 1, SideCode.BothDirections.value, Some(NumericValue(2)), 0.0, 1.9, Some("guy"),
        Some(DateTime.now()), None, None, expired = false, 140, 0, None, None, linkSource = NormalLinkInterface),
  ...
  test("project mass transit lanes to new geometry") {
    ...
    val assets = Seq(
      PersistedLinearAsset(1, 1, SideCode.TowardsDigitizing.value, None, 1.0, 10.0, Some("guy"),
        None, None, None, expired = false, 160, 0, None, None, linkSource = NormalLinkInterface),
  ...
  test("adjustSegments with None value") {
    ...
    val assets = Seq(
      PersistedLinearAsset(1, 1, SideCode.BothDirections.value, None, 0.0, 4.5, Some("guy"),
        Some(DateTime.now()), None, None, expired = false, 160, 0, None, None, linkSource = NormalLinkInterface),
      PersistedLinearAsset(2, 1, SideCode.BothDirections.value, None, 4.5, 9.0, Some("guy"),
        Some(DateTime.now().minusDays(2)), None, None, expired = false, 160, 0, None, None, linkSource = NormalLinkInterface),
      PersistedLinearAsset(3, 1, SideCode.BothDirections.value, None, 9.0, 15.0, Some("guy"),
        Some(DateTime.now().minusDays(1)), None, None, expired = false, 160, 0, None, None, linkSource = NormalLinkInterface)
    )
  ...
  test("adjust Segments with diferent direction") {
    ...
    val assets = Seq(
      PersistedLinearAsset(1, 1, SideCode.AgainstDigitizing.value, Some(NumericValue(10)), 0.0, 5.0, Some("guy"),
        Some(DateTime.now()), None, None, expired = false, 180, 0, None, None, linkSource = NormalLinkInterface),
      PersistedLinearAsset(2, 1, SideCode.AgainstDigitizing.value, Some(NumericValue(10)), 5.0, 10.0, Some("guy"),
        Some(DateTime.now().minusDays(2)), None, None, expired = false, 180, 0, None, None, linkSource = NormalLinkInterface),
      PersistedLinearAsset(3, 1, SideCode.TowardsDigitizing.value, Some(NumericValue(20)), 0.0, 4.0, Some("guy"),
        Some(DateTime.now().minusDays(3)), None, None, expired = false, 180, 0, None, None, linkSource = NormalLinkInterface),
      PersistedLinearAsset(4, 1, SideCode.TowardsDigitizing.value, Some(NumericValue(20)), 4.0, 10.0, Some("guy"),
        Some(DateTime.now().minusDays(4)), None, None, expired = false, 180, 0, None, None, linkSource = NormalLinkInterface)
    )
  ...
  test("combine segments into one") {
    ...
    val assets = Seq(
      ...
      PersistedLinearAsset(3, 1, SideCode.TowardsDigitizing.value, Some(NumericValue(10)), 0.0, 10.0, Some("guy"),
        Some(DateTime.now().minusDays(1)), None, None, expired = false, 180, 0, None, None, linkSource = NormalLinkInterface)
    )
    val (filledTopology, changeSet) = NumericalLimitFiller.fillTopology(Seq(roadLink), Map(1L -> assets), 180)
    filledTopology.length should be (1)
    filledTopology.head.startMeasure should be (0.0)
  ...
  test("huge assets") {
    ...
    val linearAssets = assets.map( a =>
      PersistedLinearAsset(a._1, a._2, a._3, a._4, a._5, a._6, Option("k123"), None, Option("k345"), Option(DateTime.parse(a._8.dropRight(6), Asset.DateTimePropertyFormatMs)), expired=false, 100, a._7, None, None, linkSource = NormalLinkInterface)
    )
