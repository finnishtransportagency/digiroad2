...
class AssetFillerSpec extends FunSuite with Matchers {
  ...
  test("expire assets that fall completely outside topology") {
    ...
    val linearAssets = Map(
      "1" -> Seq(PersistedLinearAsset(1l, "1", 1, Some(NumericValue(1)), 10.0, 15.0, None, None, None, None, false, 110, 0, None, linkSource = NormalLinkInterface, None, None, None)))
  ...
  test("cap assets that go over roadlink geometry") {
    val topology = Seq(
      RoadLink("1", Seq(Point(0.0, 0.0), Point(10.0, 0.0)), 10.0, Municipality,
        1, TrafficDirection.BothDirections, Motorway, None, None))
    val linearAssets = Map(
      "1" -> Seq(PersistedLinearAsset(1l, "1", 1, Some(NumericValue(1)), 0.0, 15.0, None, None, None, None, false, 110, 0, None, linkSource = NormalLinkInterface, None, None, None)))
    ...
    changeSet.adjustedMValues should be(Seq(MValueAdjustment(1l, "1", 0.0, 10.0)))
  }
  ...
  test("transform one-sided asset to two-sided when its defined on one-way road link") {
    ...
    val linearAssets = Map(
      "1" -> Seq(PersistedLinearAsset(1l, "1", 2, Some(NumericValue(1)), 0.0, 10.0, None, None, None, None, false, 110, 0, None, linkSource = NormalLinkInterface, None, None, None)),
      "2" -> Seq(
        PersistedLinearAsset(2l, "2", 2, Some(NumericValue(1)), 0.0, 5.0, None, None, None, None, false, 110, 0, None, linkSource = NormalLinkInterface, None, None, None),
        PersistedLinearAsset(3l, "2", 2, Some(NumericValue(1)), 7.0, 10.0, None, None, None, None, false, 110, 0, None, linkSource = NormalLinkInterface, None, None, None),
        PersistedLinearAsset(4l, "2", SideCode.BothDirections.value, Some(NumericValue(1)), 5.0, 7.0, None, None, None, None, false, 110, 0, None, linkSource = NormalLinkInterface, None, None, None)
      )
    )
    ...
    filledTopology.filter(_.linkId == "2").map(_.sideCode) should be(Seq(BothDirections))

    filledTopology.filter(_.id == 1l).map(_.sideCode) should be(Seq(BothDirections))
    filledTopology.filter(_.id == 1l).map(_.linkId) should be(Seq("1"))
  ...
  test("drop segments less than 2 meters"){
    ...
    val (filledTopology, changeSet) = assetFiller.fillTopology(Seq(roadLink), Map("1" -> assets), 140)
    filledTopology should have size 2
  ...
  test("fuse two segments with same value in same RoadLink with different side code") {
    ...
    val (filledTopology, changeSet) = assetFiller.fillTopology(roadLinks, Map("1" -> assets), 140)
    filledTopology should have size 1
    filledTopology.map(_.sideCode) should be(Seq(SideCode.BothDirections))
    filledTopology.map(_.value) should be(Seq(Some(NumericValue(2))))
  ...
  test("adjustSegments with None value") {
    ...
    val (filledTopology, changeSet) = assetFiller.fillTopology(Seq(roadLink), Map("1" -> assets), 160)
  ...
  test("adjust Segments with diferent direction") {
    ...
    val (filledTopology, changeSet) = assetFiller.fillTopology(Seq(roadLink), Map("1" -> assets), 180)
  ...
  test("combine segments into one") {
    ...
    val assets = Seq(
      ...
      PersistedLinearAsset(3, "1", SideCode.TowardsDigitizing.value, Some(NumericValue(10)), 0.0, 10.0, Some("guy"),
        Some(DateTime.now().minusDays(1)), None, None, expired = false, 180, 0, None, linkSource = NormalLinkInterface, None, None, None)
    )
    val (filledTopology, changeSet) = assetFiller.fillTopology(Seq(roadLink), Map("1" -> assets), 180)
    filledTopology.length should be (1)
    filledTopology.head.startMeasure should be (0.0)
