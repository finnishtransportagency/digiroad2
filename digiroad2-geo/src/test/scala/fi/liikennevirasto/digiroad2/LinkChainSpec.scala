...
class LinkChainSpec extends FunSuite with Matchers {
  ...
  test("generate position indices for segment sequence where segments are in order") {
    ...
    LinkChain.generateLinkPositionsAndGeometryRunningDirections(segments)._1 shouldBe Seq(1, 0)
  }
  ...
  test("generate position indices for segment sequence where segments are not in order") {
    ...
    LinkChain.generateLinkPositionsAndGeometryRunningDirections(segments)._1 shouldBe Seq(0, 1)
  }
  ...
  test("generate position indices for three segment sequence") {
    ...
    LinkChain.generateLinkPositionsAndGeometryRunningDirections(segments)._1 shouldBe Seq(1, 0, 2)
  }
  ...
  test("generate position indices for all segments") {
    ...
    LinkChain.generateLinkPositionsAndGeometryRunningDirections(segments)._1.length shouldBe 5
  }
}
