...
class IntegrationApi(val massTransitStopService: MassTransitStopService, implicit val swagger: Swagger) extends ScalatraServlet with JacksonJsonSupport with SwaggerSupport {
  ...
  def lastModifiedBy(createdBy: Option[String], modifiedBy: Option[String]): (String, Boolean) = {
    ...
    val autoGeneratedValues = List(
      AutoGeneratedValues.dr1Conversion, nonFixedUsers(modifiedBy), nonFixedUsers(createdBy),
      AutoGeneratedValues.automaticCorrection, AutoGeneratedValues.excelDataMigration.toString,
      AutoGeneratedValues.automaticGeneration, AutoGeneratedValues.generatedInUpdate,
      AutoGeneratedValues.automaticAdjustment, AutoGeneratedValues.autoGeneratedLane)
