...
class IntegrationApi(val massTransitStopService: MassTransitStopService) extends ScalatraServlet with JacksonJsonSupport with AuthenticationSupport {
  ...
  def lastModifiedBy(createdBy: Option[String], modifiedBy: Option[String]): (String, Boolean) = {
    ...
    val autoGeneratedValues = List("dr1conversion", "dr1_conversion", nonFixedUsers(modifiedBy), nonFixedUsers(createdBy), "automatic_correction", "excel_data_migration", "automatic_generation", "vvh_generated")

    modifiedBy match {
      case None => "generatedValue" -> false
      case Some(value) if (value != null) =>
        if (autoGeneratedValues.exists(agv => agv.equals(value))) return "generatedValue" -> true else return "generatedValue" -> false
    }
    createdBy match {
      case None => "generatedValue" -> false
      case Some(value) =>
