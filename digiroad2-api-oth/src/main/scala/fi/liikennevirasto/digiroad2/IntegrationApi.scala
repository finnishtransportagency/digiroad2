...
class IntegrationApi(val massTransitStopService: MassTransitStopService) extends ScalatraServlet with JacksonJsonSupport with AuthenticationSupport {
  ...
  def lastModifiedBy(createdBy: Option[String], modifiedBy: Option[String]): (String, Boolean) = {
    ...
    val splitSpeedLimit: Option[String] = "split_speedlimit_.*".r.findFirstIn(modifiedBy.getOrElse(""))
    val autoGeneratedValues = List("dr1conversion", "dr1_conversion", splitSpeedLimit.getOrElse("else"), "automatic_correction", "excel_data_migration", "automatic_generation", "vvh_generated", "batch_process_*" )
