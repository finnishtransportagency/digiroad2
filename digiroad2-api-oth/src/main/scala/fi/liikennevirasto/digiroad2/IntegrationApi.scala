  ...
  def railwayCrossingsToApi(crossings: Seq[RailwayCrossing]): Seq[Map[String, Any]] = {
    crossings.filterNot(x => x.floating | isSuggested(x)).map { railwayCrossing =>
      Map("id" -> railwayCrossing.id,
        ...
        "safetyEquipment" -> railwayCrossing.propertyData.find(_.publicId == "turvavarustus").get.values.map(_.asInstanceOf[PropertyValue]).headOption.map(_.propertyValue).getOrElse(""),
        "name" -> railwayCrossing.propertyData.find(_.publicId == "rautatien_tasoristeyksen_nimi").get.values.map(_.asInstanceOf[PropertyValue]).headOption.map(_.propertyValue).getOrElse(""),
        "railwayCrossingId" -> railwayCrossing.propertyData.find(_.publicId == "tasoristeystunnus").get.values.map(_.asInstanceOf[PropertyValue]).headOption.map(_.propertyValue).getOrElse(""),
  ...
  def obstaclesToApi(obstacles: Seq[Obstacle]): Seq[Map[String, Any]] = {
    obstacles.filterNot(x => x.floating | isSuggested(x)).map { obstacle =>
      Map("id" -> obstacle.id,
        ...
        "obstacle_type" -> obstacle.propertyData.find(_.publicId == "esterakennelma").get.values.map(_.asInstanceOf[PropertyValue]).headOption.map(_.propertyValue).getOrElse(""),
