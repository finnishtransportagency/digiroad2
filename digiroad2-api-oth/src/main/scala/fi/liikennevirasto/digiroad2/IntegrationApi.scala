...
class IntegrationApi(val massTransitStopService: MassTransitStopService) extends ScalatraServlet with JacksonJsonSupport with AuthenticationSupport {
  ...
  def lastModifiedBy(createdBy: Option[String], modifiedBy: Option[String]): (String, Boolean) = {
    ...
    val nonFixedModified = nonFixedValues match {
      case value if value.exists(nfv => nfv.r.findFirstIn(modifiedBy.getOrElse("")).equals(modifiedBy)) =>
        modifiedBy
      case _ => None
    }
    val nonFixedCreated = nonFixedValues match {
      case value if value.exists(nfv => nfv.r.findFirstIn(modifiedBy.getOrElse("")).equals(modifiedBy)) =>
        createdBy
      case _ => None
    }

    val autoGeneratedValues = List("dr1conversion", "dr1_conversion", nonFixedModified.getOrElse(""), nonFixedCreated.getOrElse(""), "automatic_correction", "excel_data_migration", "automatic_generation", "vvh_generated")

    modifiedBy match{
      case None => "generatedValue" -> false
      case Some(value) if(value != null) => if (autoGeneratedValues.exists(agv => agv.equals(modifiedBy.getOrElse(""))))return "generatedValue" -> true else return "generatedValue" -> false
    }
    createdBy match{
      case None => "generatedValue" -> false
      case Some(value) if(modifiedBy.getOrElse("") != null || modifiedBy.getOrElse("") == null) => "generatedValue" -> autoGeneratedValues.exists(agv => agv.equals(value))
    }
  }
