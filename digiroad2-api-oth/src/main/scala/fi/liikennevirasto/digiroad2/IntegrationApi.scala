...
class IntegrationApi(val massTransitStopService: MassTransitStopService, implicit val swagger: Swagger) extends ScalatraServlet with JacksonJsonSupport with SwaggerSupport {
  ...
  def lastModifiedBy(createdBy: Option[String], modifiedBy: Option[String]): (String, Boolean) = {
    ...
    val autoGeneratedValues = List(
      AutoGeneratedValues.dr1Conversion.toString, nonFixedUsers(modifiedBy), nonFixedUsers(createdBy),
      AutoGeneratedValues.automaticCorrection.toString, AutoGeneratedValues.excelDataMigration.toString,
      AutoGeneratedValues.automaticGeneration.toString, AutoGeneratedValues.generatedInUpdate.toString,
      AutoGeneratedValues.automaticAdjustment.toString, AutoGeneratedValues.autoGeneratedLane.toString)
