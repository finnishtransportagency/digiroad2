...
class ChangeApi(val swagger: Swagger) extends ScalatraServlet with JacksonJsonSupport with AuthenticationSupport with SwaggerSupport {
  ...
  get("/:assetType", operation(getChangesOfAssetsByType)) {
    ...
    params("assetType") match {
      ...
      case "total_weight_limits"         => sevenRestrictionToGeoJson(since, linearAssetService.getChanged(TotalWeightLimit.typeId , since, until, withAdjust, pageNumber))
      case "trailer_truck_weight_limits" => sevenRestrictionToGeoJson(since, linearAssetService.getChanged(TrailerTruckWeightLimit.typeId, since, until, withAdjust, pageNumber))
      case "axle_weight_limits"          => sevenRestrictionToGeoJson(since, linearAssetService.getChanged(AxleWeightLimit.typeId, since, until, withAdjust, pageNumber))
      ...
      case "height_limits"               => sevenRestrictionToGeoJson(since, linearAssetService.getChanged(HeightLimit.typeId, since, until, withAdjust, pageNumber))
      case "length_limits"               => sevenRestrictionToGeoJson(since, linearAssetService.getChanged(LengthLimit.typeId, since, until, withAdjust, pageNumber))
      case "width_limits"                => sevenRestrictionToGeoJson(since, linearAssetService.getChanged(WidthLimit.typeId, since, until, withAdjust, pageNumber))
