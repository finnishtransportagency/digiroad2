...
class AssetReportCsvExporterSpec extends FunSuite with Matchers {
  ...
  test("test function exportAssetsByMunicipality") {
    runWithRollback {
      ...
      when(mockRoadLinkService.getRoadLinksFromDBByMunicipality(408, false)).thenReturn(vvhRoadlinks1)
      when(mockRoadLinkService.getRoadLinksFromDBByMunicipality(739, false)).thenReturn(vvhRoadlinks2)
