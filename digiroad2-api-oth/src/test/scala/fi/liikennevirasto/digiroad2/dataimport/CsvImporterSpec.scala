...
//  }
//
//  private def importerWithNullService() : MassTransitStopCsvImporter = {
//    new MassTransitStopCsvImporter {
...
//    result should equal(importerWithService.ImportResultMassTransitStop())
//
...
//    val inputStream = new ByteArrayInputStream(csv.getBytes)
//    val result = importer.importAssets(inputStream)
//    result should equal(ImportResultMassTranstiStop())
//
...
//    csvImporter.importAssets(invalidCsv) should equal(ImportResultMassTranstiStop(
...
//
//    importer.importAssets(csv) should equal(ImportResultMassTranstiStop())
//
...
//    val result = importer.importAssets(inputStream)
//
//    result should equal(ImportResultMassTranstiStop(nonExistingAssets = List(NonExistingAsset(externalId = 6, csvRow = importer.rowToString(defaultValues ++ assetFields)))))
//  }
//
