...
//        override def importAssets(csvReader: List[Map[String, String]], fileName: String, user: User, roadTypeLimitations: Set[AdministrativeClass]): Unit = {
//          process = "Updater"
//        }
//      }
...
//    val updaterCsv = massTransitStopImporter.createCSV(List("Valtakunnallinen ID"), Map("Valtakunnallinen ID" -> 1))
//
//    val updateInput = new ByteArrayInputStream(updaterCsv.getBytes)
//    massTransitStopCsvOperation.importAssets(updateInput, "", testUser, Set())
//
//    process should be("Updater")
//
//    val creatorCsv = massTransitStopImporter.createCSV(List("Koordinaatti X", "Koordinaatti Y"), Map("Koordinaatti X" -> 10000, "Koordinaatti Y" -> 1000))
//    val createInput = new ByteArrayInputStream(creatorCsv.getBytes)
//    massTransitStopCsvOperation.importAssets(createInput, "", testUser, Set())
//
//    process should be("Creator")
//
//    val positionCsv = massTransitStopImporter.createCSV(List("Valtakunnallinen ID" , "Koordinaatti X", "Koordinaatti Y"), Map("Valtakunnallinen ID" -> 1))
//    val positionInput = new ByteArrayInputStream(positionCsv.getBytes)
//    massTransitStopCsvOperation.importAssets(positionInput, "", testUser, Set())
//
...
//  }
  ...
  class TestMassTransitStopCsvOperation(roadLinkServiceImpl: RoadLinkService, eventBusImpl: DigiroadEventBus) extends
    ...
    override lazy val propertyUpdater = new Updater(roadLinkServiceImpl: RoadLinkService, eventBusImpl: DigiroadEventBus) {
      ...
      override val massTransitStopService: MassTransitStopService = new TestMassTransitStopService(eventBusImpl, roadLinkServiceImpl)
    }
    override lazy val positionUpdater = new PositionUpdater(roadLinkServiceImpl: RoadLinkService, eventBusImpl: DigiroadEventBus) {
      override def withDynTransaction[T](f: => T): T = f
      override def withDynSession[T](f: => T): T = f
...
//  }
  ...
  test("update asset admin id by CSV import") {
    ...
    when(massTransitStopCsvOperation.propertyUpdater.massTransitStopService.getMassTransitStopByNationalId(1l, () => _, true)).thenReturn(Some(MassTransitStopWithProperties(1, 1, Nil, 0.0, 0.0, None, None, None, false, Nil)))
    ...
    verify(massTransitStopImporterUpdate.massTransitStopService).updateExistingById(ArgumentMatchers.eq(1l), ArgumentMatchers.eq(None), ArgumentMatchers.eq(properties), ArgumentMatchers.eq("CsvDataImportApiSpec"), anyObject(), anyBoolean())
  }
//
...
//
//    importer.processing(csv, testUser) should equal(importer.ImportResultPointAsset())
...
//
//    result should equal(importer.ImportResultPointAsset(notImportedData = List(importer.NotImportedData(reason = "Asset not found -> 6", csvRow = importer.rowToString(massTransitStopImporter.defaultValues ++ assetFields)))))
//  }
//
...
//    val csv = massTransitStopImporter.createCSV(assetFields)
//    val inputStream = new ByteArrayInputStream(csv.getBytes)
//    val result = importer.importAssets(inputStream, testUser, roadTypeLimitations = Set(Municipality))
//
//    (csvReader: List[Map[String, String]], fileName: String, user: User, roadTypeLimitations: Set[AdministrativeClass])
//    result should equal(importer.ImportResultPointAsset(
