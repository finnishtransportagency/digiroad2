...
class ServiceRoadApiSpec extends FunSuite with ScalatraSuite with BeforeAndAfter{
  ...
  when(mockMaintenanceService.getActiveMaintenanceRoadByPolygon(any[Int])).thenReturn(Seq(PersistedLinearAsset(1, 100, 1, Some(MaintenanceRoad(Seq())), 0, 10, None, None, None, None, false, 30, 0, None, Some(10), LinkGeomSource.NormalLinkInterface)))
