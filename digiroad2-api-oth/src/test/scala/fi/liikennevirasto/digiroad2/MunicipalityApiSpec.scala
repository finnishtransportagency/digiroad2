...
class MunicipalityApiSpec extends FunSuite with Matchers with ScalatraSuite with BeforeAndAfter with AuthenticatedApiSpec {
//
...
//  }
  ...
  object ServiceWithDao extends MunicipalityApi(mockVVHClient, mockRoadLinkService, speedLimitService, pavedRoadService, mockObstacleService, new OthSwagger){
  ...
  ignore("new pavementClass with valid value to be created") {
    //TODO missing a mock
    val newRoadLink = RoadLink(5000L, List(Point(0.0, 0.0), Point(100.0, 0.0)), 100.0, Municipality, 1, TrafficDirection.BothDirections, Freeway, None, None, Map("MUNICIPALITYCODE" -> BigInt(235)))
    val newVVHroadLink = VVHRoadlink(5000L, 235, List(Point(0.0, 0.0), Point(100.0, 0.0)), Municipality, TrafficDirection.BothDirections, AllOthers)
    when(mockRoadLinkService.getRoadsLinksFromVVH(Set(5000), false)).thenReturn(Seq(newRoadLink))
  ...
  test("API entry point 200"){
    //TODO
  }

  test("API entry point 400"){
    //TODO
  }

  test("API entry point 401 "){
    //TODO
  }
}