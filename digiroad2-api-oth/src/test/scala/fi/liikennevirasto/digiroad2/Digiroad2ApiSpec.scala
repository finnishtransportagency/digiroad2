...
class Digiroad2ApiSpec extends AuthenticatedApiSpec with BeforeAndAfter {
  ...
  when(mockRoadLinkService.fetchRoadlinksFromDB(Set(linkId5, linkId8, linkId7)))
  ...
  when(mockRoadLinkService.getRoadLinkByLinkIdFromDB(linkId1))
  ...
  when(mockRoadLinkService.getRoadLinkByLinkIdFromDB(linkId2))
  ...
  when(mockRoadLinkService.getRoadLinkByLinkIdFromDB(linkId3))
  ...
  when(mockRoadLinkService.getRoadLinkAndComplementaryFromDB(linkId1))
    .thenReturn(Some(toRoadLink(RoadLinkFetched(linkId1, 91, Nil, Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers))))
  when(mockRoadLinkService.getRoadLinkAndComplementaryFromDB(linkId2))
    .thenReturn(Some(toRoadLink(RoadLinkFetched(linkId2, 235, Nil, Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers))))
  ...
  when(mockRoadLinkService.getRoadLinkAndComplementaryFromDB(linkId6))
    .thenReturn(Some(toRoadLink(RoadLinkFetched(linkId6, 235, roadLinkGeometry, Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers))))
  when(mockRoadLinkService.fetchRoadlinksFromDB(Set(linkId4)))
    .thenReturn(List(RoadLinkFetched(linkId4, 235, Seq(Point(0, 0), Point(120, 0)), Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers)))
  ...
  when(mockRoadLinkService.fetchRoadlinkAndComplementaryFromDB(linkId2))
    .thenReturn(Some(RoadLinkFetched(linkId2, 235, Nil, Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers)))
  when(mockRoadLinkService.fetchRoadlinkAndComplementaryFromDB(linkId1))
    .thenReturn(Some(RoadLinkFetched(linkId1, 91, Nil, Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers)))

  when(mockRoadLinkService.fetchRoadlinksAndComplementaryFromDB(Set(linkId5, linkId8, linkId7))).thenReturn(Seq(RoadLinkFetched(linkId5, 91,  List(Point(0.0, 0.0), Point(117.318, 0.0)), Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers),
    RoadLinkFetched(linkId8, 91,  List(Point(117.318, 0.0), Point(127.239, 0.0)), Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers),
    RoadLinkFetched(linkId7, 91,  List(Point(127.239, 0.0), Point(146.9, 0.0)), Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers)))
  when(mockRoadLinkService.fetchRoadlinkAndComplementaryFromDB(linkId5)).thenReturn(Some(RoadLinkFetched(linkId5, 91,  List(Point(0.0, 0.0), Point(117.318, 0.0)), Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers)))
  ...
  when(mockRoadLinkService.getRoadLinkAndComplementaryFromDB(linkId5)).thenReturn(Some(RoadLink(linkId5, List(Point(0.0, 0.0), Point(117.318, 0.0)), 117.318, Municipality, 1, TrafficDirection.UnknownDirection, Motorway, None, None, Map("MUNICIPALITYCODE" -> BigInt(91)))))
  when(mockRoadLinkService.getRoadLinkAndComplementaryFromDB(linkId2))
