...
class OracleLinearAssetDaoSpec extends FunSuite with Matchers {
  ...
  test("splitting one link speed limit where split measure is after link middle point modifies end measure of existing speed limit", Tag("db")) {
    Database.forDataSource(ds).withDynTransaction {
      OracleLinearAssetDao.splitSpeedLimit(700114, 5537, 100, "test")
