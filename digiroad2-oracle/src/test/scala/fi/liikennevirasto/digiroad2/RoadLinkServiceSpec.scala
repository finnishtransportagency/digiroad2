...
class RoadLinkServiceSpec extends FunSuite with Matchers with BeforeAndAfter {
  ...
  test("Verify the returning of the correct VVHHistoryRoadLink"){
    ...
    OracleDatabase.withDynTransaction {
      ...
      when(mockVVHHistoryClient.fetchVVHRoadLinkHistoryByLinkIdsF(any[Set[Long]])).thenReturn(Future(Seq(firstRoadLink, secondRoadLink)))
  ...
  test("Only road links with construction type 'in use' should be saved to incomplete_link table (not 'under construction' or 'planned')") {
    OracleDatabase.withDynTransaction {
      ...
      when(mockVVHComplementaryClient.fetchByMunicipalityF(91)).thenReturn(Promise.successful(Nil).future)
      val service = new TestService(mockVVHClient, mockEventBus)
  ...
  test("Should return roadlinks and complementary roadlinks") {
    ...
    OracleDatabase.withDynTransaction {
      ...
      when(mockVVHRoadLinkClient.fetchByMunicipalitiesAndBoundsF(any[BoundingRectangle], any[Set[Int]])).thenReturn(Future(Seq()))
      when(mockVVHComplementaryClient.fetchByMunicipalitiesAndBoundsF(any[BoundingRectangle], any[Set[Int]])).thenReturn(Future(Seq(vvhRoadLink1, vvhRoadLink2, vvhRoadLink3, vvhRoadLink4)))
