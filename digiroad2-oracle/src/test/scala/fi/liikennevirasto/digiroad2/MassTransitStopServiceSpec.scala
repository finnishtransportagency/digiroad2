...
class MassTransitStopServiceSpec extends FunSuite with Matchers {
  ...
  test("Calculate mass transit stop validity periods") {
    val massTransitStops = MassTransitStopService.getByBoundingBox(userWithKauniainenAuthorization, boundingBoxWithKauniainenAssets)
  ...
  test("Get stops by bounding box") {
    val stops = MassTransitStopService.getByBoundingBox(userWithKauniainenAuthorization, BoundingRectangle(Point(374443, 6677245), Point(374444, 6677246)))
    stops.size shouldBe 1
  }

  test("Filter stops by authorization") {
    val stops = MassTransitStopService.getByBoundingBox(User(0, "test", Configuration()), boundingBoxWithKauniainenAssets)
