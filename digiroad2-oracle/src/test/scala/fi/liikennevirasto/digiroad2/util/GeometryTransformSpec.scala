...
class GeometryTransformSpec extends FunSuite with Matchers {
  ...
  test("Resolve location on left when asset SideCode different than AgainstDigitizing value") {
    ...
    val dummyRoadAddress = Some(service.RoadAddressForLink(1, 921, 2, Track.Combined, 0, 299, None, None, 1641830, 10, 298.694, SideCode.TowardsDigitizing, Seq(Point(4002, 3067), Point(385258.765,7300119.103)), false, None, None, None))
  ...
  test("Resolve location on right when asset SideCode different than AgainstDigitizing value") {
    ...
    val dummyRoadAddress = Some(service.RoadAddressForLink(1, 921, 2, Track.RightSide, 0, 299, None, None, 1641830, 0, 298.694, SideCode.BothDirections, Seq(Point(4002, 3067), Point(385258.765,7300119.103)), false, None, None, None))
  ...
  test("Resolve location on right when asset SideCode equals to TowardsDigitizing value") {
    ...
    val dummyRoadAddress = Some(service.RoadAddressForLink(1, 921, 2, Track.RightSide, 0, 299, None, None, 1641830, 0, 298.694, SideCode.TowardsDigitizing, Seq(Point(4002, 3067), Point(385258.765,7300119.103)), false, None, None, None))
  ...
  test("Resolve location on two-way road") {
    ...
    val dummyRoadAddress = Some(service.RoadAddressForLink(1, 110, 2, Track.Combined, 0, 160, None, None, 1641830, 1, 150.690, SideCode.TowardsDigitizing, Seq(Point(4002, 3067), Point(385258.765,7300119.103)), false, None, None, None))
