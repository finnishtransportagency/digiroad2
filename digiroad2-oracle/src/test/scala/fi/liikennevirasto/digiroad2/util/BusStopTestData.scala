...
object BusStopTestData {
  def generateTestData = {
    List(
      ...
      SimpleBusStop(2, Seq(2, 3), 36, LocalDate.now.plusYears(1), Some(LocalDate.now().plusYears(3))), // in the future
      SimpleBusStop(1, Seq(2, 3, 4), 36, LocalDate.now.minusYears(2), Some(LocalDate.now().minusYears(1))), // in the past
