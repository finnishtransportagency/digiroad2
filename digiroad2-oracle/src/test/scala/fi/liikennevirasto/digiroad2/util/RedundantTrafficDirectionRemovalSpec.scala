...
class RedundantTrafficDirectionRemovalSpec extends FunSuite with Matchers {
  ...
  when(mockedRoadLinkService.fetchRoadlinksByIds(any[Set[String]])).thenReturn(Seq(roadLinkWithRedundantTrafficDirection, roadLinkWithValidTrafficDirection))

  test("A redundant traffic direction is removed, but a valid is not") {
    val linkSet = mockedRoadLinkService.fetchRoadlinksByIds(Set(linkId1, linkId2))
    runWithRollback {
