...
class LinearAssetUpdaterSpec extends FunSuite with Matchers{
  ...
  test("Should map winter speed limits of two old links to one new link") {
    ...
    runWithRollback {
      when(mockRoadLinkService.getRoadLinksAndComplementariesFromDB(Set(oldLinkId1, oldLinkId2), false)).thenReturn(oldRoadLinks)
      ...
      when(mockRoadLinkService.getRoadLinksAndComplementariesFromDB(Set(newLinkId), false)).thenReturn(Seq(newRoadLink))
  ...
  test("Should map winter speed limits of old link to three new road links, asset covers part of road link") {
    ...
    runWithRollback {
      when(mockRoadLinkService.getRoadLinksAndComplementariesFromDB(Set(oldLinkId), false)).thenReturn(Seq(oldRoadLink))
      ...
      assetsBefore.size should be(1)
      assetsBefore.foreach(asset => asset.expired should be(false))
      when(mockRoadLinkService.getRoadLinksAndComplementariesFromDB(Set(newLinkId1, newLinkId2, newLinkId3), false)).thenReturn(newRoadLinks)
