...
class LaneUpdaterSpec extends FunSuite with Matchers {
  ...
  test("Lanes should not be moved to replacing WinterRoads or HardShoulder FeatureClass road links") {
    runWithRollback {
      val oldLinkId = "84e223f4-b349-4103-88ee-03b96153ef85:1"
      val newLinkId = "a1ebe866-e704-411d-8c31-53522bc04a37:1"
      val relevantChange = testChanges.filter(_.oldLink.nonEmpty).filter(_.oldLink.get.linkId == oldLinkId).head
      // Set newLink roadClass to be WinterRoad
      val winterRoadNewLink = relevantChange.newLinks.head.copy(roadClass = 12312)
      val changeWithWinterRoadNewLink  = Seq(relevantChange.copy(newLinks = Seq(winterRoadNewLink)))
      ...
      // Apply Changes
      val changeSet = LaneUpdater.handleChanges(changeWithWinterRoadNewLink)
      LaneUpdater.updateSamuutusChangeSet(changeSet, changeWithWinterRoadNewLink)
  ...
  test("Lanes should not be moved to split WinterRoads or HardShoulder FeatureClass road links") {
    runWithRollback {
      ...
      val relevantChange = testChanges.filter(change => change.changeType == RoadLinkChangeType.Split && change.oldLink.get.linkId == "3a832249-d3b9-4c22-9b08-c7e9cd98cbd7:1").head
      // Set newLink roadClass to be WinterRoad
      val winterRoadNewLink = relevantChange.newLinks.head.copy(roadClass = 12312)
      val cycleOrPedestrianLink = relevantChange.newLinks.last
      val changeWithWinterRoadNewLink  = Seq(relevantChange.copy(newLinks = Seq(winterRoadNewLink, cycleOrPedestrianLink)))
