...
class LaneUpdaterSpec extends FunSuite with Matchers {
  ...
  test("Split link traffic direction changes, correct amount of main lanes should be generated") {
    runWithRollback {
      val oldLinkID = "5a8c9c3f-8c3c-4998-b628-51a4680abc87:1"
      val newLinkID1 = "f059a4fb-4697-43fd-9519-c6d3a08ee40d:1" // TD: TowardsDigitizing
      val newLinkID2 = "152adafb-f84a-4469-94c5-6ca40171ace6:1" // TD: BothDirections

      val relevantChange = testChanges.filter(change => change.changeType == RoadLinkChangeType.Split && change.oldLink.get.linkId == oldLinkID)

      val mainLane11 = NewLane(0, 0.0, 34.210, 49, isExpired = false, isDeleted = false, mainLaneLanePropertiesA)
      val mainLane11ID = LaneServiceWithDao.create(Seq(mainLane11), Set(oldLinkID), SideCode.TowardsDigitizing.value, testUserName).head
      ...
      // Apply Changes
      LaneUpdater.updateTrafficDirectionChangesLaneWorkList(relevantChange)

      val laneWorkListItemsAfterUpdate = laneWorkListService.workListDao.getAllItems
      laneWorkListItemsAfterUpdate.size should equal(0)

      val autoProcessedLanesWorkListItems = autoProcessedLanesWorkListService.getAutoProcessedLanesWorkList(false)
      autoProcessedLanesWorkListItems.size should equal(1)

      val changeSet = LaneUpdater.handleChanges(Seq(), relevantChange)
      LaneUpdater.updateSamuutusChangeSet(changeSet, relevantChange)

      // NewLink1 should be generated main lanes on both sideCodes, newLink2 should be generated one TowardsDigitizing main lane
      val lanesAfterChanges = LaneServiceWithDao.fetchExistingLanesByLinkIds(Seq(newLinkID1, newLinkID2))
      lanesAfterChanges.size should equal(3)
      val lanesOnNewLink2 = lanesAfterChanges.filter(_.linkId == newLinkID1)
      val lanesOnNewLink1 = lanesAfterChanges.filter(_.linkId == newLinkID2)
      lanesOnNewLink1.size should equal(2)
      lanesOnNewLink1.exists(_.sideCode == TowardsDigitizing.value) should equal(true)
      lanesOnNewLink1.exists(_.sideCode == AgainstDigitizing.value) should equal(true)
      lanesOnNewLink2.size should equal(1)
      lanesOnNewLink2.exists(_.sideCode == TowardsDigitizing.value) should equal(true)

      // Old lane should be expired with no newId and 3 new lanes should be generated
      val oldLinkHistoryLanes = laneHistoryDao.fetchAllHistoryLanesByLinkIds(Seq(oldLinkID), includeExpired = true)
      oldLinkHistoryLanes.size should equal(1)
      oldLinkHistoryLanes.foreach(historyLane => {
        historyLane.historyCreatedBy should equal(AutoGeneratedUsername.generatedInUpdate)
  ...
  test("Replacement. TrafficDirection on link changes from AgainstDigitizing to BothDirections," +
    " two new main lanes should be generated and link should be inserted to AutoProcessedLanesWorkList") {
    runWithRollback {
      val oldLinkId = "4485299e-19f4-4ee2-a234-97940a6a1a1a:1"
      val newLinkId = "dc74c541-7143-4944-95ae-fb5fd6d913b9:1"
      val relevantChange = testChanges.filter(_.oldLink.nonEmpty).filter(_.oldLink.get.linkId == oldLinkId)
      val newRoadLink = roadLinkService.getRoadLinksByLinkIds(Set(newLinkId), newTransaction = false).head

      val mainLane11 = NewLane(0, 0.0, 57.600, 49, isExpired = false, isDeleted = false, mainLaneLanePropertiesA)
      val mainLane11ID = LaneServiceWithDao.create(Seq(mainLane11), Set(oldLinkId), SideCode.AgainstDigitizing.value, testUserName).head
      ...
      val autoProcessedLanesWorkListItems = autoProcessedLanesWorkListService.getAutoProcessedLanesWorkList(false)
      autoProcessedLanesWorkListItems.size should equal(1)
      autoProcessedLanesWorkListItems.head.linkId should equal(newRoadLink.linkId)

      // Two Main lanes going both ways should be created
      val lanesAfterChanges = LaneServiceWithDao.fetchExistingLanesByLinkIds(Seq(newLinkId))
      lanesAfterChanges.size should equal(2)
      val laneTowards = lanesAfterChanges.find(_.sideCode == SideCode.TowardsDigitizing.value).get
      val laneAgainst = lanesAfterChanges.find(_.sideCode == SideCode.AgainstDigitizing.value).get
      laneTowards.laneCode should equal(1)
      laneAgainst.laneCode should equal(1)
      laneTowards.endMeasure should equal(newRoadLink.length)
      laneAgainst.endMeasure should equal(newRoadLink.length)


      val laneHistory = LaneServiceWithDao.historyDao.getHistoryLanesChangedSince(DateTime.now().minusDays(1), DateTime.now().plusDays(1), withAdjust = true)
      laneHistory.size should equal(1)
      laneHistory.foreach(historyLane => {
        historyLane.oldId should equal (mainLane11ID)
        historyLane.expired should equal (true)
        historyLane.historyCreatedBy should equal(AutoGeneratedUsername.generatedInUpdate)
        historyLane.createdBy should equal(Some(testUserName))
        historyLane.newId should equal(0)
  ...
  test("Replacement + Split Merge. TrafficDirection on merged link changes from BothDirections to AgainstDigitizing," +
    " one new main lane AgainstDigitzing should be generated and link should be inserted to AutoProcessedLanesWorkList") {
    runWithRollback {
      val oldLinkIdReplace = "0a5999ff-334a-4be5-a2ba-9df390fea1b0:1"
      val oldLinkIdSplit = "6b97e713-b9bd-4938-944a-c90c725356ac:1"
      val newLinkId = "390cc8a6-b21b-4897-8c0a-4a17ecfdf77a:1"

      val replaceChange = testChanges.filter(_.oldLink.nonEmpty).filter(_.oldLink.get.linkId == oldLinkIdReplace)
      val splitChange = testChanges.filter(_.oldLink.nonEmpty).filter(_.oldLink.get.linkId == oldLinkIdSplit)

      val newRoadLink = roadLinkService.getRoadLinksByLinkIds(Set(newLinkId), newTransaction = false).head

      val mainLaneAgainstOnSplitLink = NewLane(0, 0.0, 202.676, 49, isExpired = false, isDeleted = false, mainLaneLanePropertiesA)
      LaneServiceWithDao.create(Seq(mainLaneAgainstOnSplitLink), Set(oldLinkIdSplit), SideCode.AgainstDigitizing.value, testUserName).head

      val mainLaneTowardsOnSplitLink = NewLane(0, 0.0, 202.676, 49, isExpired = false, isDeleted = false, mainLaneLanePropertiesA)
      LaneServiceWithDao.create(Seq(mainLaneTowardsOnSplitLink), Set(oldLinkIdSplit), SideCode.TowardsDigitizing.value, testUserName).head

      val mainLaneAgainstOnReplaceLink = NewLane(0, 0.0, 96.787, 49, isExpired = false, isDeleted = false, mainLaneLanePropertiesA)
      LaneServiceWithDao.create(Seq(mainLaneAgainstOnSplitLink), Set(oldLinkIdReplace), SideCode.AgainstDigitizing.value, testUserName).head
      ...
      // One main lane AgainstDigitzing should be created
      val lanesAfterChanges = LaneServiceWithDao.fetchExistingLanesByLinkIds(Seq(newLinkId))
      lanesAfterChanges.size should equal(1)
      val laneAgainst = lanesAfterChanges.find(_.sideCode == SideCode.AgainstDigitizing.value).get
      laneAgainst.laneCode should equal(1)
  ...
  test("Replacement + Replacement Merge. TrafficDirection on old links are BothDirections and AgainstDigitizing. " +
    "Merged new link traffic direction is TowardsDigitizing. One main lane TowardsDigitizing should be generated") {
    runWithRollback {
      val oldLinkIdReplace1 = "966d17d9-d9c3-455f-9198-d2bf5e180f1c:1"
      val oldLinkIdReplace2 = "95a0131e-7e6f-4a76-b43c-ead21d0717ab:2"
      val newLinkId = "5221d9f2-3d6b-466a-87f0-8760484a02d3:2"

      val replaceChange1 = testChanges.filter(_.oldLink.nonEmpty).filter(_.oldLink.get.linkId == oldLinkIdReplace1)
      val replaceChange2 = testChanges.filter(_.oldLink.nonEmpty).filter(_.oldLink.get.linkId == oldLinkIdReplace2)

      val newRoadLink = roadLinkService.getRoadLinksByLinkIds(Set(newLinkId), newTransaction = false).head

      val mainLaneAgainstOnReplace1 = NewLane(0, 0.0, 10.22, 49, isExpired = false, isDeleted = false, mainLaneLanePropertiesA)
      LaneServiceWithDao.create(Seq(mainLaneAgainstOnReplace1), Set(oldLinkIdReplace1), SideCode.AgainstDigitizing.value, testUserName).head

      val mainLaneTowardsOnReplace1 = NewLane(0, 0.0, 10.22, 49, isExpired = false, isDeleted = false, mainLaneLanePropertiesA)
      LaneServiceWithDao.create(Seq(mainLaneTowardsOnReplace1), Set(oldLinkIdReplace1), SideCode.TowardsDigitizing.value, testUserName).head

      val mainLaneAgainstOnReplace2 = NewLane(0, 0.0, 26.4, 49, isExpired = false, isDeleted = false, mainLaneLanePropertiesA)
      LaneServiceWithDao.create(Seq(mainLaneAgainstOnReplace2), Set(oldLinkIdReplace2), SideCode.AgainstDigitizing.value, testUserName).head
      ...
      val autoProcessedLanesWorkListItems = autoProcessedLanesWorkListService.getAutoProcessedLanesWorkList(false)
      //Two traffic direction changes, so two items
      autoProcessedLanesWorkListItems.size should equal(2)
      autoProcessedLanesWorkListItems.head.linkId should equal(newRoadLink.linkId)

      // One main lane TowardsDigitizing should be created
      val lanesAfterChanges = LaneServiceWithDao.fetchExistingLanesByLinkIds(Seq(newLinkId))
      lanesAfterChanges.size should equal(1)
      val generatedMainLane = lanesAfterChanges.head
      generatedMainLane.laneCode should equal(1)
      generatedMainLane.endMeasure should equal(newRoadLink.length)
      generatedMainLane.sideCode should equal(SideCode.TowardsDigitizing.value)
