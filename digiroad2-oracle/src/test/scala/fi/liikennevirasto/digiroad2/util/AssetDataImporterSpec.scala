...
class AssetDataImporterSpec extends FunSuite with Matchers {
  ...
  */
  ...
  test("Should unfloat the obstacle when exists just one roadlink inside a radius of 10 meters"){
    ...
    val pointAssetProperty = PointAssetProperty(1111, "obstacle_suggest_box", "checkbox", false, Seq(TextPropertyValue("", None, false)))
    val mValue = 10
    ...
    val floatingObstacle = Obstacle(1, oldLinkId, obstaclePoint.x, obstaclePoint.y, mValue, true, 0, 0, Seq(pointAssetProperty), Some("unit_test"), linkSource = NormalLinkInterface)
