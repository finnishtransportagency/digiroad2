...
class AssetDataImporterSpec extends FunSuite with Matchers {
  ...
  test("Two prohibition segments on the same link produces one asset with two prohibition values") {
    ...
    val roadLink = VVHRoadlink(linkId1, 235, Seq(Point(0.0, 0.0), Point(1.0, 0.0)), Municipality, TrafficDirection.BothDirections, FeatureClass.AllOthers, attributes = CommonAttributes)
  ...
  */
  ...
  test("Should unfloat the obstacle when exists just one roadlink inside a radius of 10 meters"){
    ...
    val mValue = 10
    val vvhRoadLinks = Seq(VVHRoadlink(linkId, 853, Seq(Point(15,0), Point(15,20), Point(15,40)),Municipality, TrafficDirection.BothDirections, FeatureClass.TractorRoad, attributes = CommonAttributes))
  ...
  test("Should unfloat the obstacle when exists multiple roadlinks inside a radius of 10 meters and outside a radius of 0.5 meters but one have a feature class equals to AllOthers "){
    ...
    val mValue = 10
    val vvhRoadLinks = Seq(
      VVHRoadlink(linkId, municipality, Seq(Point(15,0), Point(15,20), Point(15,40)),Municipality, TrafficDirection.BothDirections, FeatureClass.TractorRoad, attributes = CommonAttributes),
