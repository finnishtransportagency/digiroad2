...
class DynamicLinearAssetUpdaterSpec extends FunSuite with Matchers{
  ...
  test("a walking and cycling asset is removed when the link is removed") {
    ...
    runWithRollback {
      ...
      val assetsAfter = service.getPersistedAssetsByLinkIds(CyclingAndWalking.typeId, Seq(oldLinkId), false)
  ...
  test("a walking and cycling asset is split when the road link is split") {
    ...
    runWithRollback {
      ...
      TestDynamicLinearAssetUpdater.updateByRoadLinks(CyclingAndWalking.typeId, changes)
      val assetsAfter = service.getPersistedAssetsByLinkIds(CyclingAndWalking.typeId, newLinkIds, false)
  ...
  test("a cut walking and cycling asset is moved to a lengthened road link") {
    ...
    runWithRollback {
      ...
      val id = service.createWithoutTransaction(CyclingAndWalking.typeId, oldLinkId,
        cyclingAndWalkingValue, SideCode.BothDirections.value, Measures(0, 16.465), "testuser", 0L, Some(oldRoadLink), false, None, None)
      val id2 = service.createWithoutTransaction(CyclingAndWalking.typeId, oldLinkId,
      ...
      val assetsBefore = service.getPersistedAssetsByIds(CyclingAndWalking.typeId, Set(id, id2), false)
