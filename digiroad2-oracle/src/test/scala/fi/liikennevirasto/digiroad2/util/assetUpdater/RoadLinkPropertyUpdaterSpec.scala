...
class RoadLinkPropertyUpdaterSpec extends FunSuite with Matchers {
  ...
  private def withFetchedData[T](changes: Seq[RoadLinkChange])(test: (Map[String, Option[Int]], Map[String, Option[Int]], Map[String, Option[Int]], Map[String, Option[Int]], Map[String, Map[String, String]]) => T): T = {

    val allLinkIds = changes.flatMap(change => change.newLinks.map(_.linkId) ++ change.oldLink.map(_.linkId)).toSet.toSeq

    val existingFunctionalClasses = FunctionalClassDao.getExistingValues(allLinkIds).map(fc => fc.linkId -> fc.value).toMap
    val existingLinkTypes = LinkTypeDao.getExistingValues(allLinkIds).map(lt => lt.linkId -> lt.value).toMap
    val existingTrafficDirections = TrafficDirectionDao.getExistingValues(allLinkIds).map(td => td.linkId -> td.value).toMap
    val existingAdminClasses = AdministrativeClassDao.getExistingValues(allLinkIds).map(ac => ac.linkId -> ac.value).toMap
    val existingLinkAttributes = allLinkIds.map(linkId => linkId -> LinkAttributesDao.getExistingValues(linkId)).toMap

    test(existingFunctionalClasses, existingLinkTypes, existingTrafficDirections, existingAdminClasses, existingLinkAttributes)
  }
  ...
  test("on a version change, overridden values and private road attributes are transferred to a new link") {
    ...
    val replaceChange = RoadLinkChange(Replace, Some(RoadLinkInfo(oldLinkId, 186.08235918,
      ...
      List(RoadLinkInfo(newLinkId, 186.08235918, List(Point(361966.115,6682342.526,52.256), Point(361938.529,6682516.719,54.601)), 12131, State, Some(49), BothDirections)), List(ReplaceInfo(Option(oldLinkId),Option(newLinkId),Option(0.0),
    ...
    runWithRollback {
      ...
      withFetchedData(Seq(replaceChange)) { (existingFunctionalClasses, existingLinkTypes, existingTrafficDirections, existingAdminClasses, existingLinkAttributes) =>
        val reportedChanges = roadLinkPropertyUpdater.transferOverriddenPropertiesAndPrivateRoadInfo(Seq(replaceChange), existingTrafficDirections, existingAdminClasses, existingLinkAttributes)
  ...
  test("functional class and link type are primarily transferred from old link") {
    ...
    val singleCarriageWayChange = RoadLinkChange(Split, Some(RoadLinkInfo(oldLinkId, 121.67349003, List(Point(370507.036, 6678446.659, 15.777), Point(370624.618, 6678469.114, 14.312)), 12131, State, Some(49), TowardsDigitizing)),
      List(RoadLinkInfo(newLinkId1, 17.30470614, List(Point(370524.238, 6678444.954, 15.134), Point(370507.036, 6678446.659, 15.777)), 12131, State, Some(49), AgainstDigitizing),
        RoadLinkInfo(newLinkId2, 86.94114036, List(Point(370541.665, 6678445.318, 14.602), Point(370624.618, 6678469.114, 14.312)), 12131, State, Some(49), TowardsDigitizing),
        RoadLinkInfo(newLinkId3, 17.43162599, List(Point(370524.238, 6678444.954, 15.134), Point(370541.665, 6678445.318, 14.602)), 12131, State, Some(49), TowardsDigitizing)),
      List(ReplaceInfo(Option(oldLinkId), Option(newLinkId1), Option(0.0), Option(17.305), Option(0.0), Option(17.305), false), ReplaceInfo(Option(oldLinkId), Option(newLinkId2), Option(34.733), Option(121.673), Option(0.0), Option(86.941), false), ReplaceInfo(Option(oldLinkId), Option(newLinkId3), Option(17.305), Option(34.733), Option(0.0), Option(17.432), false)))
    ...
    runWithRollback {
      ...
      withFetchedData(Seq(singleCarriageWayChange)) { (existingFunctionalClasses, existingLinkTypes, existingTrafficDirections, existingAdminClasses, existingLinkAttributes) =>
        val (reportedChanges, incompleteLinks) = roadLinkPropertyUpdater.transferOrGenerateFunctionalClassesAndLinkTypes(Seq(singleCarriageWayChange), existingFunctionalClasses, existingLinkTypes)
  ...
  test("with no old link values, functional class and link type are generated for a new link with a suitable mtkClass") {
    runWithRollback {
      ...
      withFetchedData(cycleOrPedestrianPathAddChanges) { (existingFunctionalClasses, existingLinkTypes, existingTrafficDirections, existingAdminClasses, existingLinkAttributes) =>
        val (reportedChanges, incompleteLinks) = roadLinkPropertyUpdater.transferOrGenerateFunctionalClassesAndLinkTypes(cycleOrPedestrianPathAddChanges, existingFunctionalClasses, existingLinkTypes)
  ...
  test("if functional class and link type cannot be generated or transferred, an incomplete link is created") {
    ...
    runWithRollback {
      withFetchedData(Seq(changeWithoutSuitableMtkClass)) { (existingFunctionalClasses, existingLinkTypes, existingTrafficDirections, existingAdminClasses, existingLinkAttributes) =>
        val (reportedChanges, incompleteLinks) = roadLinkPropertyUpdater.transferOrGenerateFunctionalClassesAndLinkTypes(Seq(changeWithoutSuitableMtkClass), existingFunctionalClasses, existingLinkTypes)
        reportedChanges.isEmpty should be(true)
        incompleteLinks.size should be(changeWithoutSuitableMtkClass.newLinks.size)
      }
    }
  }
  ...
  test("on a merge, new value should be inserted and reported only once") {
    ...
    runWithRollback {
      ...
      val reportedChanges = roadLinkPropertyUpdater.runProcess(mergeChanges).changes
      ...
      reportedChanges.filter(_.isInstanceOf[RoadLinkAttributeChange]).size should be(1)
      reportedChanges.filter(_.isInstanceOf[FunctionalClassChange]).size should be(1)
      reportedChanges.filter(_.isInstanceOf[LinkTypeChange]).size should be(0)
    }
  }
  ...
  test("when digitization is changed, overridden traffic direction is turned around") {
    ...
    runWithRollback {
      ...
      withFetchedData(Seq(replaceChange)) { (existingFunctionalClasses, existingLinkTypes, existingTrafficDirections, existingAdminClasses, existingLinkAttributes) =>
        val overriddenValues = roadLinkPropertyUpdater.transferOverriddenPropertiesAndPrivateRoadInfo(Seq(replaceChange), existingTrafficDirections, existingAdminClasses, existingLinkAttributes)
  ...
  test("Given a merge; When old links are missing information; Then an incomplete link should be generated") {
    ...
    runWithRollback {
      withFetchedData(mergeChanges) { (existingFunctionalClasses, existingLinkTypes, existingTrafficDirections, existingAdminClasses, existingLinkAttributes) =>
        val transferredProperties = roadLinkPropertyUpdater.transferOverriddenPropertiesAndPrivateRoadInfo(mergeChanges, existingTrafficDirections, existingAdminClasses, existingLinkAttributes)
        val (createdProperties, incompleteLinks) = roadLinkPropertyUpdater.transferOrGenerateFunctionalClassesAndLinkTypes(mergeChanges, existingFunctionalClasses, existingLinkTypes)
  ...
  test("Given a merge after a split; When old links are not missing information; Then no incomplete link should be generated") {
    ...
    runWithRollback {
      ...
      RoadLinkOverrideDAO.insert(LinkType, oldLinkId1, Some("test"), 1)
      RoadLinkOverrideDAO.insert(LinkType, oldLinkId2, Some("test"), 1)
      withFetchedData(relevantChanges) { (existingFunctionalClasses, existingLinkTypes, existingTrafficDirections, existingAdminClasses, existingLinkAttributes) =>
        val transferredProperties = roadLinkPropertyUpdater.transferOverriddenPropertiesAndPrivateRoadInfo(relevantChanges, existingTrafficDirections, existingAdminClasses, existingLinkAttributes)
        val (createdProperties, incompleteLinks) = roadLinkPropertyUpdater.transferOrGenerateFunctionalClassesAndLinkTypes(relevantChanges, existingFunctionalClasses, existingLinkTypes)
        transferredProperties.size should be(0)
  ...
  test("Test that process work with roadlink lifecycle info") {
    ...
    runWithRollback {
      ...
      RoadLinkOverrideDAO.insert(LinkType, oldLinkId1, Some("test"), 1)
      RoadLinkOverrideDAO.insert(LinkType, oldLinkId2, Some("test"), 1)
      val createdProperties = roadLinkPropertyUpdater.runProcess(relevantChanges)
      createdProperties.changes.size should be(9)

      createdProperties.changes.count(_.isInstanceOf[ConstructionTypeChange]) should be(3)
  ...
  test("no link type or functional class should be transferred or generated when road class changes to hard shoulder") {
    ...
    runWithRollback {
      ...
      withFetchedData(Seq(change)) {
        (existingFunctionalClasses, existingLinkTypes, existingTrafficDirections, existingAdminClasses, existingLinkAttributes) =>
          val (createdProperties, incompleteLinks) = roadLinkPropertyUpdater.transferOrGenerateFunctionalClassesAndLinkTypes(Seq(change), existingFunctionalClasses, existingLinkTypes)
          createdProperties.size should be(0)
          incompleteLinks.size should be(0)
      }
    }
  ...
  test("Test that process inserts incomplete links into database") {
    val oldLinkId = "99ade73f-979b-480b-976a-197ad365440a:1"
    val newLinkId1 = "1cb02550-5ce4-4c0a-8bee-c7f5e1f314d1:1"
    val newLinkId2 = "76637626-e794-4088-a9cf-20dd2ba92b6a:1"
    val newLinkId3 = "7c6fc2d3-f79f-4e03-a349-80103714a442:1"
    val changeWithoutSuitableMtkClass = RoadLinkChange(Split, Some(RoadLinkInfo(oldLinkId, 429.63910641,
      ...
      List(ReplaceInfo(Option(oldLinkId), Option(newLinkId1), Option(162.199), Option(429.639), Option(0.0), Option(267.44), false), ReplaceInfo(Option(oldLinkId), Option(newLinkId2), Option(0.0), Option(145.912), Option(0.0), Option(145.912), false),
        ReplaceInfo(Option(oldLinkId), Option(newLinkId3), Option(145.912), Option(162.199), Option(0.0), Option(16.286), false)))
    runWithRollback {
      val reportedChange = roadLinkPropertyUpdater.runProcess(Seq(changeWithoutSuitableMtkClass))
  ...
  test("Test that process inserts overridden traffic direction into database") {
    val oldLinkId = "41cca8ff-4644-41aa-8de1-2702f1a57f80:1"
    val newLinkId = "41cca8ff-4644-41aa-8de1-2702f1a57f80:2"
    val replaceChange = RoadLinkChange(Replace, Some(RoadLinkInfo(oldLinkId, 186.08235918,
      List(Point(361966.115, 6682342.526, 52.256), Point(361938.529, 6682516.719, 54.601)), 12131, State, Some(49), BothDirections)),
    ...
    runWithRollback {
      RoadLinkOverrideDAO.insert(TrafficDirection, oldLinkId, Some("test"), TowardsDigitizing.value)
      val overriddenValues = roadLinkPropertyUpdater.runProcess(Seq(replaceChange)).changes
      RoadLinkOverrideDAO.get(TrafficDirection, newLinkId) should be(Some(AgainstDigitizing.value))
    }
  }

  test("Test that process inserts generated functional class into database") {
    val oldLinkId = "c19bd6b4-9923-4ce9-a9cb-09779708913e:1"
    val newLinkId1 = "0d92e4a4-51cf-4abe-8f0e-8ee22241c3ff:1"
    val newLinkId2 = "d59dd3a9-a94d-4f26-b311-6b9b8361c717:1"
    val newLinkId3 = "e92c98c9-5a62-4995-a9c0-e40ae0b65747:1"
    ...
    runWithRollback {
      RoadLinkOverrideDAO.insert(LinkType, oldLinkId, Some("test"), TractorRoad.value)
      RoadLinkOverrideDAO.get(LinkType, oldLinkId).get should be(TractorRoad.value)
      val generatedValues = roadLinkPropertyUpdater.runProcess(Seq(singleCarriageWayChange)).changes

      RoadLinkOverrideDAO.get(FunctionalClass, newLinkId1) should be(Some(4))
    }
  }

  test("Test that process inserts generated LinkTypes into database") {
    val oldLinkId = "c19bd6b4-9923-4ce9-a9cb-09779708913e:1"
    val newLinkId1 = "0d92e4a4-51cf-4abe-8f0e-8ee22241c3ff:1"
    val newLinkId2 = "d59dd3a9-a94d-4f26-b311-6b9b8361c717:1"
    val newLinkId3 = "e92c98c9-5a62-4995-a9c0-e40ae0b65747:1"
    ...
    runWithRollback {
      RoadLinkOverrideDAO.insert(FunctionalClass, oldLinkId, Some("test"), 7)
      RoadLinkOverrideDAO.get(FunctionalClass, oldLinkId).get should be(7)
      val reportedChanges = roadLinkPropertyUpdater.runProcess(Seq(singleCarriageWayChange)).changes
