...
class RoadWidthUpdaterSpec extends FunSuite with BeforeAndAfter with Matchers with UpdaterUtilsSuite  {
  ...
  test("RoadLink is split, some part are removed,  value does not need updating") {
    val oldSplitLinkId = "0d8865f7-634c-40a1-8eee-d2de8ebefb04:1"
    val newSplitLinkId1 = "e6aac7c3-f163-40e3-b1d5-12ffb9b9a417:1"
    ...
    runWithRollback {
      ...
      val id1 = roadWidthService.createWithoutTransaction(RoadWidth.typeId, oldSplitLinkId, valueDynamic4,
        SideCode.BothDirections.value, Measures(0.0, oldRoadLinkSplit.length), AutoGeneratedUsername.mtkClassDefault,
        0L, Some(oldRoadLinkSplit), false, None, None)

      val assetsBefore = roadWidthService.getPersistedAssetsByLinkIds(RoadWidth.typeId, Seq(oldSplitLinkId, oldSplitLinkId), false)
      assetsBefore.size should be(1)
      assetsBefore.head.expired should be(false)
      assetsBefore.head.value.get.asInstanceOf[DynamicValue].value.properties.filter(_.publicId=="width").head.values.head.value should be("250")
      ...
      assetsAfter.head.endMeasure should be(378.814)
      assetsAfter.head.value.get.asInstanceOf[DynamicValue].value.properties.filter(_.publicId=="width").head.values.head.value should be("250")

    }
  }

  test("RoadLink is split, some part are removed,  value need updating") {
    val oldSplitLinkId = "0d8865f7-634c-40a1-8eee-d2de8ebefb04:1"
    val newSplitLinkId1 = "e6aac7c3-f163-40e3-b1d5-12ffb9b9a417:1"
    ...
    runWithRollback {
      ...
      assetsBefore.size should be(1)
      assetsBefore.head.expired should be(false)
      assetsBefore.head.value.get.asInstanceOf[DynamicValue].value.properties.filter(_.publicId=="width").head.values.head.value should be("400")
  ...
  test("Old link is split and merged into one new link, value does not need updating") {
    val oldSplitLinkId = "8e8fd030-89e1-4fe1-aa96-fb906eba93af:1"
   
    val oldSplitLinkId2 = "e5ecd6b4-a4ad-47bc-8f17-478a1855d163:1"
    val newSplitLinkId1 = "afc4d1c7-6d0c-4202-9f90-23d9aa31fb0c:1"

    val changes = roadLinkChangeClient.convertToRoadLinkChange(source)

    runWithRollback {
      val oldRoadLinkSplit = roadLinkService.getExpiredRoadLinkByLinkId(oldSplitLinkId, false).get
      val oldRoadLink2 = roadLinkService.getExpiredRoadLinkByLinkId(oldSplitLinkId2, false).get
      
      val id1 = roadWidthService.createWithoutTransaction(RoadWidth.typeId, oldSplitLinkId, valueDynamic4,
        SideCode.BothDirections.value, Measures(0.0, oldRoadLinkSplit.length), AutoGeneratedUsername.mtkClassDefault,
        0L, Some(oldRoadLinkSplit), false, None, None)

      val id2 = roadWidthService.createWithoutTransaction(RoadWidth.typeId, oldSplitLinkId2, valueDynamic4,
        SideCode.BothDirections.value, Measures(0.0, oldRoadLink2.length), AutoGeneratedUsername.mtkClassDefault,
        0L, Some(oldRoadLink2), false, None, None)

      val assetsBefore = roadWidthService.getPersistedAssetsByLinkIds(RoadWidth.typeId, Seq(oldSplitLinkId, oldSplitLinkId2), false)
      assetsBefore.size should be(2)
      assetsBefore.head.expired should be(false)
      assetsBefore.head.value.get.asInstanceOf[DynamicValue].value.properties.filter(_.publicId=="width").head.values.head.value should be("250")
      ...
      assetsAfter.map(_.id).contains(id1) should be(true)
      assetsAfter.head.startMeasure should be(0)
      assetsAfter.head.endMeasure should be(599.952)
      assetsAfter.head.value.get.asInstanceOf[DynamicValue].value.properties.filter(_.publicId=="width").head.values.head.value should be("250")
    }
  }

  test("Old link is split and merged into one new link, value need updating") {
    val oldSplitLinkId = "8e8fd030-89e1-4fe1-aa96-fb906eba93af:1"
    ...
    runWithRollback {
      ...
      val id2 = roadWidthService.createWithoutTransaction(RoadWidth.typeId, oldSplitLinkId2, valueDynamic,
        SideCode.BothDirections.value, Measures(0.0, oldRoadLink2.length), AutoGeneratedUsername.mtkClassDefault,
        0L, Some(oldRoadLink2), false, None, None)
