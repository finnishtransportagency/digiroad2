...
class PavedRoadUpdaterSpec extends FunSuite with Matchers with UpdaterUtilsSuite with BeforeAndAfter {
  ...
  test("Should not generate duplicate assets when same change set is run twice") {
    val oldLinkId = "0000999c-b3d1-4087-a4e2-f181907550c3:1"
    val newLinkId = "0000999c-b3d1-4087-a4e2-f181907550c3:2"
    val changes = roadLinkChangeClient.convertToRoadLinkChange(source)
    runWithRollback {

      val oldRoadLink = roadLinkService.getExpiredRoadLinkByLinkId(oldLinkId).get
      val oldAssetId = dynamicLinearAssetService.createWithoutTransaction(PavedRoad.typeId, oldLinkId,
        assetValues, SideCode.BothDirections.value, Measures(0, oldRoadLink.length), "generated_in_update", 0L, Some(oldRoadLink), true, Some(AutoGeneratedUsername.generatedInUpdate),
        Some(DateTime.parse("2020-01-01")), Some(AutoGeneratedUsername.generatedInUpdate), Some(DateTime.parse("2022-01-01")))

      val assetsBefore = dynamicLinearAssetService.getPersistedAssetsByIds(PavedRoad.typeId, Set(oldAssetId, oldAssetId), false)
      assetsBefore.size should be(1)
      ...
      assetsAfter.head.linkId should be(newLinkId)
      assetsAfter.head.startMeasure should be(0)
      assetsAfter.head.endMeasure should be(33.018) // newLink length
      assetsAfter.head.createdBy should be(Some(AutoGeneratedUsername.generatedInUpdate))

      TestPavedRoadUpdater.updateByRoadLinks(PavedRoad.typeId, changes)
      val assetsAfter2 = dynamicLinearAssetService.getPersistedAssetsByLinkIds(PavedRoad.typeId, Seq(newLinkId), newTransaction = false)
      assetsAfter2.size should be(1)
      assetsAfter2.head.linkId should be(newLinkId)
      assetsAfter2.head.startMeasure should be(0)
      assetsAfter2.head.endMeasure should be(33.018) // newLink length
      assetsAfter2.head.createdBy should be(Some(AutoGeneratedUsername.generatedInUpdate))
    }
  }
}
