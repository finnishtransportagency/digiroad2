...
class PavedRoadUpdaterSpec extends FunSuite with Matchers with UpdaterUtilsSuite with BeforeAndAfter {
  ...
  test("Given a split RoadLink that has an Unknown pavement type asset; When the new part does not cover the whole new RoadLink; Then the new part should be ignored.") {
    ...
    runWithRollback {
      ...
      PavementClass.extractPavementClassValue(sorted(1).value).get.value should be(10)
  ...
  test("Merge: the pavement of one link is defined and the other is unknown. The defined pavement value should cover the merged link totally.") {
    ...
    runWithRollback {
      ...
      PavementClass.extractPavementClassValue(assetsAfter.head.value).get.value should be(20)
  ...
  test("Split and merge. The defined pavement type should be retained") {
    val oldLinkId1 = "0b873c9b-43d3-43fb-8ffd-c361c5366db6:1"
    val oldLinkId2 = "1981d30f-1a0f-4d57-bcde-759f5eedaaef:1"
    val newLinkId1 = "1581a91b-9bfb-4223-b1e2-fa7b5a156a60:1"
    val changes = roadLinkChangeClient.convertToRoadLinkChange(source).filter(c => c.newLinks.nonEmpty && c.newLinks.map(_.linkId).contains(newLinkId1))
    runWithRollback {
      val oldRoadLink1 = roadLinkService.getExpiredRoadLinkByLinkId(oldLinkId1).get
      val oldRoadLink2 = roadLinkService.getExpiredRoadLinkByLinkId(oldLinkId2).get
      val unknownValue = DynamicValue(DynamicAssetValue(Seq(DynamicProperty("paallysteluokka", "single_choice", required = false, Seq(DynamicPropertyValue("99"))), DynamicProperty("suggest_box","checkbox",false,List()))))
      val hardAsphalt = DynamicValue(DynamicAssetValue(Seq(DynamicProperty("paallysteluokka", "single_choice", required = false, Seq(DynamicPropertyValue("10"))), DynamicProperty("suggest_box","checkbox",false,List()))))
      val id1 = Service.createWithoutTransaction(PavedRoad.typeId, oldLinkId1, hardAsphalt, SideCode.BothDirections.value, Measures(0.0, 381.244), "batch_process_PavedRoad", 1644361225000L, Some(oldRoadLink1), true, Some("batch_process_pavedRoad"), Some(DateTime.parse("2022-01-20T12:55:52.000+02:00")), Some(AutoGeneratedUsername.generatedInUpdate), Some(DateTime.parse("2022-02-11T18:41:03.000+02:00")), informationSource = Some(RoadRegistry.value))
      val id2 = Service.createWithoutTransaction(PavedRoad.typeId, oldLinkId2, unknownValue, SideCode.BothDirections.value, Measures(0.0, 203.082), AutoGeneratedUsername.generatedInUpdate, 1651618825000L, Some(oldRoadLink2), true, Some(AutoGeneratedUsername.generatedInUpdate), Some(DateTime.parse("2022-05-07T03:43:25.740+03:00")), None, None, informationSource = Some(MmlNls.value))
      val assetsBefore = Service.getPersistedAssetsByLinkIds(PavedRoad.typeId, Seq(oldLinkId1, oldLinkId2), newTransaction = false)
      assetsBefore.foreach(a => println(a))
      TestPavedRoadUpdater.updateByRoadLinks(PavedRoad.typeId, changes)
      val assetsAfter = Service.getPersistedAssetsByLinkIds(PavedRoad.typeId, Seq(newLinkId1), newTransaction = false)
      assetsAfter.foreach(a => println(a))

      println(TestPavedRoadUpdater.getReport())
    }
  }

  test("Split and merge. Test parallel run.") {
    val oldLinkId1 = "0b873c9b-43d3-43fb-8ffd-c361c5366db6:1"
    val oldLinkId2 = "1981d30f-1a0f-4d57-bcde-759f5eedaaef:1"
    val newLinkId1 = "1581a91b-9bfb-4223-b1e2-fa7b5a156a60:1"
    val changes = roadLinkChangeClient.convertToRoadLinkChange(source).filter(c => c.newLinks.nonEmpty && c.newLinks.map(_.linkId).contains(newLinkId1))
    runWithRollback {
      ...
      for (i <- 1 to 15000) {
        println(i)
        Service.createWithoutTransaction(PavedRoad.typeId, oldLinkId2, unknownValue, SideCode.BothDirections.value, Measures(0.0, Random.nextDouble() * 203), AutoGeneratedUsername.generatedInUpdate, 1651618825000L, Some(oldRoadLink2), true, Some(AutoGeneratedUsername.generatedInUpdate), Some(DateTime.parse("2022-05-07T03:43:25.740+03:00")), None, None, informationSource = Some(MmlNls.value))
      }
      val assetsBefore = Service.getPersistedAssetsByLinkIds(PavedRoad.typeId, Seq(oldLinkId1, oldLinkId2), newTransaction = false)
      println(assetsBefore.size)
      TestPavedRoadUpdater.updateByRoadLinks(PavedRoad.typeId, changes)
      val assetsAfter = Service.getPersistedAssetsByLinkIds(PavedRoad.typeId, Seq(newLinkId1), newTransaction = false)
      assetsAfter.foreach(a => println(a))
    }
