...
class ManoeuvreValidatorSpec  extends FunSuite with Matchers {
  ...
  test("restriction sign without a match asset") {
    PostGISDatabase.withDynTransaction {
      ...
      when(mockRoadLinkService.getRoadLinksAndComplementariesFromVVH(Set(linkId1, linkId2))).thenReturn(Seq(roadLink1, roadLink2))
  ...
  test("left turn sign restriction with a match asset") {
    PostGISDatabase.withDynTransaction {
      ...
      when(mockRoadLinkService.getRoadLinksAndComplementariesFromVVH(Set(linkId3, linkId4))).thenReturn(Seq(roadLink3, roadLink4))
  ...
  test("turn U sign restriction with a match asset") {
    PostGISDatabase.withDynTransaction {
      ...
      when(mockRoadLinkService.getRoadLinksAndComplementariesFromVVH(Set(linkId3, linkId4, linkId5))).thenReturn(Seq(roadLink3, roadLink4, roadLink5))
      when(mockRoadLinkService.getRoadLinksAndComplementariesFromVVH(Set(linkId3, linkId4))).thenReturn(Seq(roadLink3, roadLink4))
      when(mockRoadLinkService.getRoadLinksWithComplementaryFromVVH(any[BoundingRectangle], any[Set[Int]], any[Boolean],any[Boolean])).thenReturn(Seq(roadLink1, roadLink2, roadLink3, roadLink4))
      when(manoeuvreValidator.manoeuvreDao.getByRoadLinks(any[Seq[String]])).thenReturn(Seq(manoeuvre))
