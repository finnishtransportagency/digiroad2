...
class ManoeuvreValidatorSpec  extends FunSuite with Matchers {
  ...
  test("restriction sign without a match asset") {
    OracleDatabase.withDynTransaction {
      ...
      val propNoLeftTurn = Seq(TrafficSignProperty(0, "trafficSigns_type", "", false, Seq(TextPropertyValue(NoLeftTurn.value.toString))))
  ...
  test("turn right sign restriction with a match asset") {
    OracleDatabase.withDynTransaction {
      ...
      val propNoRightTurn = Seq(TrafficSignProperty(0, "trafficSigns_type", "", false, Seq(TextPropertyValue(NoRightTurn.value.toString))))
  ...
  test("turn U sign restriction with a match asset") {
    OracleDatabase.withDynTransaction {
      ...
      val propNoUTurn = Seq(TrafficSignProperty(0, "trafficSigns_type", "", false, Seq(TextPropertyValue(NoUTurn.value.toString))))
