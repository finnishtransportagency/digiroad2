...
class TierekisteriClientSpec extends FunSuite with Matchers  {
  ...
  lazy val tierekisteriWinterSpeedLimitAsset: TierekisteriWinterSpeedLimitAssetClient = {
    new TierekisteriWinterSpeedLimitAssetClient(dr2properties.getProperty("digiroad2.tierekisteriRestApiEndPoint"),
      dr2properties.getProperty("digiroad2.tierekisteri.enabled").toBoolean,
      HttpClientBuilder.create().build())
  }
  ...
  test("fetch from tierekisteri active Winter Speed Limit with fieldCode and roadNumber") {
    assume(testConnection)
    val assets = tierekisteriWinterSpeedLimitAsset.fetchActiveAssetData( 45)

    assets.size should be (1)
  }

  test("fetch from tierekisteri active Winter Speed Limit with fieldCode, roadNumber and roadPartNumber") {
    assume(testConnection)
    val assets = tierekisteriWinterSpeedLimitAsset.fetchActiveAssetData( 45, 1)

    assets.size should be (1)
  }

  test("fetch from tierekisteri active Winter Speed Limit with fieldCode, roadNumber, roadPartNumber and startDistance") {
    assume(testConnection)
    val assets = tierekisteriWinterSpeedLimitAsset.fetchActiveAssetData(45, 1, 0)

    assets.size should be (1)
  }

  test("fetch from tierekisteri active Winter Speed Limit with fieldCode, roadNumber, roadPartNumber, startDistance, endPart and endDistance") {
    assume(testConnection)
    val assets = tierekisteriWinterSpeedLimitAsset.fetchActiveAssetData(45, 1, 0, 0, 100)
