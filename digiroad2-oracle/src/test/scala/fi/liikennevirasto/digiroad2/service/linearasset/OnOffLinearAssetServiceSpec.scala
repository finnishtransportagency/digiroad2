...
class OnOffLinearAssetServiceSpec  extends FunSuite with Matchers {
  ...
  test("Expire on-off asset with start and end measure by update - should create two assets"){
    runWithRollback {
      when(mockLinearAssetDao.fetchLinearAssetsByIds(any[Set[Long]], any[String])).thenReturn(Seq(PersistedLinearAsset(1, 100, 1, Some(NumericValue(1)), 0, 200, None, None, None, None, false, 100, 1400000000, None, LinkGeomSource.NormalLinkInterface, None, None, None)))
