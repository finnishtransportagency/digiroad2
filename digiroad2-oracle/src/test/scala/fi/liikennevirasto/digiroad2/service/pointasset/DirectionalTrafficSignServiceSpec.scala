...
class DirectionalTrafficSignServiceSpec extends FunSuite with Matchers {
  ...
  when(mockRoadLinkService.getRoadLinksWithComplementaryFromVVH(any[BoundingRectangle], any[Set[Int]], any[Boolean])).thenReturn(
    Seq(
    VVHRoadlink(linkId, 235, Seq(Point(0.0, 0.0), Point(10.0, 0.0)), Municipality,
  ...
  test("Expire directional traffic sign") {
    when(mockRoadLinkService.getRoadLinksWithComplementaryFromVVH(235)).thenReturn(Seq(
      VVHRoadlink(linkId, 235, Seq(Point(0.0, 0.0), Point(200.0, 0.0)), Municipality, TrafficDirection.BothDirections, FeatureClass.AllOthers)).map(toRoadLink))
  ...
  test("Update directional traffic sign with geometry changes") {
    ...
    val linkId1 = "123"
    when(mockRoadLinkService.getRoadLinksWithComplementaryFromVVH(235)).thenReturn(Seq(
      VVHRoadlink(linkId, 235, linkGeometry, Municipality, TrafficDirection.BothDirections, FeatureClass.AllOthers)).map(toRoadLink))
    when(mockRoadLinkService.getRoadLinksWithComplementaryFromVVH(91)).thenReturn(Seq(
      VVHRoadlink(linkId1, 91, linkGeometry, Municipality, TrafficDirection.BothDirections, FeatureClass.AllOthers)).map(toRoadLink))

    when(mockRoadLinkService.getRoadLinkFromVVH(linkId)).thenReturn(Seq(
      VVHRoadlink(linkId, 235, linkGeometry, Municipality,
