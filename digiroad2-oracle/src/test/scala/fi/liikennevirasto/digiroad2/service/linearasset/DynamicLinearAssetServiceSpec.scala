    ...
    runWithRollback {
      ...
      withClue("assetName " + AssetTypeInfo.apply(assetInfo.typeId).layerName) {
        verify(mockEventBus, times(1))
          .publish("dynamicAsset:update", ChangeSet(Set.empty[Long], Nil, Seq(SideCodeAdjustment(assetId,SideCode.BothDirections,assetInfo.typeId), SideCodeAdjustment(0,SideCode.BothDirections,assetInfo.typeId)), Set.empty[Long], valueAdjustments = Seq.empty[ValueAdjustment]))
