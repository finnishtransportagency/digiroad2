...
class RoadWidthServiceSpec extends FunSuite with Matchers {
  ...
  test("update roadWidth and check if informationSource is Municipality Maintainer "){
    ...
    when(mockRoadLinkService.fetchRoadLinkOrComplimentaryByLinkId(any[Long])).thenReturn(Some(VVHRoadlink(5000, 235, Seq(Point(0, 0), Point(100, 0)), Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers)))
