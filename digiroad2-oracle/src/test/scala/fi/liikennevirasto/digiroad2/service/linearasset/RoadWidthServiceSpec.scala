...
class RoadWidthServiceSpec extends FunSuite with Matchers {
  ...
  private def createRoadLinks(municipalityCode: Int) = {
    ...
    val attributes1 = Map("MUNICIPALITYCODE" -> BigInt(municipalityCode), "SURFACETYPE" -> BigInt(2), "roadclass" -> BigInt(12112))
    val attributes2 = Map("MUNICIPALITYCODE" -> BigInt(municipalityCode), "SURFACETYPE" -> BigInt(2), "roadclass" -> BigInt(12122))
    val attributes3 = Map("MUNICIPALITYCODE" -> BigInt(municipalityCode), "SURFACETYPE" -> BigInt(2), "roadclass" -> BigInt(2))
  ...
  test("Should be created only 1 new road width asset when get 3 roadlink change information from vvh and only 1 roadlink have RoadClass valid") {
    ...
    val attributes0 = Map("MUNICIPALITYCODE" -> BigInt(municipalityCode), "SURFACETYPE" -> BigInt(2), "roadclass" -> BigInt(12112))
    val attributes1 = Map("MUNICIPALITYCODE" -> BigInt(municipalityCode), "SURFACETYPE" -> BigInt(2), "roadclass" -> BigInt(100))
    val attributes2 = Map("MUNICIPALITYCODE" -> BigInt(municipalityCode), "SURFACETYPE" -> BigInt(2), "roadclass" -> BigInt(2))
  ...
  test("Should not create any new asset (RoadClass not valid)") {
    ...
    val functionalClass = 1
    val linkType = Freeway
    val attributes1 = Map("MUNICIPALITYCODE" -> BigInt(municipalityCode), "SURFACETYPE" -> BigInt(2), "roadclass" -> BigInt(120))
    val attributes2 = Map("MUNICIPALITYCODE" -> BigInt(municipalityCode), "SURFACETYPE" -> BigInt(2), "roadclass" -> BigInt(2))
  ...
  test("update roadWidth and check if informationSource is Municipality Maintainer "){
    ...
    when(mockRoadlinkClient.fetchRoadLinkByLinkId(any[Long])).thenReturn(Some(RoadlinkFetched(5000, 235, Seq(Point(0, 0), Point(100, 0)), Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers)))
  ...
  test("actor should update roadWidth measures even if RoadClass is not valid "){
    ...
    val attributes = Map("MUNICIPALITYCODE" -> BigInt(235), "SURFACETYPE" -> BigInt(2), "roadclass" -> BigInt(2))
    val geometry = List(Point(0.0, 0.0), Point(20.0, 0.0))
  ...
  test("actor should not update roadWidth measures if RoadClass is valid ") {
    ...
    val attributes = Map("MUNICIPALITYCODE" -> BigInt(235), "SURFACETYPE" -> BigInt(2), "roadclass" -> BigInt(12112))
    val geometry = List(Point(0.0, 0.0), Point(20.0, 0.0))
    val roadLinks = Seq(RoadLink(5000, geometry, GeometryUtils.geometryLength(geometry), Municipality, 1, TrafficDirection.BothDirections, Freeway, None, None, attributes))
