...
class MultiValueAssetServiceSpec extends FunSuite with Matchers {
  ...
  test("equals returns false when objects are not matching"){
    val asset1 = MultiAssetValue(Seq(MultiTypeProperty("name", PropertyTypes.Text, Seq(MultiTypePropertyValue("Asset1Name")))))
    val asset2 = MultiAssetValue(Seq(MultiTypeProperty("name", PropertyTypes.Text, Seq(MultiTypePropertyValue("Asset2Name")))))
    val asset3 = MultiAssetValue(Seq(MultiTypeProperty("name", PropertyTypes.Text, Seq(MultiTypePropertyValue("BigName"), MultiTypePropertyValue("ExtraName")))))
    val asset4 = MultiAssetValue(Seq(MultiTypeProperty("name", PropertyTypes.Text, Seq(MultiTypePropertyValue("BigName")))))
    val asset5 = MultiAssetValue(Seq(MultiTypeProperty("name", PropertyTypes.ReadOnlyNumber, Seq(MultiTypePropertyValue("BiggerName")))))
    val asset6 = MultiAssetValue(Seq(MultiTypeProperty("name", PropertyTypes.Text, Seq(MultiTypePropertyValue("BiggerName")))))
