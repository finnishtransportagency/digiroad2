...
class RoadLinkServiceSpec extends FunSuite with Matchers with BeforeAndAfter {
  ...
  test("Adjust link type") {
    PostGISDatabase.withDynTransaction {
      ...
      when(mockVVHRoadLinkClient.fetchByLinkId("1"))
        .thenReturn(Some(VVHRoadlink("1", 91, Nil, Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers)))
      val service = new TestService(mockVVHClient)
  ...
  test("Provide last edited date from VVH on road link modification date if there are no overrides") {
    PostGISDatabase.withDynTransaction {
      ...
      val roadLinks = Seq(VVHRoadlink("1", 0, Nil, Municipality, TrafficDirection.TowardsDigitizing, AllOthers, Some(lastEditedDate)))
      when(mockVVHClient.roadLinkData).thenReturn(mockVVHRoadLinkClient)
  ...
  test("Adjust link traffic direction to value that is in VVH") {
    PostGISDatabase.withDynTransaction {
      ...
      when(mockVVHRoadLinkClient.fetchByLinkId(linkId))
        .thenReturn(Some(VVHRoadlink(linkId, 91, Nil, Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers)))
      val service = new TestService(mockVVHClient)
  ...
  test("Autogenerate properties for tractor road, drive path, cycle or cpedestrian path, special transport with and without gate") {
    PostGISDatabase.withDynTransaction {
      ...
      val vvhRoadLinks = List(
        VVHRoadlink(linkId1, 91, Nil, Municipality, TrafficDirection.TowardsDigitizing, FeatureClass.DrivePath),
        VVHRoadlink(linkId2, 91, Nil, Municipality, TrafficDirection.TowardsDigitizing, FeatureClass.TractorRoad),
        VVHRoadlink(linkId3, 91, Nil, Municipality, TrafficDirection.TowardsDigitizing, FeatureClass.AllOthers),
        VVHRoadlink(linkId4, 91, Nil, Municipality, TrafficDirection.TowardsDigitizing, FeatureClass.CycleOrPedestrianPath),
        VVHRoadlink(linkId5, 91, Nil, Municipality, TrafficDirection.TowardsDigitizing, FeatureClass.SpecialTransportWithoutGate),
        VVHRoadlink(linkId6, 91, Nil, Municipality, TrafficDirection.TowardsDigitizing, FeatureClass.SpecialTransportWithGate))
  ...
  test("Remove road link from incomplete link list once functional class and link type are specified") {
    PostGISDatabase.withDynTransaction {
      ...
      val roadLink = VVHRoadlink(linkId, 91, Nil, Municipality, TrafficDirection.TowardsDigitizing, FeatureClass.AllOthers)
      when(mockVVHClient.roadLinkData).thenReturn(mockVVHRoadLinkClient)
  ...
  //Ignored because "linkProperties:changed" event not in use currently
  ignore("Should not save links to incomplete_link when the road source is not norma") {
    PostGISDatabase.withDynTransaction {
      val mockVVHClient = MockitoSugar.mock[VVHClient]
      val mockVVHRoadLinkClient = MockitoSugar.mock[VVHRoadLinkClient]
      val mockVVHChangeInfoClient = MockitoSugar.mock[VVHChangeInfoClient]
  ...
  test("Test to fetch road link by private road association name") {
    PostGISDatabase.withDynTransaction {
      ...
      val vvhRoadLinks = Seq(
        VVHRoadlink(linkId1, 16, Seq(Point(386136, 6671029, 15), Point(386133, 6671115, 21)), Municipality, BothDirections, FeatureClass.AllOthers, attributes = attributesRoad1, length = 100),
        VVHRoadlink(linkId2, 16, Seq(Point(386136, 6671029, 15), Point(386133, 6671115, 21)), Municipality, BothDirections, FeatureClass.AllOthers, attributes = attributesRoad2, length = 200),
        VVHRoadlink(linkId3, 16, Seq(Point(386136, 6671029, 15), Point(386133, 6671115, 21)), Municipality, BothDirections, FeatureClass.AllOthers, attributes = attributesRoad3, length = 150)
      )
  ...
  test("Test to fetch road link by private road association name having road links with different road association name, different municipalities and no road name") {
    PostGISDatabase.withDynTransaction {
      ...
      val vvhRoadLinks = Seq(
        VVHRoadlink(linkId1, 16, Seq(Point(386136, 6671029, 15), Point(386133, 6671115, 21)), Municipality, BothDirections, FeatureClass.AllOthers, attributes = attributesRoad1, length = 100),
        VVHRoadlink(linkId2, 766, Seq(Point(386133, 6671115, 21), Point(386136, 6671029, 15)), Municipality, BothDirections, FeatureClass.AllOthers, attributes = attributesRoad2, length = 200)
      )
    ...
    val roadLinkAdjucted: List[AdjustedRoadLinksAndVVHRoadLink] = List(
      ...
      AdjustedRoadLinksAndVVHRoadLink(
        RoadLink(linkId2, List(), 0.0, Municipality,
          ...
          constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface),
        VVHRoadlink(linkId2, 0, List(), Municipality,
  ...
  test("Mass save adjustedRoadLink") {
    ...
    val roadLinkAdjucted: List[AdjustedRoadLinksAndVVHRoadLink] = List(
      AdjustedRoadLinksAndVVHRoadLink(RoadLink(linkId1, List(), 0.0, Municipality,
      ...
      , AdjustedRoadLinksAndVVHRoadLink(RoadLink(linkId1, List(), 0.0, Municipality,
        ...
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface), roadLink(linkId1))
      , AdjustedRoadLinksAndVVHRoadLink(RoadLink(linkId2, List(), 0.0, Municipality,
      ...
      , AdjustedRoadLinksAndVVHRoadLink(RoadLink(linkId2, List(), 0.0, Municipality,
        AnotherPrivateRoad.value, TrafficDirection.BothDirections, UnknownLinkType, Some("10.01.2022 14:54:15"), Some("automatic_generation"),
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface), roadLink(linkId2))
      , AdjustedRoadLinksAndVVHRoadLink(RoadLink(linkId3, List(), 0.0, Municipality,
        PrimitiveRoad.value, TrafficDirection.TowardsDigitizing, SingleCarriageway, Some("10.01.2022 14:54:15"), Some("automatic_generation"),
      ...
      , AdjustedRoadLinksAndVVHRoadLink(RoadLink(linkId3, List(), 0.0, Municipality,
        ...
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface), roadLink(linkId3))
      , AdjustedRoadLinksAndVVHRoadLink(RoadLink(linkId4, List(), 0.0, Municipality,
