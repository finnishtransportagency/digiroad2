...
class RoadLinkServiceSpec extends FunSuite with Matchers with BeforeAndAfter {
  ...
  test("Provide last edited date on road link modification date if there are no overrides") {
    PostGISDatabase.withDynTransaction {
      ...
      val roadLinks = Seq(EnrichedRoadLinkFetched(testLinkId1, 0, Nil, Municipality, TrafficDirection.TowardsDigitizing, UnknownLinkType, UnknownFunctionalClass, AllOthers, Some(lastEditedDate)))
      when(mockRoadLinkDao.fetchEnrichedByMunicipalitiesAndBounds(any[BoundingRectangle], any[Set[Int]])).thenReturn(roadLinks)
  ...
  test("Should take the latest time stamp to show in UI") {
    ...
    val roadLink = EnrichedRoadLinkFetched(linkId, 235, Nil, Municipality, TrafficDirection.TowardsDigitizing, UnknownLinkType, UnknownFunctionalClass, FeatureClass.AllOthers, Option(new DateTime("2016-02-12T12:55:04")), attributes = Map("MUNICIPALITYCODE" -> BigInt(235), "MODIFIED_BY" -> AutoGeneratedUsername.automaticAdjustment))
    ...
    PostGISDatabase.withDynTransaction {
      ...
      when(mockRoadLinkDao.fetchEnrichedByMunicipalitiesAndBounds(boundingBox, Set())).thenReturn(Seq(roadLink))
  ...
  test("Should return roadlinks and complementary roadlinks") {
    ...
    PostGISDatabase.withDynTransaction {
      ...
      when(mockRoadLinkComplimentaryDao.fetchByMunicipalitiesAndBounds(any[BoundingRectangle], any[Set[Int]])).thenReturn(Seq(complRoadLink1, complRoadLink2, complRoadLink3, complRoadLink4))
