...
class RoadLinkServiceSpec extends FunSuite with Matchers with BeforeAndAfter {
  ...
  test("Changes should cause event") {
    PostGISDatabase.withDynTransaction {
      ...
      val changeSet: RoadLinkChangeSet = RoadLinkChangeSet(adjustedRoadLinks, List(IncompleteLink(789,91,Municipality)), List(), service.getRoadLinkDataByLinkIds(vvhRoadLinks),0)
  ...
  test("Only road links with construction type 'in use' should be saved to incomplete_link table (not 'under construction' or 'planned')") {
    PostGISDatabase.withDynTransaction {
      ...
      val changeSet = RoadLinkChangeSet(Seq(), List(IncompleteLink(1,91,Municipality), IncompleteLink(4,91,Municipality)), List(), roadLinks.sortBy(_.linkId),0)
      verify(mockEventBus).publish(
        org.mockito.ArgumentMatchers.eq("linkProperties:changed"),
        org.mockito.ArgumentMatchers.eq(changeSet))
  ...
  test("Should not save links to incomplete_link when the road source is not normal") {
    PostGISDatabase.withDynTransaction {
      ...
      val changeSet = RoadLinkChangeSet(List(),List(IncompleteLink(5,91,Municipality)),List(),roadLinks.sortBy(_.linkId),0)
      verify(mockEventBus).publish(
        org.mockito.ArgumentMatchers.eq("linkProperties:changed"),
        org.mockito.ArgumentMatchers.eq(changeSet))
  ...
  // updateRoadLinkChanges test

  test("updateAutoGeneratedProperties and mass insert compare time") {
    val mockEventBus = MockitoSugar.mock[DigiroadEventBus]
    val mockVVHClient = MockitoSugar.mock[VVHClient]
    val mockVVHRoadLinkClient = MockitoSugar.mock[VVHRoadLinkClient]
    val mockVVHChangeInfoClient = MockitoSugar.mock[VVHChangeInfoClient]
    ...
    when(mockVVHClient.roadLinkData).thenReturn(mockVVHRoadLinkClient)
    when(mockVVHClient.roadLinkData.fetchByLinkIds(Set(123L))).thenReturn(
      Seq(VVHRoadlink(linkId = 123,
        municipalityCode = 0, geometry = Seq(),
        administrativeClass = Municipality, trafficDirection = TrafficDirection.TowardsDigitizing,
        featureClass = FeatureClass.TractorRoad, modifiedAt = None, attributes = Map(),
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface, length = 0.0))
    )
    
    val vVHRoadlink = VVHRoadlink(linkId = 123,
      municipalityCode = 0, geometry = Seq(),
      administrativeClass = Municipality, trafficDirection = TrafficDirection.TowardsDigitizing,
      featureClass = FeatureClass.TractorRoad, modifiedAt = None, attributes = Map(),
      constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface, length = 0.0)
    ...
    val roadLinkAdjucted: List[AdjustedRoadLinksAndVVHRoadLink] = List(
      AdjustedRoadLinksAndVVHRoadLink( RoadLink(123, List(), 0.0, Municipality,
        ...
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface),vVHRoadlink),
      AdjustedRoadLinksAndVVHRoadLink(RoadLink(123, List(), 0.0, Municipality,
        UnknownFunctionalClass.value, TrafficDirection.UnknownDirection, UnknownLinkType, Some("10.01.2022 14:54:15"), Some("automatic_generation"),
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface),vVHRoadlink),
        AdjustedRoadLinksAndVVHRoadLink(RoadLink(123, List(), 0.0, Municipality,
      ...
      , AdjustedRoadLinksAndVVHRoadLink(RoadLink(123, List(), 0.0, Municipality,
        ...
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface),vVHRoadlink)
      , AdjustedRoadLinksAndVVHRoadLink(RoadLink(123, List(), 0.0, Municipality,
        UnknownFunctionalClass.value, TrafficDirection.TowardsDigitizing, SingleCarriageway, Some("10.01.2022 14:54:15"), Some("automatic_generation"),
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface),vVHRoadlink)
      , AdjustedRoadLinksAndVVHRoadLink(RoadLink(123, List(), 0.0, Municipality,
        PrimitiveRoad.value, TrafficDirection.BothDirections, SingleCarriageway, Some("10.01.2022 14:54:15"), Some("automatic_generation"),
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface),vVHRoadlink)
      , AdjustedRoadLinksAndVVHRoadLink(RoadLink(123, List(), 0.0, Municipality,
        AnotherPrivateRoad.value, TrafficDirection.UnknownDirection, UnknownLinkType, Some("10.01.2022 14:54:15"), Some("automatic_generation"),
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface),vVHRoadlink))

    val changeSet: RoadLinkChangeSet = RoadLinkChangeSet(roadLinkAdjucted, Seq(), Seq(), Seq(), 0)
    runWithRollback {
      LogUtils.time(logger, "updateAutoGeneratedProperties") {
        service.updateAutoGeneratedProperties(changeSet.adjustedRoadLinks.map(_.adjustedRoadLink),Nil, actionID = "")
      }

    }

    runWithRollback {
      LogUtils.time(logger, "updateAutoGeneratedPropertiesMassInsert2") {
        service.updateAutoGeneratedPropertiesMassInsert(changeSet.adjustedRoadLinks, actionID = "")
      }
  ...
  test("updateAutoGeneratedProperties and mass insert compare result") {
    val mockEventBus = MockitoSugar.mock[DigiroadEventBus]
    val mockVVHClient = MockitoSugar.mock[VVHClient]
    val mockVVHRoadLinkClient = MockitoSugar.mock[VVHRoadLinkClient]
    val mockVVHChangeInfoClient = MockitoSugar.mock[VVHChangeInfoClient]
    ...
    val roadlink1 = VVHRoadlink(linkId = 123,
      municipalityCode = 0, geometry = Seq(),
      administrativeClass = Municipality, trafficDirection = TrafficDirection.TowardsDigitizing,
      featureClass = FeatureClass.TractorRoad, modifiedAt = None, attributes = Map(),
      constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface, length = 0.0)
    
    when(mockVVHClient.roadLinkData.fetchByLinkIds(Set(124L))).thenReturn(
      Seq(VVHRoadlink(linkId = 124,
        municipalityCode = 0, geometry = Seq(),
        administrativeClass = Municipality, trafficDirection = TrafficDirection.TowardsDigitizing,
        featureClass = FeatureClass.TractorRoad, modifiedAt = None, attributes = Map(),
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface, length = 0.0))
    )
    val roadlink2 =VVHRoadlink(linkId = 124,
      municipalityCode = 0, geometry = Seq(),
      administrativeClass = Municipality, trafficDirection = TrafficDirection.TowardsDigitizing,
      featureClass = FeatureClass.TractorRoad, modifiedAt = None, attributes = Map(),
      constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface, length = 0.0)
    ...
    val roadLinkAdjucted: List[AdjustedRoadLinksAndVVHRoadLink] = List(
      AdjustedRoadLinksAndVVHRoadLink(RoadLink(123, List(), 0.0, Municipality,
        AnotherPrivateRoad.value, TrafficDirection.TowardsDigitizing, SingleCarriageway, Some("10.01.2022 14:54:15"), Some("automatic_generation"),
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface),roadlink2),
        AdjustedRoadLinksAndVVHRoadLink(RoadLink(123, List(), 0.0, Municipality,
        UnknownFunctionalClass.value, TrafficDirection.UnknownDirection, UnknownLinkType, Some("10.01.2022 14:54:15"), Some("automatic_generation"),
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface),roadlink1)
      , AdjustedRoadLinksAndVVHRoadLink(RoadLink(123, List(), 0.0, Municipality,
        AnotherPrivateRoad.value, TrafficDirection.BothDirections, SingleCarriageway, Some("10.01.2022 14:54:15"), Some("automatic_generation"),
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface),roadlink1)
      , AdjustedRoadLinksAndVVHRoadLink(RoadLink(123, List(), 0.0, Municipality,
    ...
    val roadLinkAdjuctedMassInsert: List[AdjustedRoadLinksAndVVHRoadLink] = List(
      AdjustedRoadLinksAndVVHRoadLink(RoadLink(124, List(), 0.0, Municipality,
        AnotherPrivateRoad.value, TrafficDirection.TowardsDigitizing, SingleCarriageway, Some("10.01.2022 14:54:15"), Some("automatic_generation"),
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface),roadlink2),
        AdjustedRoadLinksAndVVHRoadLink(RoadLink(124, List(), 0.0, Municipality,
        UnknownFunctionalClass.value, TrafficDirection.UnknownDirection, UnknownLinkType, Some("10.01.2022 14:54:15"), Some("automatic_generation"),
    ...
    val changeSet: RoadLinkChangeSet = RoadLinkChangeSet(roadLinkAdjucted, Seq(), Seq(), Seq(), 0)
    val changeSetMass: RoadLinkChangeSet = RoadLinkChangeSet(roadLinkAdjuctedMassInsert, Seq(), Seq(), Seq(), 0)
    runWithRollback {
      LogUtils.time(logger, "updateAutoGeneratedProperties") {
        service.updateAutoGeneratedProperties(changeSet.adjustedRoadLinks.map(_.adjustedRoadLink),Nil, actionID = "")
        service.updateAutoGeneratedPropertiesMassInsert(changeSetMass.adjustedRoadLinks, actionID = "")
  ...
  test("updateAutoGeneratedProperties and mass insert and update") {
    val mockEventBus = MockitoSugar.mock[DigiroadEventBus]
    val mockVVHClient = MockitoSugar.mock[VVHClient]
    val mockVVHRoadLinkClient = MockitoSugar.mock[VVHRoadLinkClient]
    val mockVVHChangeInfoClient = MockitoSugar.mock[VVHChangeInfoClient]
    ...
    when(mockVVHClient.roadLinkChangeInfo).thenReturn(mockVVHChangeInfoClient)
    val service = new TestService(mockVVHClient, mockEventBus)
  val roadklink1 = VVHRoadlink(linkId = 124, municipalityCode = 0, geometry = Seq(),
  administrativeClass = Municipality, trafficDirection = TrafficDirection.TowardsDigitizing,
  featureClass = FeatureClass.TractorRoad, modifiedAt = None, attributes = Map(),
  constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface, length = 0.0)
    ...
    val roadLinkAdjuctedMassInsert: List[AdjustedRoadLinksAndVVHRoadLink] = List(
      AdjustedRoadLinksAndVVHRoadLink(RoadLink(124, List(), 0.0, Municipality,
        AnotherPrivateRoad.value, TrafficDirection.TowardsDigitizing, SingleCarriageway, Some("10.01.2022 14:54:15"), Some("automatic_generation"),
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface),roadklink1),
        AdjustedRoadLinksAndVVHRoadLink(RoadLink(124, List(), 0.0, Municipality,
        UnknownFunctionalClass.value, TrafficDirection.UnknownDirection, UnknownLinkType, Some("10.01.2022 14:54:15"), Some("automatic_generation"),
        constructionType = ConstructionType.InUse, linkSource = LinkGeomSource.NormalLinkInterface),roadklink1)
      , AdjustedRoadLinksAndVVHRoadLink(RoadLink(124, List(), 0.0, Municipality,
    ...
    val changeSetMass: RoadLinkChangeSet = RoadLinkChangeSet(roadLinkAdjuctedMassInsert, Seq(), Seq(), Seq(), 0)
    runWithRollback {
      sqlu"""insert into functional_class (id, link_id, functional_class, modified_by, modified_date) values (1, 124,${Freeway.value} ,'test', TO_TIMESTAMP('2014-02-10 10:03:51.047483', 'YYYY-MM-DD HH24:MI:SS.FF6'))""".execute
      sqlu"""insert into traffic_direction (id, link_id, traffic_direction, modified_by, modified_date) values (2, 124 , ${TrafficDirection.TowardsDigitizing.value}, 'test', TO_TIMESTAMP('2014-02-10 10:03:51.047483', 'YYYY-MM-DD HH24:MI:SS.FF6'))""".execute
      sqlu"""insert into link_type (id, link_id, link_type, modified_by, modified_date) values (5, 124, ${Freeway.value}, 'test', TO_TIMESTAMP('2015-03-10 10:03:51.047483', 'YYYY-MM-DD HH24:MI:SS.FF6'))""".execute
  ...
  test("updateAutoGeneratedProperties and mass insert and update2 TrafficDirection") {
    val mockEventBus = MockitoSugar.mock[DigiroadEventBus]
    val mockVVHClient = MockitoSugar.mock[VVHClient]
    val mockVVHRoadLinkClient = MockitoSugar.mock[VVHRoadLinkClient]
    val mockVVHChangeInfoClient = MockitoSugar.mock[VVHChangeInfoClient]
    ...
    val service = new TestService(mockVVHClient, mockEventBus)
    val dummy1 =  Seq(
      LinkPropertiesEntries(RoadLinkDAO.TrafficDirection,LinkProperties(124,6,SingleCarriageway,TowardsDigitizing,Municipality,None,None,None),Some("automatic_generation"),VVHRoadlink(124,0,List(),Municipality,TowardsDigitizing,FeatureClass.TractorRoad,None,Map(),ConstructionType.InUse,NormalLinkInterface,0.0),None,None,None),
      LinkPropertiesEntries(RoadLinkDAO.TrafficDirection,LinkProperties(124,6,SingleCarriageway,BothDirections,Municipality,None,None,None),Some("automatic_generation"),VVHRoadlink(124,0,List(),Municipality,TowardsDigitizing,FeatureClass.TractorRoad,None,Map(),ConstructionType.InUse,NormalLinkInterface,0.0),None,None,None),
      LinkPropertiesEntries(RoadLinkDAO.TrafficDirection,LinkProperties(12,6,SingleCarriageway,BothDirections,Municipality,None,None,None),Some("automatic_generation"),VVHRoadlink(124,0,List(),Municipality,TowardsDigitizing,FeatureClass.TractorRoad,None,Map(),ConstructionType.InUse,NormalLinkInterface,0.0),None,None,None),
      LinkPropertiesEntries(RoadLinkDAO.TrafficDirection,LinkProperties(12,6,SingleCarriageway,BothDirections,Municipality,None,None,None),Some("automatic_generation"),VVHRoadlink(124,0,List(),Municipality,TowardsDigitizing,FeatureClass.TractorRoad,None,Map(),ConstructionType.InUse,NormalLinkInterface,0.0),None,None,None)
    )


        runWithRollback {
      sqlu"""insert into traffic_direction (id, link_id, traffic_direction, modified_by, modified_date) values (nextval('primary_key_seq'), 124 , ${AgainstDigitizing.value}, 'test', TO_TIMESTAMP('2014-02-10 10:03:51.047483', 'YYYY-MM-DD HH24:MI:SS.FF6'))""".execute
     val (update,insert)=service.separateUpdateAndInsert(dummy1,RoadLinkDAO.TrafficDirection)
      update.size should be (2)
      insert.size should be (2)
    }
  }
  
  

  test("updateAutoGeneratedProperties and mass insert and update2 FunctionalClass") {
    val mockEventBus = MockitoSugar.mock[DigiroadEventBus]
    val mockVVHClient = MockitoSugar.mock[VVHClient]
    val mockVVHRoadLinkClient = MockitoSugar.mock[VVHRoadLinkClient]
    val mockVVHChangeInfoClient = MockitoSugar.mock[VVHChangeInfoClient]
    ...
    val service = new TestService(mockVVHClient, mockEventBus)
    val dummy1 =  Seq(
      LinkPropertiesEntries(RoadLinkDAO.FunctionalClass,LinkProperties(12,6,SingleCarriageway,BothDirections,Municipality,None,None,None),Some("automatic_generation"),VVHRoadlink(124,0,List(),Municipality,TowardsDigitizing,FeatureClass.TractorRoad,None,Map(),ConstructionType.InUse,NormalLinkInterface,0.0),Some("10.01.2022 14:54:15"),Some("automatic_generation"),None),
      LinkPropertiesEntries(RoadLinkDAO.FunctionalClass,LinkProperties(12,6,SingleCarriageway,BothDirections,Municipality,None,None,None),Some("automatic_generation"),VVHRoadlink(124,0,List(),Municipality,TowardsDigitizing,FeatureClass.TractorRoad,None,Map(),ConstructionType.InUse,NormalLinkInterface,0.0),Some("10.01.2022 14:54:15"),Some("automatic_generation"),None),
      LinkPropertiesEntries(RoadLinkDAO.FunctionalClass,LinkProperties(124,6,SingleCarriageway,TowardsDigitizing,Municipality,None,None,None),Some("automatic_generation"),VVHRoadlink(124,0,List(),Municipality,TowardsDigitizing,FeatureClass.TractorRoad,None,Map(),ConstructionType.InUse,NormalLinkInterface,0.0),Some("10.01.2022 14:54:15"),Some("automatic_generation"),None),
      LinkPropertiesEntries(RoadLinkDAO.FunctionalClass,LinkProperties(124,6,SingleCarriageway,BothDirections,Municipality,None,None,None),Some("automatic_generation"),VVHRoadlink(124,0,List(),Municipality,TowardsDigitizing,FeatureClass.TractorRoad,None,Map(),ConstructionType.InUse,NormalLinkInterface,0.0),Some("10.01.2022 14:54:15"),Some("automatic_generation"),None))



        runWithRollback {
          sqlu"""insert into functional_class (id, link_id, functional_class, modified_by, modified_date) values (nextval('primary_key_seq'), 124,${Freeway.value} ,'test', TO_TIMESTAMP('2014-02-10 10:03:51.047483', 'YYYY-MM-DD HH24:MI:SS.FF6'))""".execute
      val (update,insert)=service.separateUpdateAndInsert(dummy1,RoadLinkDAO.FunctionalClass)
      update.size should be (2)
      insert.size should be (2)
    }
  }
  test("updateAutoGeneratedProperties and mass insert and update LinkType2") {
    val mockEventBus = MockitoSugar.mock[DigiroadEventBus]
    val mockVVHClient = MockitoSugar.mock[VVHClient]
    val mockVVHRoadLinkClient = MockitoSugar.mock[VVHRoadLinkClient]
    val mockVVHChangeInfoClient = MockitoSugar.mock[VVHChangeInfoClient]
    ...
    val service = new TestService(mockVVHClient, mockEventBus)
    val dummy1 =  Seq(
 LinkPropertiesEntries(RoadLinkDAO.LinkType,LinkProperties(124,6,SingleCarriageway,TowardsDigitizing,Municipality,None,None,None),Some("automatic_generation"),VVHRoadlink(124,0,List(),Municipality,TowardsDigitizing,FeatureClass.TractorRoad,None,Map(),ConstructionType.InUse,NormalLinkInterface,0.0),Some("2022-01-10T14:54:15.000+02:00"),Some("automatic_generation"),None),
      LinkPropertiesEntries(RoadLinkDAO.LinkType,LinkProperties(124,6,SingleCarriageway,BothDirections,Municipality,None,None,None),Some("automatic_generation"),VVHRoadlink(124,0,List(),Municipality,TowardsDigitizing,FeatureClass.TractorRoad,None,Map(),ConstructionType.InUse,NormalLinkInterface,0.0),Some("2022-01-10T14:54:15.000+02:00"),Some("automatic_generation"),None),

      LinkPropertiesEntries(RoadLinkDAO.LinkType,LinkProperties(12,6,SingleCarriageway,BothDirections,Municipality,None,None,None),Some("automatic_generation"),VVHRoadlink(124,0,List(),Municipality,TowardsDigitizing,FeatureClass.TractorRoad,None,Map(),ConstructionType.InUse,NormalLinkInterface,0.0),Some("2022-01-10T14:54:15.000+02:00"),Some("automatic_generation"),None),
    ...
    runWithRollback {
      sqlu"""insert into link_type (id, link_id, link_type, modified_by, modified_date) values (nextval('primary_key_seq'), 124, ${Freeway.value}, 'test', TO_TIMESTAMP('2015-03-10 10:03:51.047483', 'YYYY-MM-DD HH24:MI:SS.FF6'))""".execute
      val (update,insert)=service.separateUpdateAndInsert(dummy1,RoadLinkDAO.LinkType)
      update.size should be (2)
      insert.size should be (2)
    }
  }

  test("updateAutoGeneratedProperties mass insert"){
    val mockEventBus = MockitoSugar.mock[DigiroadEventBus]
    val mockVVHClient = MockitoSugar.mock[VVHClient]
    val mockVVHRoadLinkClient = MockitoSugar.mock[VVHRoadLinkClient]
    val mockVVHChangeInfoClient = MockitoSugar.mock[VVHChangeInfoClient]
    ...
    val changeSet:RoadLinkChangeSet=RoadLinkChangeSet(roadLinkAdjucted, Seq(), Seq(),Seq() ,0)
    runWithRollback{
      LogUtils.time(logger,"updateAutoGeneratedPropertiesMassInsert"){
        service.updateAutoGeneratedPropertiesMassInsert(changeSet.adjustedRoadLinks,actionID="")
      }
  ...
  test("partition into trafficDirection, functionalClass and linkType seq"){
    val mockEventBus = MockitoSugar.mock[DigiroadEventBus]
    val mockVVHClient = MockitoSugar.mock[VVHClient]
    val mockVVHRoadLinkClient = MockitoSugar.mock[VVHRoadLinkClient]
    val mockVVHChangeInfoClient = MockitoSugar.mock[VVHChangeInfoClient]
   ...
   val roadlink1= VVHRoadlink(linkId=123,
    ...
    val (trafficDirections,functionalClass,linkTypes)=service.partitionIntoTypes(changeSet.adjustedRoadLinks)
    
    trafficDirections.size should be (8)
    functionalClass.size should be (8)
    linkTypes.size should be (6)
  }

  test("fillRoadLinkAttributes"){
    val mockEventBus = MockitoSugar.mock[DigiroadEventBus]
    val boundingBox = BoundingRectangle(Point(123, 345), Point(567, 678))
    val mockVVHClient = MockitoSugar.mock[VVHClient]
    val mockVVHRoadLinkClient = MockitoSugar.mock[VVHRoadLinkClient]
    ...
    val service = new TestService(mockVVHClient, mockEventBus)
    val result = service.getRoadLinksFromVVH(boundingBox)
    val fillRoadLinkAttributesLinks= service.getRoadLinkDataByLinkIds(vvhRoadLinks)
    val changes:Seq[ChangeInfo] = Seq()
    val changeSet:RoadLinkChangeSet=RoadLinkChangeSet(Seq(), Seq(), changes,fillRoadLinkAttributesLinks ,0)
    
    service.fillRoadLinkAttributes(changeSet.roadLinks, changeSet.changes,actionID="")
