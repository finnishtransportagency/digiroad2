...
class LaneServiceSpec extends LaneTestSupporter {
  ...
  test("Lane Change: Show 2 Add"){
    runWithRollback {
      ...
      when(mockRoadAddressService.roadLinkWithRoadAddressFromSpecificDate(any(), any())).thenReturn(roadlink1)
  ...
  test("Lane Change: Show 1 Add with roadAddress"){
    runWithRollback {
      ...
      val laneOnLink1 = laneDao.fetchLanesByLinkIdsAndLaneCode(Seq(linkId1)).head
      val laneOnLink2 = laneDao.fetchLanesByLinkIdsAndLaneCode(Seq(linkId2)).head

      when(mockRoadLinkService.getRoadLinksByLinkIds(Set(linkId1,linkId2), false)).thenReturn(
        Seq(roadlink1,roadlink2)
      )
      when(mockRoadAddressService.roadLinkWithRoadAddressFromSpecificDate(roadlink1, laneOnLink1.createdDateTime.get)).thenReturn(roadlink1)
      when(mockRoadAddressService.roadLinkWithRoadAddressFromSpecificDate(roadlink2, laneOnLink2.createdDateTime.get)).thenReturn(roadlink2)
