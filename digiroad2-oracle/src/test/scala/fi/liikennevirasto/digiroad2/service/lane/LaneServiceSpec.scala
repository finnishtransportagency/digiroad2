...
class LaneServiceSpec extends LaneTestSupporter {
  ...
  test("Update Lane Information") {
    runWithRollback {
      ...
      val updatedLane = ServiceWithDao.update(Seq(NewLane(newLane1.head, 0, 500, 745, false, false, updateValues1)), Set(linkId1), 1, usernameTest, Seq(SideCodesForLinkIds("100L", 1)), allExistingLanes)
  ...
  test("Remove unused attributes from database") {
    runWithRollback {
      ...
      val updatedLaneId = ServiceWithDao.update(Seq(NewLane(newLaneId.head, 0, 500, 745, false, false, lanePropertiesWithEmptyDate)), Set(linkId1), 1, usernameTest, Seq(SideCodesForLinkIds("100L", 1)), createdLane)
  ...
  test("Update two lanes in one roadlink to only one lane") {
    runWithRollback {
      ...
      val currentMainLane = mainLane.copy(id = mainLane1Id)
      ServiceWithDao.processNewLanes(Set(currentMainLane, subLane2), Set(linkId1), 1, usernameTest, sideCodesForLinkIds)
  ...
  test("Lane Change: Show 1 Add and 1 attribute changed"){
    runWithRollback {
      ...
      val allExistingLanes = laneDao.fetchLanesByLinkIdsAndLaneCode(Seq("100L"))
      ServiceWithDao.update(Seq(newLane1.copy(id = lane1Id, properties = newLanePropertiesValues11)), Set(linkId1), 1, usernameTest, Seq(SideCodesForLinkIds("100L", 2)), allExistingLanes)
  ...
  test("Lane Change: Show 2 Add, 1 lane code change"){
    runWithRollback {
      ...
      ServiceWithDao.update(Seq(newLane4.copy(id = lane4Id, properties = newLanePropertiesValuesOld4)), Set(linkId1), 1, usernameTest, Seq(SideCodesForLinkIds("100L", 2)), allExistingLanes)
  ...
  test("Lane Change: Show 2 Add and 1 shortened lane"){
    runWithRollback {
      ...
      ServiceWithDao.update(Seq(subLane2Split), Set(linkId1), 2, usernameTest, Seq(SideCodesForLinkIds("100L", 2)), allExistingLanes)
  ...
  test("Lane Change: Get only the 2 Add with token"){
    ...
    runWithRollback {
      ...
      val allExistingLanes = laneDao.fetchLanesByLinkIdsAndLaneCode(Seq("100L"))
      val subLane2Split = NewLane(0, 0, 50, 745, false, false, lanePropertiesValues2)
      val subLane2Id = ServiceWithDao.update(Seq(subLane2Split), Set(linkId1), 2, usernameTest, Seq(SideCodesForLinkIds("100L", 2)), allExistingLanes).head
  ...
  test("LaneCodes should be correct two digit codes") {
    ...
    when(mockRoadLinkService.getRoadLinksByLinkIdsFromVVH(Set(linkId1), true)).thenReturn(
    ...
    when(mockRoadLinkService.getRoadLinksByLinkIdsFromVVH(Set(linkId2), true)).thenReturn(
