...
class PavedRoadServiceSpec extends FunSuite with Matchers {
  ...
  when(mockKgvRoadLinkClient.fetchByLinkId("388562360")).thenReturn(Some(RoadLinkFetched("388562360", 235, Seq(Point(0, 0), Point(10, 0)), Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers)))
  when(mockKgvRoadLinkClient.fetchByLinkIds(any[Set[String]])).thenReturn(Seq(RoadLinkFetched("388562360", 235, Seq(Point(0, 0), Point(10, 0)), Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers)))
  when(mockRoadLinkClient.fetchRoadLinkByLinkId(any[String])).thenReturn(Some(RoadLinkFetched("388562360", 235, Seq(Point(0, 0), Point(10, 0)), Municipality, TrafficDirection.UnknownDirection, FeatureClass.AllOthers)))
  ...
  test("Should not create new paved road assets and return the existing paved road assets when VVH doesn't have change information") {
    ...
    when(mockRoadLinkService.getRoadLinksAndComplementariesFromVVH(any[Set[String]], any[Boolean])).thenReturn(Seq(newRoadLink))
    runWithRollback {
  ...
  test("Should apply pavement on whole segment") {
    ...
    val mockRoadLinkClient = MockitoSugar.mock[RoadLinkClient]
    val mockKgvRoadLinkClient = MockitoSugar.mock[KgvRoadLinkClient]
    val timeStamp = new KgvRoadLinkClient(Some(DummyCollection.Dummy),Some(DummySource.Dummy)).createVVHTimeStamp(-5)
  ...
  test("Expire OTH Assets and create new assets based on VVH RoadLink data") {
    ...
    val newRoadLink2 = RoadLinkFetched(newLinkId2, municipalityCode, List(Point(0.0, 0.0), Point(20.0, 0.0)), administrativeClass, trafficDirection, FeatureClass.DrivePath, None, attributes2)
    val newRoadLink3 = RoadLinkFetched(newLinkId3, municipalityCode, List(Point(0.0, 0.0), Point(20.0, 0.0)), administrativeClass, trafficDirection, FeatureClass.DrivePath, None, attributes3)
    val newRoadLink4 = RoadLinkFetched(newLinkId4, municipalityCode, List(Point(0.0, 0.0), Point(20.0, 0.0)), administrativeClass, trafficDirection, FeatureClass.DrivePath, None, attributes3)
    ...
    runWithRollback {
      ...
      when(mockKgvRoadLinkClient.createVVHTimeStamp(any[Int])).thenReturn(12222L)
  ...
  test("Timestamp of the asset is bigger than the vvhChange, should not update asset"){
    ...
    val assets = Seq(PersistedLinearAsset(1, linkId, 1, Some(propertyData), 0, 10, None, None, None, None, false, PavedRoad.typeId, 1461970812000L, None, LinkGeomSource.NormalLinkInterface, None, None, None))
    runWithRollback {
      val changeInfo = createChangeInfo(roadLinks, 11L)
      val (expiredIds, updated) = service.getPavedRoadAssetChanges(assets, roadLinks, changeInfo, PavedRoad.typeId.toLong)
      expiredIds.size should be(0)
  ...
  test("Timestamp of the asset is lower than the vvhChange, should not update asset"){
    ...
    val assets = Seq(PersistedLinearAsset(1, linkId, 1, Some(propertyData), 0, 10, None, None, None, None, false, PavedRoad.typeId, 0, None, LinkGeomSource.NormalLinkInterface, None, None, None))
    runWithRollback {
      val changeInfo = createChangeInfo(roadLinks, 11L)
      val (expiredIds, updated) = service.getPavedRoadAssetChanges(assets, roadLinks, changeInfo, PavedRoad.typeId.toLong)
      expiredIds.size should be(0)
