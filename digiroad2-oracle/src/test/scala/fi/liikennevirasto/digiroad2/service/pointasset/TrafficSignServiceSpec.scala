...
class TrafficSignServiceSpec extends FunSuite with Matchers with BeforeAndAfter {
  ...
  test("Should call creation of manoeuvre actor on traffic sign creation"){
    runWithRollback {
      ...
      verify(mockEventBus, times(1)).publish("trafficSign:create",TrafficSignInfo(asset.id, asset.linkId, asset.validityDirection, NoLeftTurn.OTHvalue, asset.mValue, roadLink, Set()))
    }
  }
