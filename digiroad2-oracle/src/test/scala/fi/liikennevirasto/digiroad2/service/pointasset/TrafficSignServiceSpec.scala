...
class TrafficSignServiceSpec extends FunSuite with Matchers with BeforeAndAfter {
  ...
  test("Create new Traffic Sign") {
    runWithRollback {
      val properties = Set(
        SimpleTrafficSignProperty("trafficSigns_type", List(TrafficSignPropertyValue(TextPropertyValue("1")))),
        SimpleTrafficSignProperty("trafficSigns_value", List(TrafficSignPropertyValue(TextPropertyValue("80")))),
        SimpleTrafficSignProperty("trafficSigns_info", List(TrafficSignPropertyValue(TextPropertyValue("Additional Info for test")))))
  ...
  test("Update Traffic Sign") {
    runWithRollback {
      ...
      val updatedProperties = Set(
        SimpleTrafficSignProperty("trafficSigns_type", List(TrafficSignPropertyValue(TextPropertyValue("2")))),
        SimpleTrafficSignProperty("trafficSigns_value", List(TrafficSignPropertyValue(TextPropertyValue("90")))),
        SimpleTrafficSignProperty("trafficSigns_info", List(TrafficSignPropertyValue(TextPropertyValue("Updated Additional Info for test")))))
  ...
  test("Update traffic sign with geometry changes"){
    runWithRollback {
      ...
      val propertiesToUpdate = Set(
        SimpleTrafficSignProperty("trafficSigns_type", List(TrafficSignPropertyValue(TextPropertyValue("2")))),
        SimpleTrafficSignProperty("trafficSigns_value", List(TrafficSignPropertyValue(TextPropertyValue("60")))),
        SimpleTrafficSignProperty("trafficSigns_info", List(TrafficSignPropertyValue(TextPropertyValue("Additional Info for test")))))
  ...
  test("Update traffic sign without geometry changes"){
    runWithRollback {
      ...
      val propertiesToUpdate = Set(
        SimpleTrafficSignProperty("trafficSigns_type", List(TrafficSignPropertyValue(TextPropertyValue("2")))),
        SimpleTrafficSignProperty("trafficSigns_value", List(TrafficSignPropertyValue(TextPropertyValue("60")))),
        SimpleTrafficSignProperty("trafficSigns_info", List(TrafficSignPropertyValue(TextPropertyValue("Additional Info for update test")))))
  ...
  test("Create traffic sign with direction towards digitizing using coordinates without asset bearing") {
    ...
    runWithRollback {
      val id = service.createFromCoordinates(3, 2, TRTrafficSignType.SpeedLimit, Some(100), Some(false), TrafficDirection.UnknownDirection, None, None)
      val assets = service.getPersistedAssetsByIds(Set(id))
      assets.size should be(1)
      val asset = assets.head
      asset.id should be(id)
  ...
  test("Create traffic sign with direction against digitizing using coordinates without asset bearing") {
    ...
    runWithRollback {
      val id = service.createFromCoordinates(3, 4, TRTrafficSignType.SpeedLimit, Some(100), Some(false), TrafficDirection.UnknownDirection, None, None)
      val assets = service.getPersistedAssetsByIds(Set(id))
      assets.size should be(1)
      val asset = assets.head
      asset.id should be(id)
  ...
  test("Create traffic sign with direction towards digitizing using coordinates with asset bearing") {
    ...
    runWithRollback {
      val id = service.createFromCoordinates(3, 2, TRTrafficSignType.SpeedLimit, Some(100), Some(false), TrafficDirection.UnknownDirection, Some(225), None)
      val assets = service.getPersistedAssetsByIds(Set(id))
      assets.size should be(1)
      val asset = assets.head
      asset.id should be(id)
  ...
  test("Create traffic sign with direction against digitizing using coordinates with asset bearing") {
    /*asset bearing in this case indicates towards which direction the traffic sign is facing, not the flow of traffic*/
    runWithRollback {
      val id = service.createFromCoordinates(3, 4, TRTrafficSignType.SpeedLimit, Some(100), Some(false), TrafficDirection.UnknownDirection, Some(45), None)
      val assets = service.getPersistedAssetsByIds(Set(id))
      assets.size should be(1)
      val asset = assets.head
      asset.id should be(id)
  ...
  test("two-sided traffic signs are effective in both directions ") {
    runWithRollback {
      val id = service.createFromCoordinates(3, 4, TRTrafficSignType.PedestrianCrossing, None, Some(true), TrafficDirection.UnknownDirection, Some(45), None)
      val assets = service.getPersistedAssetsByIds(Set(id))
      assets.size should be(1)
      val asset = assets.head
      asset.id should be(id)
  ...
  test("Create traffic sign with additional information") {
    runWithRollback {
      val id = service.createFromCoordinates(3, 4, TRTrafficSignType.FreeWidth, None, Some(false), TrafficDirection.UnknownDirection, Some(45), Some("Info Test"))
      val assets = service.getPersistedAssetsByIds(Set(id))
      assets.size should be(1)
      val asset = assets.head
      asset.id should be(id)
  ...
  test("Get trafficSigns by radius and sign type") {
    runWithRollback {
      ...
      val propertiesMaximumRestrictions= Set(
        SimpleTrafficSignProperty("trafficSigns_type", List(TrafficSignPropertyValue(TextPropertyValue("3")))),
        SimpleTrafficSignProperty("trafficSigns_value", List(TrafficSignPropertyValue(TextPropertyValue("10")))))
  ...
  test("Pedestrian crossings are filtered") {
    runWithRollback {
      ...
      val properties = Set(
        SimpleTrafficSignProperty("trafficSigns_type", List(TrafficSignPropertyValue(TextPropertyValue("7")))),
        SimpleTrafficSignProperty("trafficSigns_value", List(TrafficSignPropertyValue(TextPropertyValue("")))),
        SimpleTrafficSignProperty("trafficSigns_info", List(TrafficSignPropertyValue(TextPropertyValue("Pedestrian crossing for test purpose")))))
  ...
  test("get by distance with same roadLink, trafficType and Direction") {
      val speedLimitProp = Seq(TrafficSignProperty(0, "trafficSigns_type", "", false, Seq(TrafficSignPropertyValue(TextPropertyValue(TrafficSignType.SpeedLimit.value.toString)))))
      val speedLimitZoneProp = Seq(TrafficSignProperty(0, "trafficSigns_type", "", false, Seq(TrafficSignPropertyValue(TextPropertyValue(TrafficSignType.SpeedLimitZone.value.toString)))))
  ...
  test("Get by municipality and group"){
    ...
    val properties = Set(
      SimpleTrafficSignProperty("trafficSigns_type", List(TrafficSignPropertyValue(TextPropertyValue("82")))),
      SimpleTrafficSignProperty("trafficSigns_value", List(TrafficSignPropertyValue(TextPropertyValue("")))),
      SimpleTrafficSignProperty("trafficSigns_info", List(TrafficSignPropertyValue(TextPropertyValue("Additional Info for test")))))
    ...
    val properties1 = Set(
      SimpleTrafficSignProperty("trafficSigns_type", List(TrafficSignPropertyValue(TextPropertyValue("83")))),
      SimpleTrafficSignProperty("trafficSigns_value", List(TrafficSignPropertyValue(TextPropertyValue("")))),
      SimpleTrafficSignProperty("trafficSigns_info", List(TrafficSignPropertyValue(TextPropertyValue("Additional Info for test")))))
