...
class TrafficSignServiceSpec extends FunSuite with Matchers with BeforeAndAfter {
  ...
  when(mockRoadLinkService.getRoadLinkByLinkIdFromVVH(randomLinkId2)).thenReturn(Seq(
  ...
  test("Get Traffic Signs changes by type group") {
    runWithRollback {
      ...
      when(mockRoadLinkService.getRoadLinksByLinkIdsFromVVH(Set(randomLinkId1), false)).thenReturn(
  ...
  test("Get only one Traffic Sign change"){
    runWithRollback{
      ...
      when(mockRoadLinkService.getRoadLinksByLinkIdsFromVVH(Set(randomLinkId1))).thenReturn(Seq(RoadLink(randomLinkId1, Seq(Point(0.0, 0.0), Point(10.0, 0.0)), 10, Municipality, 1, TrafficDirection.AgainstDigitizing, Motorway, None, None, Map("MUNICIPALITYCODE" -> BigInt(235)))))
