...
class RoadLinkChangeClientSpec extends FunSuite with Matchers {
  ...
  test("merge replace info with same old and new link") {
    ...
    replaceInfo.head.oldLinkId.get should be("350e7577-020e-4abe-bf12-509f070371ed:1")
    replaceInfo.head.newLinkId.get should be("dfdf4f1e-9e12-4dc0-9b44-868320164539:1")
    replaceInfo.head.oldFromMValue.get should be(0)
    replaceInfo.head.oldToMValue.get should be(189.231)
  ...
  test("merge replace info with same old and new link with a gap in between") {
    ...
    sortedReplaceInfo.head.oldFromMValue.get should be(0)
    sortedReplaceInfo.head.oldToMValue.get should be(149.987)
    sortedReplaceInfo.head.newFromMValue.get should be(0)
    sortedReplaceInfo.head.newToMValue.get should be(157.289)
    sortedReplaceInfo.head.digitizationChange should be(false)

    sortedReplaceInfo(1).oldLinkId.get should be("151128c6-a2d5-44b4-a756-391b0b40fff5:1")
    sortedReplaceInfo(1).newLinkId should be(None)
    sortedReplaceInfo(1).oldFromMValue.get should be(149.987)
    sortedReplaceInfo(1).oldToMValue.get should be(206.728)
    sortedReplaceInfo(1).newFromMValue should be(None)
    sortedReplaceInfo(1).newToMValue should be(None)
    sortedReplaceInfo(1).digitizationChange should be(false)

    sortedReplaceInfo.last.oldLinkId.get should be("151128c6-a2d5-44b4-a756-391b0b40fff5:1")
    sortedReplaceInfo.last.newLinkId.get should be("4cc65e5d-4f63-4ace-bc92-fbf8e8013239:2")
    sortedReplaceInfo.last.oldFromMValue.get should be(206.728)
    sortedReplaceInfo.last.oldToMValue.get should be(382.644)
  ...
  test("merge replace info with same old and new link with duplicate info") {
    ...
    sortedReplaceInfo.head.oldLinkId.get should be("8f053004-b583-4d0d-a0da-508aaa7567b2: 1")
    sortedReplaceInfo.head.newLinkId.get should be("8117fcd6-0b75-4147-9a40-152faa5a5f9c: 1")
    sortedReplaceInfo.head.oldFromMValue.get should be(0)
    sortedReplaceInfo.head.oldToMValue.get should be(471.388)
    ...
    sortedReplaceInfo.last.oldLinkId.get should be("8f053004-b583-4d0d-a0da-508aaa7567b2: 1")
    sortedReplaceInfo.last.newLinkId.get should be("8117fcd6-0b75-4147-9a40-152faa5a5f9c: 1")
    sortedReplaceInfo.last.oldFromMValue.get should be(685.718)
    sortedReplaceInfo.last.oldToMValue.get should be(786.052)
    sortedReplaceInfo.last.newFromMValue.get should be(627.487)
    sortedReplaceInfo.last.newToMValue.get should be(727.409)
    sortedReplaceInfo.last.digitizationChange should be(false)
  }

  test("merge replace info with same old and new link containing a continuous and a removed part") {
    val replaceInfo = changes(39).replaceInfo
    replaceInfo.size should be(2)
    val sortedReplaceInfo = replaceInfo.sortBy(_.oldFromMValue)

    sortedReplaceInfo.head.oldLinkId.get should be("5c7a3ed4-b500-4106-9da4-0fd91a150b7c: 1")
    sortedReplaceInfo.head.newLinkId.get should be("c8532192-2c1e-4d24-a654-98aa6e642139: 1")
    sortedReplaceInfo.head.oldFromMValue.get should be(0)
    sortedReplaceInfo.head.oldToMValue.get should be(110.265)
    sortedReplaceInfo.head.newFromMValue.get should be(0)
    sortedReplaceInfo.head.newToMValue.get should be(111.852)
    sortedReplaceInfo.head.digitizationChange should be(false)

    sortedReplaceInfo.last.oldLinkId.get should be("5c7a3ed4-b500-4106-9da4-0fd91a150b7c: 1")
    sortedReplaceInfo.last.newLinkId should be(None)
    sortedReplaceInfo.last.oldFromMValue.get should be(110.265)
    sortedReplaceInfo.last.oldToMValue.get should be(127.117)
    sortedReplaceInfo.last.newFromMValue should be(None)
    sortedReplaceInfo.last.newToMValue should be(None)
    sortedReplaceInfo.last.digitizationChange should be(false)
  }

  test("merge replace info for add with continuous values") {
    val replaceInfo = changes(40).replaceInfo
    replaceInfo.size should be(1)

    replaceInfo.head.oldLinkId should be(None)
    replaceInfo.head.newLinkId.get should be("e24e9e6c-e011-4889-9ca4-4a13734d0f41: 1")
    replaceInfo.head.oldFromMValue should be(None)
    replaceInfo.head.oldToMValue should be(None)
    replaceInfo.head.newFromMValue.get should be(0)
