...
import fi.liikennevirasto.digiroad2.asset.{AutoGeneratedUsername, AdministrativeClass => AdminClassType}
    ...
    if (linkValues.nonEmpty) {
      ...
      try {
        linkValues.foreach { case (linkId, value) =>
          ...
          preparedStatement.setString(4, username.getOrElse(AutoGeneratedUsername.automaticGeneration))
      ...
      if (linkValues.nonEmpty) {
        ...
        try {
          linkValues.foreach { case (linkId, value) =>
            preparedStatement.setString(1, linkId)
            preparedStatement.setInt(2, value)
            preparedStatement.setString(3, username.getOrElse(AutoGeneratedUsername.automaticGeneration))
        ...
        try {
          linkAttributes.foreach { case (linkId, attributes) =>
            attributes.foreach { case (attributeName, attributeValue) =>
              ...
              preparedStatement.setString(5, username.getOrElse(AutoGeneratedUsername.automaticGeneration))
