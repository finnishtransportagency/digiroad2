...
import fi.liikennevirasto.digiroad2.asset.{AssetTypeInfo, Manoeuvres, ServicePoints}
...
object HistoryDAO {
  ...
  def getExpiredAssetsIdsByAssetTypeAndYearGap(assetType: AssetTypeInfo, yearGap: Int) = {
    val yearGapCondition = s"EXTRACT(YEAR FROM VALID_TO) < EXTRACT(YEAR FROM SYSDATE) - $yearGap"

    assetType match {
      case Manoeuvres =>
        sql"""
          SELECT ID FROM MANOEUVRE WHERE #$yearGapCondition
        """.as[Long].list
      case _ =>
        sql"""
          SELECT ID FROM ASSET WHERE ASSET_TYPE_ID = ${assetType.typeId} AND #$yearGapCondition
        """.as[Long].list
    }
  }
