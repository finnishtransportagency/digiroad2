...
class LaneDao(){
  ...
  def updateLanePositionAndModifiedDate(laneId: Long, linkId: String, startMeasure: Double, endMeasure: Double, username: String ): Unit = {
    sqlu"""UPDATE LANE_POSITION
          SET start_measure = $startMeasure, end_measure = $endMeasure, link_id = $linkId, modified_date = current_timestamp
