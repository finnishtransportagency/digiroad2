...
class VerificationDao {
  ...
  def getModifiedAssetTypes(linkIds : Set[Long]) : List[LatestModificationInfo] = {
    val modifiedAssetTypes =
      if (linkIds.size > 30000) {
        logger.info(s"BATCH LOG Using mass query (${linkIds.size} links)")
        MassQuery.withIds(linkIds) { idTableName =>
          queryModifiedAssetTypes(s"left join $idTableName i on i.id = lrm.link_id where i.id is not null")
        }
      } else {
        logger.info(s"BATCH LOG Querying with id list (${linkIds.size} links)")
        queryModifiedAssetTypes(s"where lrm.link_id in (${linkIds.mkString(",")})")
      }
  ...
  def queryModifiedAssetTypes(idQuery: String): List[(Int, Option[String], Option[DateTime])] = {
