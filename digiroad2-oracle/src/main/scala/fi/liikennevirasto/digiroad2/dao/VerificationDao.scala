...
class VerificationDao {
  ...
  def getSuggestedLinearAssets(ids: Set[Long]): Seq[(Long, Int)] = {
    MassQuery.withIds(ids) { idTableName =>
      ...
      select a.id, a.asset_type_id
        ...
        join enumerated_value ev on ev.property_id = p.id and ev.value = 1 and lrm.link_id in (select id from #$idTableName)
        """.as[(Long, Int)].list
    }
  }
  ...
  def getSuggestedPointAssets(municipalityCode: Int): Seq[(Long, Int)] = {
    ...
    select a.id, a.asset_type_id
      from asset a
      join property p on p.asset_type_id = a.asset_type_id and p.property_type = 'checkbox' and p.public_id = 'obstacle_suggest_box'
      join multiple_choice_value mcv on mcv.asset_id = a.id and mcv.property_id = p.id
      join enumerated_value ev on ev.property_id = p.id and ev.value = 1 where a.municipality_code = #$municipalityCode
    """.as[(Long, Int)].list
  }
