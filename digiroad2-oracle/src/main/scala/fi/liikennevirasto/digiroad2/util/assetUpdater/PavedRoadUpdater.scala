...
class PavedRoadUpdater(service: PavedRoadService) extends DynamicLinearAssetUpdater(service) {
  ...
  private def selectOnlyHumanOrBatchCreated(asset: PersistedLinearAsset): Boolean = {
    val isbatchCreated = asset.createdBy.contains(AutoGeneratedUsername.batchProcessPrefix + "pavedRoad")
    //Regex to match user created assets. Username must start by 'k' or 'lx', followed by six digits. Case insensitive
    val userCreatedRegex = "^(?i)(k|lx)\\d{6}$".r
    val isUserCreated = userCreatedRegex.findFirstIn(asset.createdBy.getOrElse("")).nonEmpty

    isbatchCreated || isUserCreated
  }
  ...
  private def removePavement(operatio: OperationStep,changes: Seq[RoadLinkChange]) = {
    ...
    val assetsNotCreatedByUserOrBatch = assetsAll.filterNot(selectOnlyHumanOrBatchCreated)
    val expiredPavement = assetsNotCreatedByUserOrBatch.filter(a => changesRemovePavement.contains(a.linkId)).map(asset => {
        if (asset.id != 0) {
