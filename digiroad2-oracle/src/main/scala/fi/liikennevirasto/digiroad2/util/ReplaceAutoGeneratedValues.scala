...
object ReplaceAutoGeneratedUsernames {
  ...
  def main(tranactionOpen:Boolean)  = {
    tablesWithValuesToReplace.foreach {
      tableName => columnsWithValuesToReplace.foreach {
        columnName => valuesToReplace.foreach {
          value =>
            val idGroups = selectIdsToUpdate(tableName, columnName, value._1, tranactionOpen).grouped(batchSize).toSeq
            idGroups.foreach {
              ids =>
                try {
                  updateColumnValues(tableName, columnName, ids, value._2, tranactionOpen)
