...
import fi.liikennevirasto.digiroad2.service.{AdjustedRoadLinksAndRoadLinkFetched, IncompleteLink, RoadLinkChangeSet, RoadLinkService}
...
object UpdateIncompleteLinkList {
  ...
  def getRoadLinksFromVvhAndUpdateProperties(municipality: Int): Seq[RoadLink] = {
    val (links, _, complementaryLinks) = LogUtils.time(logger, "Get roadlinks and update properties for municipality " + municipality)(
      roadLinkService.getLinksWithComplementaryAndGenerateProperties(municipality))
  ...
  /**
    ...
    *
    * @param allRoadLinkFetcheds
    * @param changes
    * @return Road links
    */
  def generateProperties(allRoadLinkFetcheds: Seq[RoadLinkFetched], changes: Seq[ChangeInfo] = Nil): Seq[RoadLink] = {
    val vvhRoadLinks = allRoadLinkFetcheds.filterNot(_.featureClass == FeatureClass.WinterRoads)
    ...
    def autoGenerateProperties(roadLink: RoadLink): RoadLink = {
      ...
      vvhRoadLink.get.featureClass match {
        ...
        case FeatureClass.HardShoulder => roadLink.copy(functionalClass = 9, linkType = HardShoulder, modifiedBy = Some("automatic_generation"), modifiedAt = Some(DateTimePropertyFormat.print(DateTime.now())))
    ...
    val vvhRoadLinksGroupBy = allRoadLinkFetcheds.groupBy(_.linkId)
    val pair = adjustedRoadLinks.map(r => AdjustedRoadLinksAndRoadLinkFetched(r, vvhRoadLinksGroupBy(r.linkId).last))
    ...
    LogUtils.time(logger, "TEST LOG UpdateRoadLinkChanges") {
      try {
        updateRoadLinkChanges(changeSet)
         ...
         case sqle: SQLException => logger.error("Updating road link attributes failed with message: " + sqle.getMessage)
      }
    }
  ...
  def updateRoadLinkChanges(roadLinkChangeSet: RoadLinkChangeSet): Unit = {
    ...
    LogUtils.time(logger, s"updateIncompleteLinks incompleteLinks: ${roadLinkChangeSet.incompleteLinks.size},") {
      roadLinkService.updateIncompleteLinks(roadLinkChangeSet.incompleteLinks)
    }

    LogUtils.time(logger, s"updateAutoGeneratedProperties adjustedRoadLinks: ${roadLinkChangeSet.adjustedRoadLinks.size},") {
      roadLinkService.updateAutoGeneratedProperties(roadLinkChangeSet.adjustedRoadLinks)
    }

    LogUtils.time(logger, s"fillRoadLinkAttributes roadLinks: ${roadLinkChangeSet.roadLinks.size}, changes: ${roadLinkChangeSet.changes.size},") {
