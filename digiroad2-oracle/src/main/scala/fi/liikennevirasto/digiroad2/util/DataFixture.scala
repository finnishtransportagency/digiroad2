  ...
  def fillRoadWidthInRoadLink(): Unit = {
    ...
    municipalities.foreach { municipality =>
      ...
      PostGISDatabase.withDynTransaction {
        ...
        val expiredAssetsIds = changedAssets.flatMap {
          case (_, changeInfo, assets) =>
            assets.filter(asset => asset.modifiedBy.getOrElse(asset.createdBy.getOrElse("")) == "dr1_conversion" ||
              (asset.timeStamp < changeInfo.timeStamp && (asset.modifiedBy.getOrElse(asset.createdBy.getOrElse("")) == AutoGeneratedValues.mtkClassDefault.toString ||
                asset.modifiedBy.getOrElse("") == AutoGeneratedValues.generatedInUpdate.toString && asset.createdBy.getOrElse("") == AutoGeneratedValues.mtkClassDefault.toString))
            ).map(_.id)
        }.toSet
          ...
          case (roadLink, changeInfo, allAssets) =>
            ...
            pieces.flatMap { measures =>
              Some(PersistedLinearAsset(0L, roadLink.linkId, SideCode.BothDirections.value, Some(NumericValue(roadLink.extractMTKClass(roadLink.attributes).width)),
                measures._1, measures._2, Some(AutoGeneratedValues.mtkClassDefault.toString), None, None, None, false, roadWidthAssetTypeId, changeInfo.timeStamp, None, linkSource = roadLink.linkSource, Some(AutoGeneratedValues.mtkClassDefault.toString), None, None))
            }.filterNot(a =>
              assets.
        ...
        expiredAssetsIds.foreach(dao.updateExpiration(_, expired = true, AutoGeneratedValues.mtkClassDefault.toString))
        ...
        newAssets.foreach { linearAsset =>
          ...
          val id = dao.createLinearAsset(linearAsset.typeId, linearAsset.linkId, linearAsset.expired, linearAsset.sideCode,
            Measures(linearAsset.startMeasure, linearAsset.endMeasure), linearAsset.createdBy.getOrElse(AutoGeneratedValues.mtkClassDefault.toString), linearAsset.timeStamp, Some(roadLink.linkSource.value), geometry = roadLink.geometry)
    ...
    municipalities.foreach { municipality =>
      ...
      PostGISDatabase.withDynTransaction {
        ...
        existingAssets.foreach { asset =>
          if(asset.createdBy.contains(AutoGeneratedValues.mtkClassDefault.toString) && (asset.modifiedBy.isEmpty || asset.modifiedBy.contains(AutoGeneratedValues.generatedInUpdate.toString))){
            ...
            if(( (asset.createdBy.contains(AutoGeneratedValues.dr1Conversion.toString) || asset.createdBy.contains(AutoGeneratedValues.generatedInUpdate.toString))&& asset.modifiedBy.isEmpty)  ||
              (asset.createdBy.contains(AutoGeneratedValues.dr1Conversion.toString) && asset.modifiedBy.contains(AutoGeneratedValues.generatedInUpdate.toString))) {
              if(!asset.informationSource.contains(MunicipalityMaintenainer)) {
              ...
              if (asset.createdBy.contains("batch_process_roadWidth") && (asset.modifiedBy.isEmpty || asset.modifiedBy.contains(AutoGeneratedValues.generatedInUpdate.toString))) {
                if (!asset.informationSource.contains(MunicipalityMaintenainer))
  ...
  def updatePavedRoadInformationSource(): Unit = {
    ...
    municipalities.foreach { municipality =>
      ...
      PostGISDatabase.withDynTransaction {
        ...
        existingAssets.foreach { asset =>
          if (asset.createdBy.contains("batch_process_pavedRoad") && (asset.modifiedBy.isEmpty || asset.modifiedBy.contains(AutoGeneratedValues.generatedInUpdate.toString))) {
            ...
            if (isKIdentifier(asset.createdBy) || isKIdentifier(asset.modifiedBy)) {
              ...
              if (asset.createdBy.contains(AutoGeneratedValues.generatedInUpdate.toString) && (asset.modifiedBy.isEmpty || asset.modifiedBy.contains(AutoGeneratedValues.generatedInUpdate.toString))) {
  ...
  def fillNewRoadLinksWithPreviousInfo(): Unit = {
    ...
    def createNewSpeedLimits(newSpeedLimits: Seq[SpeedLimit], roadLink: RoadLink): Unit = {
      ...
      newSpeedLimits.foreach { speedLimit =>
        speedLimitDao.createSpeedLimit(LinearAssetTypes.generatedInUpdate, speedLimit.linkId, Measures(speedLimit.startMeasure, speedLimit.endMeasure), speedLimit.sideCode, speedLimit.value.get, Some(LinearAssetUtils.createTimeStamp()), linkSource = roadLink.linkSource)
