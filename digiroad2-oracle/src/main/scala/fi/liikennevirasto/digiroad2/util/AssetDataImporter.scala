...
class AssetDataImporter {
  ...
  def importEuropeanRoads(conversionDatabase: DatabaseDef, vvhHost: String) = {
    ...
    val linksByLinkId = vvhLinks.foldLeft(Map.empty[String, VVHRoadlink]) { (m, link) => m + (link.linkId -> link) }
  ...
  def convertToProhibitions(prohibitionSegments: Seq[(Long, String, Double, Double, Int, Int, Int, Option[String])], roadLinks: Seq[VVHRoadlink], exceptions: Seq[(Long, String, Int, Int)]): Seq[Either[String, PersistedLinearAsset]] = {
