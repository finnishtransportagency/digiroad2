...
case class MatchedRoadLinks(id: Long = 0, addedLink: RoadLinkInfo, removedLink: RoadLinkInfo, hausdorffSimilarityMeasure: Double, areaSimilarityMeasure: Double)
...
object RoadLinkReplacementFinder {
  ...
  def findMissingReplacements(changes: Seq[RoadLinkChange]) = {
  ...
  def matchRemovesAndAdds(replaceInfosWithGeometry: Seq[ReplaceInfoWithGeometry]) = {
    ...
    def findMatchesWithSimilarity(addedLink: ReplaceInfoWithGeometry, removedLinks: Seq[ReplaceInfoWithGeometry]) = {
    ...
    val matchedLinksByParallel = added.map(addedLink => (addedLink, findMatchesWithSimilarity(addedLink, removed)))
  }
