  ...
  def updateChangeSet(changeSet: ChangeSet) : Unit = {
    def treatChangeSetData(changeSetToTreat: Seq[baseAdjustment]): Unit = {
      ...
      changeSetToTreat.foreach { adjustment =>
        ...
        if(oldLane.nonEmpty){
          ...
          val newLaneID = create(Seq(newLane), Set(oldLane.get.linkId), oldLane.get.sideCode, AutoGeneratedUsername.generatedInUpdate)
          moveToHistory(oldLane.get.id, Some(newLaneID.head), true, true, AutoGeneratedUsername.generatedInUpdate)
        }
        else{
    ...
    def persistedToNewLaneWithNewMeasures(persistedLane: PersistedLane, newStartMeasure: Double, newEndMeasure: Double): NewLane = {
      ...
      changeSet.adjustedSideCodes.foreach { adjustment =>
        moveToHistory(adjustment.laneId, None, false, false, AutoGeneratedUsername.generatedInUpdate)
      ...
      ids.foreach(moveToHistory(_, None, true, true, AutoGeneratedUsername.generatedInUpdate))
