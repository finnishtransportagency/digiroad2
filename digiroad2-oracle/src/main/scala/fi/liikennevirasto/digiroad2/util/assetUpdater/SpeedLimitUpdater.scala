...
class SpeedLimitUpdater(service: SpeedLimitService) extends DynamicLinearAssetUpdater(service) {
  ...
  override def persistProjectedLinearAssets(newLinearAssets: Seq[PersistedLinearAsset]): Unit = {
    ...
    newlimits.foreach { limit =>
      // TODO remove this get creating one by one
  ...
  protected override def adjustedSideCode(adjustment: SideCodeAdjustment, oldAssets: Seq[PersistedLinearAsset], roadLinks: Seq[RoadLink]): Unit = {
    val oldSpeedLimit = oldAssets.find(_.id == adjustment.assetId).getOrElse(throw new IllegalStateException("Asset no longer available"))
    ...
    logger.debug("SideCodeAdjustment newID" + newId)
  }
}
