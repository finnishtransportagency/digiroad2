...
class SpeedLimitUpdater(eventbus: DigiroadEventBus, roadLinkService: RoadLinkService, service: SpeedLimitService) {
  ...
  def updateChangeSet(changeSet: ChangeSet) : Unit = {
    ...
    changeSet.adjustedVVHChanges.foreach { adjustment =>
      dao.updateMValuesChangeInfo(adjustment.assetId, (adjustment.startMeasure, adjustment.endMeasure), adjustment.timeStamp, LinearAssetTypes.generatedInUpdate)
    }
    ...
    changeSet.expiredAssetIds.toSeq.foreach(dao.updateExpiration(_, expired = true, LinearAssetTypes.generatedInUpdate))
  ...
  def persistProjectedLimit(limits: Seq[SpeedLimit]): Unit = {
    ...
    newlimits.foreach { limit =>
      dao.createSpeedLimit(limit.createdBy.getOrElse(LinearAssetTypes.generatedInUpdate), limit.linkId, Measures(limit.startMeasure, limit.endMeasure),
  ...
  def adjustedSideCode(adjustment: SideCodeAdjustment): Unit = {
    ...
    service.updateByExpiration(oldSpeedLimit.id, true, LinearAssetTypes.generatedInUpdate, false)
    val newId = service.createWithoutTransaction(Seq(NewLimit(oldSpeedLimit.linkId, oldSpeedLimit.startMeasure, oldSpeedLimit.endMeasure)), SpeedLimitValue(oldSpeedLimit.value.get.value, oldSpeedLimit.value.get.isSuggested), LinearAssetTypes.generatedInUpdate, adjustment.sideCode)
