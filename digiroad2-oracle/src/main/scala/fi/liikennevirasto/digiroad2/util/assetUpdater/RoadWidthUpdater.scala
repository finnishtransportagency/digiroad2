...
class RoadWidthUpdater(service: RoadWidthService) extends DynamicLinearAssetUpdater(service) {
  ...
  override def operationForNewLink(change: RoadLinkChange, assetsAll: Seq[PersistedLinearAsset], changeSets: ChangeSet): Option[OperationStep] = {
    ...
    if (newLinks.adminClass != State && roadWidth.value != MTKClassWidth.Unknown.value){
      ...
      Some(OperationStep(Seq(newAsset), Some(changeSets),Some(change),newLinks.linkId,Seq()))
    }else {
      None
    }
  }

  override def additionalUpdateOrChangeReplace(change: RoadLinkChange, operatio: OperationStep, assetsAll: Seq[PersistedLinearAsset], changeSets: ChangeSet): Option[OperationStep] ={
    change.changeType match {
    // expire and recreate width
      case RoadLinkChangeType.Replace =>
        expireAndCreate(change, assetsAll, changeSets,operatio)
      case _ =>  None
    }
  }

  override def additionalUpdateOrChangeSplit(change: RoadLinkChange, operatio: OperationStep, assetsAll: Seq[PersistedLinearAsset], changeSets: ChangeSet): Option[OperationStep] = {
    change.changeType match {
      // expire and recreate width
  ...
  private def expireAndCreate(change: RoadLinkChange, assetsAll: Seq[PersistedLinearAsset], changeSets: ChangeSet,operatio: OperationStep) = {
    ...
    val filterOnlyRelevant = assetsAll.filter(selectOnlyMachineCreated).filter(a => newLinksMapped.map(_._1).contains(a.linkId))
    if (filterOnlyRelevant.nonEmpty) {
      val systemEditedUpdated = filterOnlyRelevant.map(asset => {
        ...
        if (asset.id != 0) {
          val operation = operatio.copy(
            assetsAfter = Seq(PersistedLinearAsset(0, replace._1, sideCode = SideCode.BothDirections.value, value = Some(NumericValue(replace._3.width)),
          ...
          // other solution would be mark this as addition and not use property changes,''
          //propertyChangesBasedOnLink.append((asset.id, 0))
          //operation
          reportAssetChanges(operation.assetsBefore.find(_.id == asset.id), operation.assetsAfter.headOption, Seq(change), operation, ChangeTypeReport.PropertyChange, useGivenChange = true)
        } else {
          val operation = operatio.copy(
            assetsAfter = Seq(asset.copy(value = Some(NumericValue(replace._3.width)))),
            changeInfo = Some(changeSets)
          )
          if (extractPropertyValue("width", asset.value.get.asInstanceOf[DynamicValue].value.properties).head != replace._3.width.toString) {
            reportAssetChanges(
              Some(asset), Some(asset.copy(value = Some(NumericValue(replace._3.width)))), Seq(change),
              operation
              , ChangeTypeReport.PropertyChange, useGivenChange = true)
          } else {
            reportAssetChanges(None, Some(asset.copy(value = Some(NumericValue(replace._3.width)))), Seq(change),
              operation
              , ChangeTypeReport.Creation, useGivenChange = true)
          }
        }
      })
      val merge = systemEditedUpdated.foldLeft(OperationStep(assetsAll, Some(changeSets), Some(change)))((a, b) => {
        OperationStep((a.assetsAfter ++ b.assetsAfter).distinct, Some(LinearAssetFiller.combineChangeSets(a.changeInfo.get, b.changeInfo.get)), a.roadLinkChange, b.newLinkId, b.assetsBefore)
      })
      Some(merge)
    } else None
  }
