        ...
        if (asset.id != 0) {
          if (widthNeedUpdating(asset, newLink)) {
            val modifiedAsset = PersistedLinearAsset(asset.id, newLink.linkId,
              ...
              informationSource = Some(MmlNls), oldId = asset.id, externalIds = asset.externalIds
            )
      ...
      val id = (linearAsset.createdBy, linearAsset.createdDateTime) match {
        case (Some(createdBy), Some(createdDateTime)) =>
          dao.createLinearAsset(linearAsset.typeId, linearAsset.linkId, linearAsset.expired, linearAsset.sideCode,
            ...
            service.getLinkSource(roadLink), fromUpdate = true, Some(createdBy), Some(createdDateTime), linearAsset.modifiedBy, linearAsset.modifiedDateTime, linearAsset.verifiedBy, linearAsset.verifiedDate, Some(MmlNls.value), geometry = service.getGeometry(roadLink), externalIds = linearAsset.externalIds)
        case _ =>
          dao.createLinearAsset(linearAsset.typeId, linearAsset.linkId, linearAsset.expired, linearAsset.sideCode,
            ...
            service.getLinkSource(roadLink), verifiedBy = linearAsset.verifiedBy, informationSource = Some(MmlNls.value), geometry = service.getGeometry(roadLink), externalIds = linearAsset.externalIds)
      }
