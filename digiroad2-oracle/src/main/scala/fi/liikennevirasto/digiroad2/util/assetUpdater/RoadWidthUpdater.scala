...
class RoadWidthUpdater(service: RoadWidthService) extends DynamicLinearAssetUpdater(service) {
      ...
      newLinksWithAssetsGroup.flatMap(newLinkWithAssets => {
        ...
        assets.map(asset => {
          if (asset.id != 0) {
            ...
            val operation = Some(operationStep.copy(
  ...
  private def handleGeneratedPart(changeSets: ChangeSet, asset: PersistedLinearAsset, newLink: RoadLinkInfo, operationStep: OperationStep) = {
    ...
    val operationUpdated = Some(operationStep.copy( assetsAfter = Seq(asset.copy(value = Some(newValue))), changeInfo = Some(changeSets)))
    if (widthNeedUpdating(asset, newLink)) operationUpdated else  Some(operationStep.copy(assetsAfter = Seq(asset)))
  }
