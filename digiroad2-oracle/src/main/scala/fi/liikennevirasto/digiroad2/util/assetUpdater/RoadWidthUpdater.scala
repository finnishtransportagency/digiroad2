    ...
    // expire and recreate width
      case RoadLinkChangeType.Replace | RoadLinkChangeType.Split =>
       ...
       val systemEditedUpdated =  assetsAll.filter(selectOnlyMachineCreated).filter(a=>newLinksMapped.map(_._1).contains(a.linkId)).map(asset => {
         ...
         if (asset.id!=0) {
           val operation = OperationStep(
             Seq(PersistedLinearAsset(0, replace._1, sideCode = SideCode.BothDirections.value, value = Some(NumericValue(replace._3.width)),
             ...
             linkSource = LinkGeomSource.NormalLinkInterface, verifiedBy = service.getVerifiedBy(AutoGeneratedUsername.mtkClassDefault, LinearAssetTypes.RoadWidthAssetTypeId), verifiedDate = None,
             informationSource = Some(MmlNls))),
             Some(changeSets.copy(expiredAssetIds = changeSets.expiredAssetIds ++ Set(asset.id))),Some(change),replace._1,Seq(asset)
           )
           reportAssetChanges(Some(asset),operation.assetsAfter.headOption, Seq(change), operation,ChangeTypeReport.PropertyChange,useGivenChange = true)
         }else {
           reportAssetChanges(Some(asset),Some(asset.copy(value = Some(NumericValue(replace._3.width)))), Seq(change),  
             OperationStep (Seq(asset.copy(value = Some(NumericValue(replace._3.width)))),Some(changeSets),
               Some(change),replace._1,Seq(asset)
             )
