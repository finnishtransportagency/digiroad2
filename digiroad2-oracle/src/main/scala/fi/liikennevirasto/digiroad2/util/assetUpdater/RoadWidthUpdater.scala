...
import fi.liikennevirasto.digiroad2.asset.{AutoGeneratedValues, DynamicProperty, DynamicPropertyValue, MmlNls, UnknownLinkType}
...
class RoadWidthUpdater(service: RoadWidthService) extends DynamicLinearAssetUpdater(service) {
      ...
      val id = (linearAsset.createdBy, linearAsset.createdDateTime) match {
        case (Some(createdBy), Some(createdDateTime)) =>
          dao.createLinearAsset(linearAsset.typeId, linearAsset.linkId, linearAsset.expired, linearAsset.sideCode,
            Measures(linearAsset.startMeasure, linearAsset.endMeasure), linearAsset.modifiedBy.getOrElse(LinearAssetTypes.generatedInUpdate), linearAsset.timeStamp,
        ...
        case _ =>
          dao.createLinearAsset(linearAsset.typeId, linearAsset.linkId, linearAsset.expired, linearAsset.sideCode,
            Measures(linearAsset.startMeasure, linearAsset.endMeasure), linearAsset.createdBy.getOrElse(LinearAssetTypes.generatedInUpdate), linearAsset.timeStamp,
  ...
  override def updateChangeSet(changeSet: ChangeSet) : Unit = {
    ...
    ids.foreach(dao.updateExpiration(_, expired = true, AutoGeneratedValues.mtkClassDefault.toString))
