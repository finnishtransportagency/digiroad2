...
import fi.liikennevirasto.digiroad2.asset.{AutoGeneratedUsername, DynamicProperty, DynamicPropertyValue, InformationSource, LinkGeomSource, MmlNls, SideCode, UnknownLinkType}
...
class RoadWidthUpdater(service: RoadWidthService) extends DynamicLinearAssetUpdater(service) {
  ...
  override def operationForNewLink(change: RoadLinkChange, assetsAll: Seq[PersistedLinearAsset], changeSets: ChangeSet): Seq[(PersistedLinearAsset, ChangeSet)] = {
    // TODO here logic to generate width road
    // hint getRoadWidthAssetChanges Remove after coping main logic
    val roadWidth = MTKClassWidth(change.newLinks.head.roadClass)
    val newAsset = PersistedLinearAsset(0, change.newLinks.head.linkId, sideCode = SideCode.BothDirections.value, value = Some(NumericValue(roadWidth.width)),
      startMeasure = 0, endMeasure = GeometryUtils.geometryLength(change.newLinks.head.geometry), createdBy = Some(AutoGeneratedUsername.mtkClassDefault),
...
// reference point only
  ...
  override def updateChangeSet(changeSet: ChangeSet) : Unit = {
    ...
    changeSet.adjustedVVHChanges.foreach { adjustment =>
      dao.updateMValuesChangeInfo(adjustment.assetId,adjustment.linkId, Measures(adjustment.startMeasure, adjustment.endMeasure), adjustment.timeStamp, AutoGeneratedUsername.generatedInUpdate)
    }
