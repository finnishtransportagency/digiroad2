...
class RoadWidthUpdater(service: RoadWidthService) extends DynamicLinearAssetUpdater(service) {
  ...
  private def expireAndCreate(operationStep: OperationStep) = {
    ...
    val filterOnlyRelevant = operationStep.assetsAfter.filter(selectOnlyMachineCreated).filter(a => newLinksMapped.map(_._1).contains(a.linkId))
    ...
    if (filterOnlyRelevant.nonEmpty) {
      ...
      val merge = systemEditedUpdated.foldLeft(OperationStep(operationStep.assetsAfter, Some(changeSets), Some(change)))((a, b) => {
  ...
  private def handleGeneratedPart(change: RoadLinkChange, changeSets: ChangeSet, asset: PersistedLinearAsset, replace: (String, AdministrativeClass, MTKClassWidth, Double),operationStep: OperationStep) = {
    ...
    if (extractPropertyValue("width", asset.value.get.asInstanceOf[DynamicValue].value.properties).head != replace._3.width.toString && oldAsset.isDefined) {
      reportAssetChanges(
        oldAsset, Some(asset.copy(value = Some(newValue))), Seq(change),
        operationUpdated
        , ChangeTypeReport.PropertyChange)
    } else {
      if (oldAsset.isEmpty) { 
        ...
        reportAssetChanges(None, Some(asset.copy(value = Some(newValue))), Seq(change),
          operationUpdated, ChangeTypeReport.Creation)
      } else {
        operationUpdated
      }
    }
