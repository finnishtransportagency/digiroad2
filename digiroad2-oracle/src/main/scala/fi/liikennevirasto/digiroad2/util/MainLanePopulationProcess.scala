...
import fi.liikennevirasto.digiroad2.asset.{AutoGeneratedUsername, CycleOrPedestrianPath, LinkType, MotorwayServiceAccess, SpecialTransportWithGate, SpecialTransportWithoutGate, TractorRoad, TrafficDirection}
...
object MainLanePopulationProcess {
  ...
  private def mainLanesForMunicipality(municipality: Int, initialProcessing: Boolean): Unit = {
    ...
    if (municipalityMainLanes.nonEmpty) {
      val lanes = laneService.createMultipleLanes(municipalityMainLanes, AutoGeneratedUsername.autoGeneratedLane, newTransaction = true)
