...
object MainLanePopulationProcess {
  ...
  def createMainLanesForRoadLinks(roadLinksWithoutMainLanes: Seq[RoadLink], saveResult: Boolean = true): Seq[PersistedLane] = {
    ...
    if (municipalityMainLanes.nonEmpty && saveResult) {
      val lanes = laneService.createMultipleLanes(municipalityMainLanes, AutoGeneratedUsername.autoGeneratedLane)
