...
class RoadLinkPropertyUpdater {
  ...
  def transferOverriddenPropertiesAndPrivateRoadInfo(changes: Seq[RoadLinkChange]) = {
    changes.foreach { change =>
      ...
      optionalOverriddenAdminClass match {
        case Some(overriddenAdminClass) =>
          change.newLinks.foreach { newLink =>
            ...
            AdministrativeClassDao.expireValues(oldLink.linkId, Some(AutoGeneratedUsername.automaticAdjustment), Some(LinearAssetUtils.createTimeStamp()))
          }
        case _ => //do nothing
      }
      ...
      LinkAttributesDao.expireValues(oldLink.linkId, Some(AutoGeneratedUsername.automaticAdjustment), Some(LinearAssetUtils.createTimeStamp()))
    }
  }
