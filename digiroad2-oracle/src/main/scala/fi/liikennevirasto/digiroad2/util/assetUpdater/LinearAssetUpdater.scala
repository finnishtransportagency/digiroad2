...
class LinearAssetUpdater(service: LinearAssetOperations) {
  ...
  private def handleSplits(typeId: Int, links: Seq[RoadLink], changeSet: ChangeSet,
    ...
    val updatedOperationStep = updateOperationStepWithExpiredIds(operationStep, getIdsForAssetsOutsideGeometry(adjusted))
  ...
  private def updateSplitAdjustmentsWithExpiredIds(linksAndOperations: Seq[LinkAndOperation]): Seq[LinkAndOperation] = {
    def updateLinkAndOperationIfEmptyNewLinkId(linkAndOperation: LinkAndOperation, expiredIds: Set[Some[Long]]): LinkAndOperation = {
    ...
    val expiredIds = oldAssetsOutsideGeometry
      .map(oldAssetToExpire => Some(oldAssetToExpire.id)).toSet//service.expireAsset(oldAssetToExpire.typeId, oldAssetToExpire.id, AutoGeneratedUsername.generatedInUpdate, expired = true, newTransaction = false))
    val updatedLinksAndOperations = linksAndOperations.map(linkAndOperation => updateLinkAndOperationIfEmptyNewLinkId(linkAndOperation, expiredIds))
    updatedLinksAndOperations
  }

  private def getIdsForAssetsOutsideGeometry(linksAndOperations: Seq[LinkAndOperation]): Set[Option[Long]] = {
    val oldAssetsInsideGeometry = linksAndOperations.filter(_.newLinkId.nonEmpty).flatMap(_.operation.assetsAfter).toSet
    val oldAssetsOutsideGeometry = linksAndOperations
      .filter(_.newLinkId.isEmpty).flatMap(_.operation.assetsBefore)
      .filter(asset => !oldAssetsInsideGeometry.map(_.id).contains(asset.id)).toSet
