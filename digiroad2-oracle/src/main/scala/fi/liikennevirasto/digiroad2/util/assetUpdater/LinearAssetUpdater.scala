...
class LinearAssetUpdater(service: LinearAssetOperations) {
  ...
  def updateChangeSet(changeSet: ChangeSet): Unit = {
    ...
    if (ids.nonEmpty) {
      ...
      dao.updateExpirations(ids,expired = true)
    }
  ...
  protected def adjustedSideCode(adjustment: SideCodeAdjustment, oldAssets: Seq[PersistedLinearAsset], roadLinks: Seq[RoadLink], adjustedSideCodes: Seq[SideCodeAdjustment]= Seq()): Unit = {
    service.expireAssets(oldAssets.map(_.id),expired = true, newTransaction = false)
   ...
   val assets = oldAssets.filter(a=>adjusted.contains(a.id)).map(a=>
      {
        ...
        NewLinearAssetMassOperation(
          ...
          Some(roadLink), true, a.createdBy, a.createdDateTime, a.modifiedBy, a.modifiedDateTime, a.verifiedBy, a.informationSource.map(_.value)
        )
      }
    )
