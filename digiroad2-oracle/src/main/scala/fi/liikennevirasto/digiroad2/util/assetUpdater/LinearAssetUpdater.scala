...
class LinearAssetUpdater(service: LinearAssetOperations) {
  ...
  def updateByRoadLinks(typeId: Int, municipality: Int, roadLinks: Seq[RoadLink], changes: Seq[ChangeInfo]) = {
    try {
      ...
      val groupedAssets = assetFiller.toLinearAssetsOnMultipleLinks((assetsOnChangedLinks.filterNot(a => projectedAssets.exists(_.linkId == a.linkId)) ++ projectedAssets ++ assetsWithoutChangedLinks), roadLinks).groupBy(_.linkId)
  ...
  def updateChangeSet(changeSet: ChangeSet) : Unit = {
    // TODO remove these dao method call, use service abstraction 
    ...
    toInsert.foreach { linearAsset =>
      ...
      val id =
        (linearAsset.createdBy, linearAsset.createdDateTime) match {
          case (Some(createdBy), Some(createdDateTime)) => // TODO remove these dao method call, use service abstraction
