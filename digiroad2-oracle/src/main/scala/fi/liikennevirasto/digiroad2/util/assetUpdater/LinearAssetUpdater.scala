...
class LinearAssetUpdater(service: LinearAssetOperations) {
  ...
  def updateChangeSet(changeSet: ChangeSet) : Unit = {
    ...
    ids.foreach(dao.updateExpiration(_, expired = true, LinearAssetTypes.generatedInUpdate))
  ...
  protected def updateProjected(toUpdate: Seq[PersistedLinearAsset], persisted: Map[Long, Seq[PersistedLinearAsset]]) = {
    ...
    toUpdate.foreach { linearAsset =>
      ...
      if (valueChanged(linearAsset, persistedLinearAsset)) {
        linearAsset.value match {
          case Some(NumericValue(intValue)) =>
            dao.updateValue(id, intValue, LinearAssetTypes.numericValuePropertyId, LinearAssetTypes.generatedInUpdate)
          case Some(TextualValue(textValue)) =>
            dao.updateValue(id, textValue, LinearAssetTypes.getValuePropertyId(linearAsset.typeId), LinearAssetTypes.generatedInUpdate)
          case _ => None
        }
      }
    }
