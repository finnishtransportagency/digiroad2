...
class LinearAssetUpdater(service: LinearAssetOperations) {
  ...
  def updateChangeSet(changeSet: ChangeSet): Unit = {
    ...
    if (changeSet.adjustedSideCodes.nonEmpty) {
      logger.debug(s"Saving SideCode adjustments for asset/link ids=${changeSet.adjustedSideCodes.map(a => "" + a.assetId).mkString(", ")}")
      adjustedSideCodes(changeSet.adjustedSideCodes)
    }
    
    if (changeSet.valueAdjustments.nonEmpty)
      logger.debug(s"Saving value adjustments for assets: ${changeSet.valueAdjustments.map(a => "" + a.asset.id).mkString(", ")}")
  ...
  protected def adjustedSideCodes(adjustedSideCodes: Seq[SideCodeAdjustment]): Unit = {
    ...
    adjustedSideCodes.foreach { adjustment =>
      adjustedSideCode(adjustment, assets, roadLinks)
    }
  }
  ...
  protected def adjustedSideCode(adjustment: SideCodeAdjustment, oldAssets: Seq[PersistedLinearAsset], roadLinks: Seq[RoadLink]): Unit = {
