  ...
  def updateSamuutusChangeSet(changeSet: ChangeSet, roadLinkChanges: Seq[RoadLinkChange]): Seq[ChangedAsset] = {
    ...
    def expireLanes(laneIdsToExpire: Set[Long]): Seq[ChangedAsset] = {
      laneIdsToExpire.map(laneId => {
        ...
        reportLaneChanges(laneToExpire, Seq(), Deletion, roadLinkChanges)
      }).toSeq
    }
    ...
    def saveLanePositionAdjustments(positionAdjustments: Seq[LanePositionAdjustment]): Seq[ChangedAsset] = {
      ...
      positionAdjustments.map { adjustment =>
        ...
        val newLaneId = laneService.createWithoutTransaction(laneToCreate, oldLane.get.createdBy.get)
        ...
        reportLaneChanges(oldLane, Seq(laneToCreate.copy(id = newLaneId)), Replaced, roadLinkChanges)
      }
    }
    ...
    def saveDividedLanes(laneSplits: Seq[LaneSplit]): Seq[ChangedAsset] = {
      ...
      val createdLanesWithOldLanes = laneSplits.map(split => {
        val createdLanes = split.lanesToCreate.map(laneToCreate => {
          val newId = laneService.createWithoutTransaction(laneToCreate, split.originalLane.createdBy.get)
      ...
      val changedLanes = createdLanesWithOldLanes.map(createdLanesAndOldLane => {
        val createdLanes = createdLanesAndOldLane._1
        val originalLane = createdLanesAndOldLane._2
        if(createdLanes.isEmpty) {
          laneService.moveToHistory(originalLane.id, None, expireHistoryLane = true, deleteFromLanes = false, AutoGeneratedUsername.generatedInUpdate)
        } else {
        ...
        reportLaneChanges(Some(originalLane), createdLanes, Divided, roadLinkChanges)
      })
