...
object LaneUpdater {
  ...
  def updateChangeSet(changeSet: ChangeSet, roadLinkChanges: Seq[RoadLinkChange] = Seq(), generateReports: Boolean = false): Seq[ChangedAsset] = {
    ...
    def saveMValueAdjustment(mValueAdjustments: Seq[MValueAdjustment]): Seq[ChangedAsset] = {
      val toAdjustLanes = laneService.getPersistedLanesByIds(mValueAdjustments.map(_.laneId).toSet, newTransaction = false)

      mValueAdjustments.flatMap { adjustment =>
    ...
    def saveSideCodeAdjustments(sideCodesToAdjust: Seq[SideCodeAdjustment]): Seq[ChangedAsset] = {
      val originalLanes = laneService.getPersistedLanesByIds(sideCodesToAdjust.map(_.laneId).toSet, newTransaction = false)
      changeSet.adjustedSideCodes.flatMap { adjustment =>
        ...
        if (generateReports) {
          Some(reportLaneChanges(originalLane, Seq(originalLane.get.copy(sideCode = adjustment.sideCode.value)), Replaced, roadLinkChanges))
        } else None
      }
    }
