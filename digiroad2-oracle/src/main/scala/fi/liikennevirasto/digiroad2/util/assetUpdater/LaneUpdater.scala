...
object LaneUpdater {
  ...
  def updateChangeSet(changeSet: ChangeSet) : Unit = {
    ...
    def saveMValueAdjustment(changeSetToTreat: Seq[MValueAdjustment]): Unit = {
      val toAdjustLanes = laneService.getPersistedLanesByIds(changeSetToTreat.map(_.laneId).toSet, newTransaction = false)
