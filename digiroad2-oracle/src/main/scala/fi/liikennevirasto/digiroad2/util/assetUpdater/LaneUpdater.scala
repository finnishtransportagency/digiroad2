...
object LaneUpdater {
  ...
  def updateSamuutusChangeSet(changeSet: ChangeSet, roadLinkChanges: Seq[RoadLinkChange]): Seq[ChangedAsset] = {
    ...
    def expireLanes(laneIdsToExpire: Set[Long]): Seq[ChangedAsset] = {
      val persistedLanesToExpire = laneService.getPersistedLanesByIds(laneIdsToExpire, newTransaction = false)
    ...
    def saveLanePositionAdjustments(positionAdjustments: Seq[LanePositionAdjustment]): Seq[ChangedAsset] = {
      ...
      val newIds = laneService.createMultipleLanes(oldLanesAndLanesToCreate.map(_._2), AutoGeneratedUsername.generatedInUpdate).map(_.id)
      ...
      laneService.moveToHistoryBatch(oldLanesWithNewIdAndLaneToCreate.map(_._1), AutoGeneratedUsername.generatedInUpdate)
    ...
    def saveDividedLanes(laneSplits: Seq[LaneSplit]): Seq[ChangedAsset] = {
      ...
      val laneSplitsWithCreatedIds = laneSplits.map(split => {
      ...
      laneService.moveToHistoryBatch(oldLanesWithNewIds ++ oldLanesWithNoNewIds,  AutoGeneratedUsername.generatedInUpdate)
