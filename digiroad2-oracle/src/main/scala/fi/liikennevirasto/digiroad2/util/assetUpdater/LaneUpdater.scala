...
object LaneUpdater {
  ...
  private def reportLoop[T](roadLinkChanges: Seq[RoadLinkChange], lanes: Seq[T], operation: (T, Seq[RoadLinkChange]) => Option[ChangedAsset]): GenIterable[Option[ChangedAsset]] = {
    lanes.size match {
      case a if a >= parallelizationThreshold =>
        val g = lanes.grouped(groupSizeForParallelRun).toList.par
        val (_, level: Int) = setParallelismLevel(g.size)
        LogUtils.time(logger, s"Reporting lanes, multithreaded") {
          new Parallel().operation(g, level) { tasks =>
            tasks.map { al =>
              LogUtils.time(logger, s"Reporting lanes task with items count ${al.size} ") {
                al.map(laneToExpire => operation(laneToExpire, roadLinkChanges))
              }
            }
          }.flatten
        }
      case _ => LogUtils.time(logger, s"Reporting lanes task with items count ${lanes.size} ") {
        lanes.map(laneToExpire => operation(laneToExpire, roadLinkChanges))}
    }
  }

  private def getReported(list: GenIterable[Option[ChangedAsset]]): List[ChangedAsset] = {
    val listOut = new ListBuffer[ChangedAsset]
    for (t <- list) {
      if (t.nonEmpty) {
        listOut.append(t.get)
      }
    }
    listOut.toList
  }
  ...
  def updateSamuutusChangeSet(changeSet: ChangeSet, roadLinkChanges: Seq[RoadLinkChange]): Seq[ChangedAsset] = {
    ...
    def expireLanes(laneIdsToExpire: Set[Long]): Seq[ChangedAsset] = {
      ...
      logger.info(s"Start reporting ${persistedLanesToExpire.size} expired lanes")

      def report(laneToExpire: PersistedLane,roadLinkChanges: Seq[RoadLinkChange]): Option[ChangedAsset] = {
        reportLaneChanges(Some(laneToExpire), Seq(), Deletion, roadLinkChanges)
      }
      
      val reportedExpiredLanes = getReported(reportLoop(roadLinkChanges, persistedLanesToExpire,report))
      logger.info(s"Finished reporting ${persistedLanesToExpire.size} expired lanes")

      reportedExpiredLanes
    }

    def saveLanePositionAdjustments(positionAdjustments: Seq[LanePositionAdjustment]): Seq[ChangedAsset] = {
      val oldLanesAndLanesToCreate =
        LogUtils.time(logger, s"OldLanesAndLanesToCreate", startLogging = true) {
          positionAdjustments.grouped(10000).flatMap(addAttributes => {
      ...
      val newIds = LogUtils.time(logger, "Create adjusted lanes", startLogging = true){
        laneService.createMultipleLanes(oldLanesAndLanesToCreate.map(_._2), AutoGeneratedUsername.generatedInUpdate).map(_.id)
      }
      val oldLanesWithNewIdAndLaneToCreate =  LogUtils.time(logger, s"oldLanesWithNewIdAndLaneToCreate", startLogging = true) {oldLanesAndLanesToCreate.zip(newIds).map {
      ...
      LogUtils.time(logger, s"Move ${oldLanesToHistory.size} adjusted lanes to history",startLogging = true) {
        laneService.moveToHistoryBatch(oldLanesToHistory, AutoGeneratedUsername.generatedInUpdate)
      }

      logger.info(s"Start reporting ${oldLanesWithNewIdAndLaneToCreate.size} position adjustments")

      def report(oldAndNewLane: (OldLaneWithNewId, PersistedLane),roadLinkChanges: Seq[RoadLinkChange]) = {
      ...
      val reportedPositionAdjustments = getReported(reportLoop(roadLinkChanges, oldLanesWithNewIdAndLaneToCreate,report))
      logger.info(s"Finished reporting ${oldLanesWithNewIdAndLaneToCreate.size} position adjustments")
      reportedPositionAdjustments
    }
    ...
    def saveDividedLanes(laneSplits: Seq[LaneSplit]): Seq[ChangedAsset] = {
      ...
      val laneSplitsWithCreatedIds = LogUtils.time(logger, s"Create ${lanesToCreate.size} lanes due to split",startLogging = true) {
      ...
      def partitionSplitLanes(laneSplitsWithCreatedIds: Seq[LaneSplit]):Seq[OldLaneWithNewId] = {
        val lanes = new ListBuffer[OldLaneWithNewId]
        for (t <- laneSplitsWithCreatedIds) {
          if (t.lanesToCreate.nonEmpty)
          // Each old lane must have history row referencing to each new lane created from it
            lanes.appendAll(t.lanesToCreate.map(newLane => {
              OldLaneWithNewId(t.originalLane, Some(newLane.id))
            }))
          else lanes.append(OldLaneWithNewId(t.originalLane, None))
        }
        lanes
      }

      val oldLanesToHistory = LogUtils.time(logger, s"Add new lane id for split lanes",startLogging = true) {partitionSplitLanes(laneSplitsWithCreatedIds)}
      LogUtils.time(logger, s"Move ${oldLanesToHistory.size} split lanes to history",startLogging = true) {
        laneService.moveToHistoryBatch(oldLanesToHistory,  AutoGeneratedUsername.generatedInUpdate)
      }

      logger.info(s"Start reporting ${laneSplitsWithCreatedIds.size} divided lanes")

      def report(split: LaneSplit, roadLinkChanges: Seq[RoadLinkChange]): Option[ChangedAsset] = {
        reportLaneChanges(Some(split.originalLane), split.lanesToCreate, Divided, roadLinkChanges)
      }

      val reportedDivideLanes = getReported(reportLoop(roadLinkChanges, laneSplitsWithCreatedIds,report))
      logger.info(s"Finished reporting ${laneSplitsWithCreatedIds.size} divided lanes")
      reportedDivideLanes
    }
    ...
    def saveGeneratedLanes(lanesToGenerate: Seq[PersistedLane]): Seq[ChangedAsset] = {
      ...
      logger.info(s"Start reporting ${createdLanes.size} created lanes")

      def report(createdLane: PersistedLane, roadLinkChanges: Seq[RoadLinkChange]): Option[ChangedAsset] = {
        reportLaneChanges(None, Seq(createdLane), Creation, roadLinkChanges)
      }

      val reportedCreatedLanes = getReported(reportLoop(roadLinkChanges, createdLanes,report))
      logger.info(s"Finished reporting ${createdLanes.size} created lanes")
      reportedCreatedLanes
    }
  ...
  def updateTrafficDirectionChangesLaneWorkList(roadLinkChanges: Seq[RoadLinkChange]): Unit = {
    ...
    changesFiltered.foreach(change => {
        change.newLinks.foreach(newLink => {
          if (isRealTrafficDirectionChange(change, newLink)) {
  ...
  def isRealTrafficDirectionChange(change: RoadLinkChange, newLink: RoadLinkInfo): Boolean = {
      ...
      val replaceInfo = change.replaceInfo.find(_.newLinkId.getOrElse("") == newLink.linkId).getOrElse(throw new NoSuchElementException(s"Replace info for link ${newLink.linkId} not found from change ${change}"))
  ...
  def reportLaneChanges(oldLane: Option[PersistedLane], newLanes: Seq[PersistedLane], changeType: ChangeType, roadLinkChanges: Seq[RoadLinkChange]): Option[ChangedAsset] = {
    ...
    val relevantRoadLinkChangeOpt = LogUtils.time(logger, s"find roadLinkChanges") {roadLinkChanges.find(change => {
