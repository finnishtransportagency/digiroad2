...
import fi.liikennevirasto.digiroad2.lane.LaneFiller.{ChangeSet, MValueAdjustment, SideCodeAdjustment, baseAdjustment}
...
object LaneUpdater {
  ...
  def fillSplitLinksWithExistingLanes(newRoadLinks: Seq[RoadLink], oldRoadLink: RoadLink, lanesToUpdate: Seq[PersistedLane], change: RoadLinkChange) = {
  ...
  def fillReplacementLinksWithExistingLanes(newRoadLinks: Seq[RoadLink], lanesToUpdate: Seq[PersistedLane], change: RoadLinkChange): ChangeSet = {
    val allLaneAdjustments = newRoadLinks.map(newRoadlink => {
      ...
      val mainLaneAdjustments = mainLanes.map(mainLane => {
       val mainLaneSideCodeAdjustment = if (digitizationChange) {
        ...
        val mainLaneMValueAdjustment = MValueAdjustment(mainLane.id, newRoadlink.linkId, 0.0, newEndMeasure)
      ...
      val additionalLaneMValueAndSideCodeAdjustments = additionalLanes.map(additionalLane => {
        ...
        val measureAdjustment = MValueAdjustment(additionalLane.id, newRoadlink.linkId, newMeasures.startMeasure, newMeasures.endMeasure)
      ...
      val mainLaneMValueAdjustments = mainLaneAdjustments.map(_._1)
      val mainLaneSideCodeAdjustments = mainLaneAdjustments.flatMap(_._2)
      val additionalLaneMValueAdjustments = additionalLaneMValueAndSideCodeAdjustments.map(_._1)
