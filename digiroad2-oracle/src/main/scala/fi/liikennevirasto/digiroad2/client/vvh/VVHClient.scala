...
case class VVHHistoryRoadLink(linkId: String, municipalityCode: Int, geometry: Seq[Point], administrativeClass: AdministrativeClass,
...
class VVHRoadLinkClient(vvhRestApiEndPoint: String) extends VVHClientOperations{
  ...
  /**
    ...
    */
  def fetchByLinkIds(linkIds: Set[String]): Seq[VVHRoadlink] = {
    // TODO: Temporary parsing from string to long. Remove after not needed anymore
    queryByLinkIds(parseLinkIdsToLong(linkIds), None, true, extractRoadLinkFeature, withLinkIdFilter)
  }
  ...
  def fetchVVHRoadlinks[T](linkIds: Set[String],
    ...
    // TODO: Temporary parsing from string to long. Remove after not needed anymore
    queryByLinkIds(parseLinkIdsToLong(linkIds), fieldSelection, fetchGeometry, resultTransition, withLinkIdFilter)
...
class VVHHistoryClient(vvhRestApiEndPoint: String) extends VVHRoadLinkClient(vvhRestApiEndPoint) {
  ...
  /**
    ...
    */
  def fetchVVHRoadLinkByLinkIds(linkIds: Set[String] = Set()): Seq[VVHHistoryRoadLink] = {
    if (linkIds.isEmpty)
      Nil
    else {
      val batchSize = 1000
  ...
  def fetchVVHRoadLinkByLinkIdsF(linkIds: Set[String] = Set()): Future[Seq[VVHHistoryRoadLink]] = {
