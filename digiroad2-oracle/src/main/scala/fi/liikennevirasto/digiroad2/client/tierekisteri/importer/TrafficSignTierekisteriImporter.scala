...
class TrafficSignTierekisteriImporter extends TierekisteriAssetImporterOperations {
      ...
      roadNumber =>
        ...
        trAssetsSections.foreach {
          case (section, trAssetData) =>
            withDynTransaction {
              createAsset(section, trAssetData, mappedRoadAddresses, mappedRoadLinks, additionalProperties, Seq())
            }
        }
    }
  }

  protected def createAsset(section: AddressSection, trAssetData: TierekisteriAssetData, existingRoadAddresses: Map[(Long, Long, Track), Seq[ViiteRoadAddress]], vvhRoadLinks: Seq[VVHRoadlink], trAdditionalData: Seq[AdditionalPanelInfo], historicMappedRoadLinks: Seq[VVHHistoryRoadLink]): Unit = {
  ...
  override protected def createAsset(section: AddressSection, trAssetData: TierekisteriAssetData, sectionRoadAddresses: Map[(Long, Long, Track), Seq[ViiteRoadAddress]], mappedRoadLinks: Seq[VVHRoadlink], historyMappedRoadLinks: Seq[VVHHistoryRoadLink]): Unit = throw new UnsupportedOperationException("Not Supported Method")
}
