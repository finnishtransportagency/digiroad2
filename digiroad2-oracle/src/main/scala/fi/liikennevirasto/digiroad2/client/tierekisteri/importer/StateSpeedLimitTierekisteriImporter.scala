...
import fi.liikennevirasto.digiroad2.client.vvh.{FeatureClass, VVHHistoryRoadLink, VVHRoadlink}
...
class StateSpeedLimitTierekisteriImporter extends TierekisteriAssetImporterOperations {
  ...
  override type TierekisteriClientType = TierekisteriTrafficSignAssetSpeedLimitClient
  override def withDynSession[T](f: => T): T = OracleDatabase.withDynSession(f)
  override def withDynTransaction[T](f: => T): T = OracleDatabase.withDynTransaction(f)

  override protected def createAsset(section: AddressSection, trAssetData: TierekisteriAssetData, existingRoadAddresses: Map[(Long, Long, Track), Seq[ViiteRoadAddress]], mappedRoadLinks: Seq[VVHRoadlink], historicMappedRoadLinks: Seq[VVHHistoryRoadLink]) = throw new UnsupportedOperationException("Not supported method")
