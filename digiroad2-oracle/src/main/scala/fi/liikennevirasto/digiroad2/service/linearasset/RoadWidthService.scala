    ...
    val newAssets = changedAssets.flatMap{
        ...
        assetsLastModification.get(roadLink.linkId) match {
          ...
          case _ =>
            Some(PersistedLinearAsset(0L, roadLink.linkId, SideCode.BothDirections.value, Some(NumericValue(roadLink.extractMTKClass(roadLink.attributes).width)),
              0, GeometryUtils.geometryLength(roadLink.geometry), Some(AutoGeneratedValues.mtkClassDefault.toString), None, None, None, false, LinearAssetTypes.RoadWidthAssetTypeId,
              changeInfo.timeStamp, None, linkSource = roadLink.linkSource, getVerifiedBy(AutoGeneratedValues.mtkClassDefault.toString, LinearAssetTypes.RoadWidthAssetTypeId), None, Some(MmlNls)))
        }
      case (Some(roadLink), changeInfo, assets) =>
        ...
        PersistedLinearAsset(0L, roadLink.linkId, SideCode.BothDirections.value, Some(NumericValue(roadLink.extractMTKClass(roadLink.attributes).width)),
          startMeasure, endMeasure, asset.createdBy, asset.createdDateTime, Some(AutoGeneratedValues.mtkClassDefault.toString), None, false, LinearAssetTypes.RoadWidthAssetTypeId,
          changeInfo.timeStamp, None, linkSource = roadLink.linkSource, getVerifiedBy(AutoGeneratedValues.mtkClassDefault.toString, LinearAssetTypes.RoadWidthAssetTypeId), None, Some(MmlNls))}
      case _ =>
        None
    }.toSeq
