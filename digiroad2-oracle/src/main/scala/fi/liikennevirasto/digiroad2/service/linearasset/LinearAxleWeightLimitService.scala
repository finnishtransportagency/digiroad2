...
class LinearAxleWeightLimitService(roadLinkServiceImpl: RoadLinkService, eventBusImpl: DigiroadEventBus) extends LinearAssetOperations(roadLinkServiceImpl: RoadLinkService, eventBusImpl: DigiroadEventBus) {
  ...
  override def update(ids: Seq[Long], value: Value, username: String): Seq[Long] = {
    ...
    eventBus.publish("axleWeightLimit:Validator",AssetValidatorInfo(ids.toSet, outputIds.toSet))
    outputIds
  }

  override def getInaccurateRecords(municipalities: Set[Int] = Set(), adminClass: Set[AdministrativeClass] = Set()): List[(Long, String, Int)] = {
    inaccurateDAO.getInaccurateAssetByTypeId(AxleWeightLimit.typeId, municipalities, adminClass)
  }
}
