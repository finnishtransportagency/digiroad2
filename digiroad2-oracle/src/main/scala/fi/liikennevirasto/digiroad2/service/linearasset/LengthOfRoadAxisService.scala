  ...
  override def validateCondition(asset: NewLinearAsset): Unit = {
    ...
    val PT_regulatory_numberFromLink = assestsInLink
      .map(item => item.value.getOrElse("error").asInstanceOf[DynamicValue]
        .value.properties.filter(_.publicId == "PT_regulatory_number"))
      .flatMap(item => item.map(item => item.values.map(item => item.value.toString)))
    val expresion = PT_regulatory_numberFromLink.contains(Seq(validateEnumurationExist(PT_regulatory_number)))
    if (validateEnumurationExist(PT_regulatory_number).toInt == 99) {} else {
      if (expresion) {
