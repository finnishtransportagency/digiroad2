...
class ObstacleService(val roadLinkService: RoadLinkService) extends PointAssetOperations {
  ...
  def updateWithoutTransaction(id: Long, updatedAsset: IncomingObstacle, mValue: Option[Double], linkGeom: Seq[Point],
                               linkMunicipality: Int, linkSource: LinkGeomSource, username: String, timeStamp: Option[Long]): Long = {
    ...
    getPersistedAssetsByIdsWithoutTransaction(Set(id)).headOption.getOrElse(throw new NoSuchElementException("Asset not found"))  match {
      case old if old.lat != updatedAsset.lat || old.lon != updatedAsset.lon =>
        ...
        PostGISObstacleDao.create(setAssetPosition(updatedAsset, linkGeom, value), value, username, linkMunicipality, timeStamp.getOrElse(createTimeStamp()), linkSource, old.createdBy, old.createdAt, old.externalId)
      case _ =>
        PostGISObstacleDao.update(id, setAssetPosition(updatedAsset, linkGeom, value), value, username, linkMunicipality, Some(timeStamp.getOrElse(createTimeStamp())), linkSource)
    }
  }
  ...
  override def adjustmentOperation(persistedAsset: PersistedAsset, adjustment: AssetUpdate, link: RoadLinkInfo): Long = {
    ...
    updateWithoutTransaction(adjustment.assetId, updated, Some(adjustment.mValue), link.geometry, link.municipality,
      LinkGeomSource.NormalLinkInterface, AutoGeneratedUsername.generatedInUpdate, Some(adjustment.timeStamp))
  }
