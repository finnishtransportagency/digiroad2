...
class LinearWidthLimitService(roadLinkServiceImpl: RoadLinkService, eventBusImpl: DigiroadEventBus) extends LinearAssetOperations(roadLinkServiceImpl: RoadLinkService, eventBusImpl: DigiroadEventBus) {
  ...
  override def getInaccurateRecords(municipalities: Set[Int] = Set(), adminClass: Set[AdministrativeClass] = Set()): List[(Long, String, Int)] = {
    inaccurateDAO.getInaccurateAssetByTypeId(WidthLimit.typeId, municipalities, adminClass)
  }
}
