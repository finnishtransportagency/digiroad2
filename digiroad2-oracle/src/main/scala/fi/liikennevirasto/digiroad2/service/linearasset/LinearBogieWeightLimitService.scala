...
import fi.liikennevirasto.digiroad2.asset.{AdministrativeClass, AxleWeightLimit, BogieWeightLimit}
...
class LinearBogieWeightLimitService(roadLinkServiceImpl: RoadLinkService, eventBusImpl: DigiroadEventBus) extends LinearSevenRestrictionsOperations {
...
//  def inaccurateDAO: InaccurateAssetDAO = new InaccurateAssetDAO
  ...
  override def update(ids: Seq[Long], value: Value, username: String): Seq[Long] = {
    ...
    eventBus.publish("bogieWeightLimit:Validator",AssetValidatorInfo(ids.toSet, outputIds.toSet))
    outputIds
  }
...
//
//    withDynTransaction {
//      inaccurateDAO.getInaccurateAsset(BogieWeightLimit.typeId, municipalities, adminClass)
