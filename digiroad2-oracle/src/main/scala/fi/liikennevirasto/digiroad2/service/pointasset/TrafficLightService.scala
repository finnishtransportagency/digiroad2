...
class TrafficLightService(val roadLinkService: RoadLinkService) extends PointAssetOperations {
  ...
  def updateWithoutTransaction(id: Long, updatedAsset: IncomingTrafficLight, linkGeometry: Seq[Point], municipality: Int,
                               ...
                               linkSource: LinkGeomSource): Long = {
    ...
    getPersistedAssetsByIdsWithoutTransaction(Set(id)).headOption.getOrElse(throw new NoSuchElementException("Asset not found")) match {
      case old if  old.lat != updatedAsset.lat || old.lon != updatedAsset.lon =>
        ...
        PostGISTrafficLightDao.create(setAssetPosition(updatedAsset, linkGeometry, value), value, username, municipality, timeStamp.getOrElse(createTimeStamp()), linkSource, old.createdBy, old.createdAt, old.externalId)
      case _ =>
        PostGISTrafficLightDao.update(id, setAssetPosition(updatedAsset, linkGeometry, value), value, username, municipality, Some(timeStamp.getOrElse(createTimeStamp())), linkSource)
    }
  }
    ...
    val propertyData = persistedAsset.propertyData.map(property =>
      property.publicId match {
        case "sidecode" if adjustment.validityDirection.nonEmpty =>
          ...
          SimplePointAssetProperty(property.publicId, Seq(PropertyValue(validityDirection, Some(validityDirection))))
        case "bearing" if adjustment.bearing.nonEmpty =>
          val bearing = adjustment.bearing.get.toString
          SimplePointAssetProperty(property.publicId, Seq(PropertyValue(bearing, Some(bearing))))
        case _ => SimplePointAssetProperty(property.publicId, property.values)
      }).toSet
    val updated = IncomingTrafficLight(adjustment.lon, adjustment.lat, adjustment.linkId, propertyData)
    updateWithoutTransaction(adjustment.assetId, updated, roadLink.geometry, roadLink.municipality, AutoGeneratedUsername.generatedInUpdate,
                             Some(adjustment.mValue), Some(adjustment.timeStamp), LinkGeomSource.NormalLinkInterface)
  }
