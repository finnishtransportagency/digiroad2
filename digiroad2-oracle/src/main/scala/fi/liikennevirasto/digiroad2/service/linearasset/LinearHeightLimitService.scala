...
import fi.liikennevirasto.digiroad2.asset.{AdministrativeClass, AxleWeightLimit, HeightLimit}
...
class LinearHeightLimitService(roadLinkServiceImpl: RoadLinkService, eventBusImpl: DigiroadEventBus) extends LinearAssetOperations(roadLinkServiceImpl: RoadLinkService, eventBusImpl: DigiroadEventBus) {
  ...
  override def getInaccurateRecords(municipalities: Set[Int] = Set(), adminClass: Set[AdministrativeClass] = Set()): List[(Long, String, Int)] = {
    inaccurateDAO.getInaccurateAssetByTypeId(HeightLimit.typeId, municipalities, adminClass)
  }
}
