...
class RoadLinkService(val vvhClient: VVHClient, val eventbus: DigiroadEventBus, val vvhSerializer: VVHSerializer) {
  ...
  def updateAutoGeneratedProperties(adjustedRoadLinks: Seq[AdjustedRoadLinksAndVVHRoadLink]): Unit = {
    ...
    val trafficDirectionEntries: Seq[LinkPropertiesEntries] = trafficDirections.map(link => {
      ...
      LinkPropertiesEntries(RoadLinkDAO.TrafficDirection, linkProperty, username, link.vVHRoadLink, None, None, None)
    })
    ...
    val functionalClassEntries: Seq[LinkPropertiesEntries] = functionalClass.map(link => {
      ...
      LinkPropertiesEntries(RoadLinkDAO.FunctionalClass, linkProperty, username, link.vVHRoadLink, parsedDate, link.adjustedRoadLink.modifiedBy, None)
    })

    val linkTypeEntries: Seq[LinkPropertiesEntries] = linkTypes.map(link => {
      val (username, linkProperty) = setupEntries(link.adjustedRoadLink)
      val parsedDate = parseDate(link.adjustedRoadLink.modifiedAt)
      LinkPropertiesEntries(RoadLinkDAO.LinkType, linkProperty, username, link.vVHRoadLink, parsedDate, link.adjustedRoadLink.modifiedBy, None)
    })
  ...
  def enrichRoadLinksFromVVH(allVvhRoadLinks: Seq[VVHRoadlink], changes: Seq[ChangeInfo] = Nil): Seq[RoadLink] = {
    ...
    val oldChangesId = changes.flatMap(_.oldId).toSet
    ...
    val roadLinkProperties = LogUtils.time(logger,s"TEST LOG ID:${IDSingleton} fetchRoadLinkPropertyRows",true){
      fetchRoadLinkPropertyRows(oldChangesId ++ vvhRoadLinkFiltered.map(_.linkId).toSet)
    }
    
    val roadLinkDataByLinkId: Seq[RoadLink] = LogUtils.time(logger,s"TEST LOG ID:${IDSingleton} roadLinkDataByLinkId",true){
      getRoadLinkDataByLinkIds(vvhRoadLinkFiltered,roadLinkProperties)
    }
    val( completeLinks, incompleteOtherLinks,autoGeneratedLinks) =LogUtils.time(logger,s"TEST LOG ID:${IDSingleton} partition and map",true){
      ...
      ( completeLinks, incompleteOtherLinks,autoGeneratedLinks)
    }
    // possible unit of work
    val (changedLinks, stillIncompleteLinks) = LogUtils.time(logger,s"TEST LOG ID:${IDSingleton} fillIncompleteLinksWithPreviousLinkData",true){ 
      fillIncompleteLinksWithPreviousLinkData(incompleteOtherLinks, changes,roadLinkProperties)
    }
    
    val (changedPartiallyIncompleteLinks,stillIncompleteLinksInUse) = LogUtils.time(logger,s"TEST LOG ID:${IDSingleton} stillIncompleteLinksPartition",true){stillIncompleteLinksPartition(stillIncompleteLinks)}
    
    val adjustedRoadLinks = autoGeneratedLinks ++ changedLinks ++ changedPartiallyIncompleteLinks
    val pair = LogUtils.time(logger,s"TEST LOG ID:${IDSingleton} adjustedRoadLinks map",true){adjustedRoadLinks.map(r=>AdjustedRoadLinksAndVVHRoadLink(r,vvhRoadLinks(r.linkId).last))}
  ...
  def fetchRoadLinkPropertyRows(linkIds: Set[Long]): RoadLinkPropertyRows = {
    ...
    val (linksProperties,overridedRoadLinkAttributes) =
