...
/*  def getRoadLinksAndChangesWithPolygon(polygon :Polygon): (Seq[RoadLink], Seq[ChangeInfo])= {
  ...
  def deleteComplementaryRoadLinksAndPropertiesByLinkIds(deletedLinkIds: Set[String]): Unit = {
    withDynSession {
      LogUtils.time(logger, "TEST LOG Delete complementary road link properties and delete expired complementary road links") {
        ...
        complementaryLinkDAO.deleteComplementaryRoadLinksByLinkIds(deletedLinkIds)
      }
    }
  }
