...
class RoadLinkService(val vvhClient: VVHClient, val eventbus: DigiroadEventBus, val vvhSerializer: VVHSerializer) {
  ...
  def reloadRoadLinksWithComplementaryAndChangesFromVVH(municipalities: Int): (Seq[RoadLink], Seq[ChangeInfo], Seq[RoadLink])= {
    ...
    val (complementaryLinks, changes, links) = Await.result(fut, Duration.Inf)
    println("roadlink took  %.3f sec".format((System.currentTimeMillis()-timer1)*0.001))
    println("start enrichment")
    withDynTransaction {
      val timer2 = System.currentTimeMillis()
  val returnvalue = (enrichRoadLinksFromVVH(links, changes), changes, enrichRoadLinksFromVVH(complementaryLinks, changes))
      println("enrichment took  %.3f sec".format((System.currentTimeMillis()-timer2)*0.001))
      returnvalue
    }
  ...
  def updateRoadLinkChanges(roadLinkChangeSet: RoadLinkChangeSet): Unit = {
    println("updateRoadLinkChanges thread: "+Thread.currentThread().getName)
    println("updateRoadLinkChanges start");


    withDynTransaction {
      val timer1 = System.currentTimeMillis()
      println("updateRoadLinkChanges updateAutoGeneratedProperties start");
      updateAutoGeneratedProperties(roadLinkChangeSet.adjustedRoadLinks)
      println("updateRoadLinkChanges updateAutoGeneratedProperties end");
      println("updateRoadLinkChanges updateIncompleteLinks start");
      updateIncompleteLinks(roadLinkChangeSet.incompleteLinks)
      println("updateRoadLinkChanges updateIncompleteLinks end");
      println("updateRoadLinkChanges fillRoadLinkAttributes start");
      fillRoadLinkAttributes(roadLinkChangeSet.roadLinks, roadLinkChangeSet.changes)
      println("updateRoadLinkChanges fillRoadLinkAttributes end");
      println("updateRoadLinkChanges took  %.3f sec".format((System.currentTimeMillis()-timer1)*0.001))
      println("updateRoadLinkChanges event end")
    }
  }
  ...
  def fillRoadLinkAttributes(roadLinks: Seq[RoadLink], changes: Seq[ChangeInfo], changeUsername: String = "vvh_change"): Unit = {
    println("start fillRoadLinkAttributes")
    println("fillRoadLinkAttributes thread: "+Thread.currentThread().getName)
    val timer1 = System.currentTimeMillis()
    def resolveNewChange(change: ChangeInfo): Unit = {
      val timer2 = System.currentTimeMillis()
      println(" updateRoadLinkChanges -> fillRoadLinkAttributes -> resolveNewChange")
      ...
      if(roadLinkAttributesRelatedWithThisChange.isEmpty && roadLinkFind.isDefined){
        ...
        println("updateRoadLinkChanges -> fillRoadLinkAttributes -> resolveNewChange   %.3f sec".format((System.currentTimeMillis()-timer2)*0.001))
      }
    }

    def returnEqualAttributes(oldAttributes: Seq[Map[String, String]]): Map[String, String] = {
      val timer3 = System.currentTimeMillis()
      //println("returnEqualAttributes")
     val returnattribute= oldAttributes.flatMap { mapToFilter =>
      ...
      //println("returnEqualAttributes   %.3f sec".format((System.currentTimeMillis()-timer3)*0.001))
      returnattribute
    }

    def resolveChanges(changesToBeProcessed: Seq[ChangeInfo]): Unit = {
      val timer4 = System.currentTimeMillis()
      println("updateRoadLinkChanges -> fillRoadLinkAttributes -> resolveChanges size " +changesToBeProcessed.size)
      changesToBeProcessed.foreach { change =>
        val timer8 = System.currentTimeMillis()
        ChangeType.apply(change.changeType) match {
          case ChangeType.New =>
            resolveNewChange(change)
          case ChangeType.Removed =>
            //val timer5 = System.currentTimeMillis()
            ...
            //println("updateRoadLinkChanges -> fillRoadLinkAttributes -> resolveRemovedChange   %.3f sec".format((System.currentTimeMillis()-timer5)*0.001))  
          case _ if LinkAttributesDao.getExistingValues(change.newId.get).isEmpty =>
            //val timer6 = System.currentTimeMillis()
            ...
            if (newIdFromVariousOld.size > 1) {
              ...
              //println("updateRoadLinkChanges -> fillRoadLinkAttributes -> resolveNoIDExistingChange   %.3f sec".format((System.currentTimeMillis()-timer6)*0.001))
            } else {
              //val timer7 = System.currentTimeMillis()
              ...
              //println("updateRoadLinkChanges -> fillRoadLinkAttributes -> resolveChanges roadLinkAttributesRelatedWithThisChange   %.3f sec".format((System.currentTimeMillis()-timer7)*0.001))
            }
          case _ =>
        }
        println("updateRoadLinkChanges -> fillRoadLinkAttributes -> resolveChanges change processing took   %.3f sec".format((System.currentTimeMillis()-timer8)*0.001))
      }
      println("updateRoadLinkChanges -> fillRoadLinkAttributes -> resolveChanges   %.3f sec".format((System.currentTimeMillis()-timer4)*0.001))
      println("updateRoadLinkChanges ending resolving changesToBeProcessed " +changesToBeProcessed.size)
    }
    ...
    println("updateRoadLinkChanges -> fillRoadLinkAttributes   %.3f sec".format((System.currentTimeMillis()-timer1)*0.001))
  }
  ...
  def enrichRoadLinksFromVVH(allVvhRoadLinks: Seq[VVHRoadlink], changes: Seq[ChangeInfo] = Nil): Seq[RoadLink] = {
    ...
    //println("publich linkProperties:changed")
    println("updateRoadLinkChanges")
