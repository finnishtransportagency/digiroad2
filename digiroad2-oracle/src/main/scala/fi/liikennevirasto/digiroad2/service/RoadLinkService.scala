...
case class RoadLinkValueCollection(functionalClassValues: Seq[RoadLinkValue], linkTypeValues: Seq[RoadLinkValue], trafficDirectionValues: Seq[RoadLinkValue], adminClassValues:Seq[RoadLinkValue], linkAttributeValues: Seq[RoadLinkAttribute])
...
class RoadLinkService(val roadLinkClient: RoadLinkClient, val eventbus: DigiroadEventBus) {
  ...
  def getAllExpiredRoadLinksWithExpiredDates(): Seq[RoadLinkWithExpiredDate]= {
    LogUtils.time(logger, "TEST LOG fetchExpiredRoadLinks", startLogging = true) {
      roadLinkDAO.fetchExpiredRoadLinks()
    }
  }

  def getRoadLinksWithExpiredDatesAfterLastSamuutus(lastSuccess: DateTime, municipalityId: Int): Seq[RoadLinkWithExpiredDate] = {
    LogUtils.time(logger, s"Fetch expired road links for municipality $municipalityId after the last successful samuutus on $lastSuccess") {
      roadLinkDAO.fetchExpiredRoadLinksAfterLastSamuutus(lastSuccess, municipalityId)
    }
  }
  ...
  def getPrivateRoadsInfoByLinkIds(linkIds: Set[String]): List[(String, Option[(String, String)])] = {
    withDynTransaction {
        fetchOverridedRoadLinkAttributes(linkIds)
    }
  }
...
/*  def getRoadLinksAndChangesWithPolygon(polygon :Polygon): (Seq[RoadLink], Seq[ChangeInfo])= {
  ...
  private def fetchOverrides(links: Set[String]): Map[String, (Option[RoadLinkPropertyRow],
    Option[RoadLinkPropertyRow], Option[RoadLinkPropertyRow], Option[RoadLinkPropertyRow])] = {
    sql"""select links.id, t.link_id, t.traffic_direction, t.modified_date, t.modified_by,
  ...
  private def fetchOverridedRoadLinkAttributes(links: Set[String]): List[(String, Option[(String, String)])] = {
    val fetchResult =
      sql"""select rla.id, rla.link_id, rla.name, rla.value, rla.created_date, rla.created_by, rla.modified_date, rla.modified_by
          ...
          join road_link_attributes rla on links.id = rla.link_id and rla.valid_to IS NULL"""
  ...
  def updateIncompleteLinks(incompleteLinks: Seq[IncompleteLink]): Unit = {
    LogUtils.time(logger, s"TEST LOG updateIncompleteLinks with ${incompleteLinks.size} links", startLogging = true) {
  ...
  def getComplementaryRoadLinkIdsByMunicipalities(municipalities: Seq[Int]): Seq[String] = {
    val complementaryLinkIds = PostGISDatabase.withDynSession {
      complementaryLinkDAO.getComplementaryRoadLinkIdsByMunicipality(municipalities)
    }
    complementaryLinkIds
  }
  ...
  /**
    ...
    */
  private def deleteLinksAndProperties(
                                        linkIds: Set[String],
                                        deleteFn: Set[String] => Unit,
                                        logMessage: String
                                      ): Unit = {
    LogUtils.time(logger, logMessage) {
      linkIds.foreach { linkId =>
        TrafficDirectionDao.deleteValues(linkId)
        LinkTypeDao.deleteValues(linkId)
        FunctionalClassDao.deleteValues(linkId)
        AdministrativeClassDao.expireValues(
          linkId,
          Some(AutoGeneratedUsername.automaticAdjustment),
          Some(LinearAssetUtils.createTimeStamp())
        )
        LinkAttributesDao.expireValues(
          linkId,
          Some(AutoGeneratedUsername.automaticAdjustment),
          Some(LinearAssetUtils.createTimeStamp())
        )
  ...
  def deleteRoadLinksAndPropertiesByLinkIds(expiredLinkIds: Set[String]): Unit = {
    deleteLinksAndProperties(
      expiredLinkIds,
      roadLinkDAO.deleteRoadLinksByIds,
      "TEST LOG Delete and expire road link properties and delete expired road links"
    )
  }

  def deleteComplementaryRoadLinksAndPropertiesByLinkIds(deletedLinkIds: Set[String]): Unit = {
    withDynSession {
      deleteLinksAndProperties(
        deletedLinkIds,
        complementaryLinkDAO.deleteComplementaryRoadLinksByLinkIds,
        "TEST LOG Delete complementary road link properties and delete expired complementary road links"
      )
    }
  }
  ...
  def insertRoadLinkValuesMass(valueCollection: RoadLinkValueCollection): Unit = {
    LogUtils.time(logger, "TEST LOG insertRoadLinkValuesMass with " +
      s"${valueCollection.functionalClassValues.size} functionalClassValues and " +
      s"${valueCollection.linkTypeValues.size} linkTypeValues and " +
      s"${valueCollection.trafficDirectionValues.size} trafficDirectionValues and " +
      s"${valueCollection.adminClassValues.size} adminClassValues and " +
      s"${valueCollection.linkAttributeValues.size} linkAttributeValues", startLogging = true) {
      val timestamp = DateTime.now().toString
      FunctionalClassDao.insertValuesMass(valueCollection.functionalClassValues, Some(AutoGeneratedUsername.automaticGeneration), timestamp)
      LinkTypeDao.insertValuesMass(valueCollection.linkTypeValues, Some(AutoGeneratedUsername.automaticGeneration), timestamp)
      TrafficDirectionDao.insertValuesMass(valueCollection.trafficDirectionValues, Some(AutoGeneratedUsername.automaticGeneration), timestamp)
      AdministrativeClassDao.insertValuesMass(valueCollection.adminClassValues, Some(AutoGeneratedUsername.automaticGeneration))
      LinkAttributesDao.insertValuesMass(valueCollection.linkAttributeValues, Some(AutoGeneratedUsername.automaticGeneration), timestamp)
    }
  }
