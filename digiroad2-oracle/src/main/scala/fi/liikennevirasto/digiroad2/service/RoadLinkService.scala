...
case class RoadLinkChangeSet(adjustedRoadLinksOld: Seq[RoadLink],adjustedRoadLinks: Seq[AdjustedRoadLinksAndVVHRoadLink], incompleteLinks: Seq[IncompleteLink], changes: Seq[ChangeInfo] = Nil, roadLinks: Seq[RoadLink] = Nil,roadLinkMap:mutable.HashMap[Long, RoadLinkSet],municipality:Int)
...
class RoadLinkService(val vvhClient: VVHClient, val eventbus: DigiroadEventBus, val vvhSerializer: VVHSerializer) {
  ...
  def updateRoadLinkChanges(roadLinkChangeSet: RoadLinkChangeSet): Unit = {
    ...
    withDynTransaction {
      LogUtils.time(logger,s"ID: $actionID updateRoadLinkChanges fillRoadLinkAttributes "){
        ...
        fillRoadLinkAttributes(roadLinkChangeSet.roadLinks, roadLinkChangeSet.changes,roadLinkChangeSet.roadLinkMap,actionID=actionID)
      }
    }
  ...
  def updateRoadLinkChangesNoAkka(roadLinkChangeSet: RoadLinkChangeSet): Unit = {
    ...
    logger.info("updateRoadLinkChanges roadLinks: " + roadLinkChangeSet.roadLinks.size)
    logger.info("updateRoadLinkChanges changes: " + roadLinkChangeSet.changes.size)
    logger.info("updateRoadLinkChanges adjustedRoadLinks: " + roadLinkChangeSet.adjustedRoadLinksOld.size)
    logger.info("updateRoadLinkChanges incompleteLinks: " + roadLinkChangeSet.incompleteLinks.size)
    ...
    println("updateRoadLinkChanges adjustedRoadLinks: " + roadLinkChangeSet.adjustedRoadLinksOld.size)
    println("updateRoadLinkChanges incompleteLinks: " + roadLinkChangeSet.incompleteLinks.size)
    logger.info("updateRoadLinkChanges thread: " + Thread.currentThread().getName)

    LogUtils.time(logger, s"ID: $actionID updateRoadLinkChanges updateIncompleteLinks ") {
      updateIncompleteLinksOld(roadLinkChangeSet.incompleteLinks, actionID = actionID)
    }
    // eesier operation first
    
    LogUtils.time(logger, s"ID: $actionID updateRoadLinkChanges updateAutoGeneratedProperties ") {
      updateAutoGeneratedPropertiesOld(roadLinkChangeSet.adjustedRoadLinksOld, actionID = actionID)
    }
    groups(roadLinkChangeSet.changes, actionID)
    LogUtils.time(logger, s"ID: $actionID updateRoadLinkChanges fillRoadLinkAttributes ") {
      fillRoadLinkAttributesOld(roadLinkChangeSet.roadLinks, roadLinkChangeSet.changes, actionID = actionID)
    }
  }
  ...
  def updateRoadLinkChangesSpeedNoAkka(roadLinkChangeSet: RoadLinkChangeSet): Unit = {
    ...
    //groups(roadLinkChangeSet.changes, actionID)
    LogUtils.time(logger, s"ID: $actionID updateRoadLinkChanges fillRoadLinkAttributes speededup",true) {
      fillRoadLinkAttributes(roadLinkChangeSet.roadLinks, roadLinkChangeSet.changes, roadLinkChangeSet.roadLinkMap, actionID = actionID)
    }
  }
  ...
  /**
    ...
    */
  def updateAutoGeneratedPropertiesOld(adjustedRoadLinks: Seq[RoadLink], actionID:String="") {
  ...
  /**
    ...
    */
  protected def updateIncompleteLinksOld(incompleteLinks: Seq[IncompleteLink], actionID:String="") = {
  ...
  /**
    ...
    */
  def fillRoadLinkAttributesOld(roadLinks: Seq[RoadLink], changes: Seq[ChangeInfo], changeUsername: String = "vvh_change", actionID:String=""): Unit = {
...
//      logger.info("ID: " +IDSingleton+" "+"fillRoadLinkAttributes: "+tempMunicipality.municipalityCode)
    ...
    def resolveChanges(changesToBeProcessed: Seq[ChangeInfo]): Unit = {
      ...
      changesToBeProcessed.foreach { change => // these are inserting only one change at time but order is crucial
        ChangeType.apply(change.changeType) match {
          case ChangeType.New =>
            //LogUtils.time(logger,"resolve new changes"){
  ...
  def enrichRoadLinksFromVVH(allVvhRoadLinks: Seq[VVHRoadlink], changes: Seq[ChangeInfo] = Nil,municipality:Option[Int]=None): Seq[RoadLink] = {
    ...
    eventbus.publish("linkProperties:changed",
      RoadLinkChangeSet(Seq(),pair, stillIncompleteLinksInUse.map(toIncompleteLink), changes, roadLinkDataByLinkId,new mutable.HashMap(),municipality = municipality.getOrElse(0) ))

    completeLinks ++ autoGeneratedLinks ++ changedLinks ++ stillIncompleteLinks
  }

// testing method
  def enrichRoadLinksFromVVHNoAkka(allVvhRoadLinks: Seq[VVHRoadlink], changes: Seq[ChangeInfo] = Nil, municipality:Option[Int]=None, speeded:Boolean=false): Seq[RoadLink] = {
    val vvhRoadLinks = allVvhRoadLinks.filterNot(_.featureClass == FeatureClass.WinterRoads)
    println("start enrichRoadLinksFromVVHNoAkka")
    def autoGenerateProperties(roadLink: RoadLink): RoadLink = {
      val vvhRoadLink = vvhRoadLinks.find(_.linkId == roadLink.linkId)
      vvhRoadLink.get.featureClass match {
        case FeatureClass.TractorRoad => roadLink.copy(functionalClass = 7, linkType = TractorRoad, modifiedBy = Some("automatic_generation"), modifiedAt = Some(DateTimePropertyFormat.print(DateTime.now())))
    ...
    println(s"enriching ended completed in $duration ms and in second ${duration/1000}")
    
    val adjustedRoadLinks = autoGeneratedLinks ++ changedLinks ++ changedPartiallyIncompleteLinks
    
    if(!speeded){
      updateRoadLinkChangesNoAkka(RoadLinkChangeSet(adjustedRoadLinks,Seq(),
        stillIncompleteLinksInUse.map(toIncompleteLink), changes, roadLinkDataByLinkId,new mutable.HashMap(),municipality = municipality.getOrElse(0) ))
    }else {
      println("speed versio")
      var pair: Seq[AdjustedRoadLinksAndVVHRoadLink] = null
      var roadLinkMap: mutable.HashMap[Long, RoadLinkSet] = null
      println("link");    
      val begin1 = System.currentTimeMillis()
      val links = allVvhRoadLinks.groupBy(_.linkId)
      println(s"link creation completed in ${System.currentTimeMillis() - begin1} ms and in second ${(System.currentTimeMillis() - begin1)/1000}")
      LogUtils.time(logger, "preparing data pair", true) {
        pair = adjustedRoadLinks.map(r => AdjustedRoadLinksAndVVHRoadLink(r, links(r.linkId).last))
      }
      LogUtils.time(logger, "preparing data map", true) {
        roadLinkMap = roadLinkToMap(roadLinkDataByLinkId, changes)
      }
      val duration2 = System.currentTimeMillis() - begin
      println(s"enriching and preparing data ended completed in $duration2 ms and in second ${duration2/1000}")
     // updateRoadLinkChangesSpeedNoAkka(RoadLinkChangeSet(Seq(),pair,
      //  stillIncompleteLinksInUse.map(toIncompleteLink), changes, roadLinkDataByLinkId, roadLinkMap, municipality = municipality.getOrElse(0)))
    }
