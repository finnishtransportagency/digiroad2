...
case class RoadLinkChangeSet(adjustedRoadLinks: Seq[RoadLink], incompleteLinks: Seq[IncompleteLink], changes: Seq[ChangeInfo] = Nil)
...
class RoadLinkService(val vvhClient: VVHClient, val eventbus: DigiroadEventBus, val vvhSerializer: VVHSerializer) {
  ...
  def updateRoadLinkChanges(roadLinkChangeSet: RoadLinkChangeSet): Unit = {
    withDynTransaction {
      ...
      fillRoadLinkAttributes(roadLinkChangeSet.changes)
    }
  }
  ...
  /**
    ...
    */
  def fillRoadLinkAttributes(changes: Seq[ChangeInfo]): Unit = {
    ...
    def resolveChanges(changesToBeProcessed: Seq[ChangeInfo], changeUsername: String = "vvh_change"): Unit = {
    ...
    val changesToBeProcessed = changes.filter(change => (change.oldId != change.newId) && change.oldId.isDefined /*<- TODO: new*/).sortWith(_.vvhTimeStamp < _.vvhTimeStamp)
  ...
  def enrichRoadLinksFromVVH(allVvhRoadLinks: Seq[VVHRoadlink], changes: Seq[ChangeInfo] = Nil): Seq[RoadLink] = {
    ...
    eventbus.publish("linkProperties:changed",
      RoadLinkChangeSet(autoGeneratedLinks ++ changedLinks ++ changedPartiallyIncompleteLinks, stillIncompleteLinksInUse.map(toIncompleteLink), changes))
