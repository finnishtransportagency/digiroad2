...
/*  def getRoadLinksAndChangesWithPolygon(polygon :Polygon): (Seq[RoadLink], Seq[ChangeInfo])= {
  ...
  def updateIncompleteLinks(incompleteLinks: Seq[IncompleteLink]): Unit = {
    LogUtils.time(logger, s"TEST LOG updateIncompleteLinks with ${incompleteLinks.size} links") {
  ...
  RoadLinkValueCollection = {
    LogUtils.time(logger, s"TEST LOG getRoadLinkValuesMass for ${linkIds.size} links") {
  ...
  def insertRoadLinkValuesMass(valueCollection: RoadLinkValueCollection): Unit = {
    LogUtils.time(logger, "TEST LOG insertRoadLinkValuesMass") {
