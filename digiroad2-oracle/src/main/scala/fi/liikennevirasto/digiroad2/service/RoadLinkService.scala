...
class RoadLinkService(val vvhClient: VVHClient, val eventbus: DigiroadEventBus, val vvhSerializer: VVHSerializer) {
  ...
  //  withDynTransaction {
   ...
   // withDynTransaction {
    val templinks=roadLinkChangeSet.adjustedRoadLinks.map(_.adjustedRoadLink)
    val templinks2=roadLinkChangeSet.adjustedRoadLinks.map(_.vVHRoadlink)
      LogUtils.time(logger,s"ID: $actionID updateRoadLinkChanges updateAutoGeneratedProperties "){
        updateAutoGeneratedProperties(templinks,templinks2,actionID=actionID)
        //updateAutoGeneratedPropertiesMassInsert(roadLinkChangeSet.adjustedRoadLinks,actionID=actionID)
      }
   // }
