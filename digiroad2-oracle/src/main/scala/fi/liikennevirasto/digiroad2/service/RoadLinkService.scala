...
/*  def getRoadLinksAndChangesWithPolygon(polygon :Polygon): (Seq[RoadLink], Seq[ChangeInfo])= {
  ...
  (Map[String, Option[Int]], Map[String, Option[Int]], Map[String, Option[Int]], Map[String, Option[Int]], Map[String, Map[String, String]]) = {
    val functionalClassMap = FunctionalClassDao.getExistingValues(linkIds).map(fc => fc.linkId -> fc.value).toMap
    val linkTypeMap = LinkTypeDao.getExistingValues(linkIds).map(lt => lt.linkId -> lt.value).toMap
    val trafficDirectionMap = TrafficDirectionDao.getExistingValues(linkIds).map(td => td.linkId -> td.value).toMap
    val adminClassMap = AdministrativeClassDao.getExistingValues(linkIds).map(ac => ac.linkId -> ac.value).toMap
    val linkAttributeMap = linkIds.map(linkId => linkId -> LinkAttributesDao.getExistingValues(linkId)).toMap
    (functionalClassMap, linkTypeMap, trafficDirectionMap, adminClassMap, linkAttributeMap)
  }

  def insertRoadLinkValuesMass(
                                  functionalClassChanges: Map[String, Int],
                                  linkTypeChanges: Map[String, Int],
                                  trafficDirectionChanges: Map[String, Int],
                                  administrativeClassChanges: Map[String, Int],
                                  linkAttributeChanges: Map[String, Map[String, String]]
                                ): Unit = {
    val timestamp = DateTime.now().toString
    FunctionalClassDao.insertValuesMass(functionalClassChanges, Some(AutoGeneratedUsername.automaticGeneration), timestamp)
    LinkTypeDao.insertValuesMass(linkTypeChanges, Some(AutoGeneratedUsername.automaticGeneration), timestamp)
    TrafficDirectionDao.insertValuesMass(trafficDirectionChanges, Some(AutoGeneratedUsername.automaticGeneration), timestamp)
    AdministrativeClassDao.insertValuesMass(administrativeClassChanges, Some(AutoGeneratedUsername.automaticGeneration), timestamp)
    LinkAttributesDao.insertValuesMass(linkAttributeChanges, Some(AutoGeneratedUsername.automaticGeneration), timestamp)
  }
