...
case class RoadLinkValueCollection(functionalClassValues: Map[String, Option[Int]], linkTypeValues: Map[String, Option[Int]], trafficDirectionValues: Map[String, Option[Int]], adminClassValues: Map[String, Option[Int]], linkAttributeValues: Map[String, Map[String, String]])
...
/*  def getRoadLinksAndChangesWithPolygon(polygon :Polygon): (Seq[RoadLink], Seq[ChangeInfo])= {
  ...
  def insertRoadLinkValuesMass(valueCollection: RoadLinkValueCollection): Unit = {
    ...
    FunctionalClassDao.insertValuesMass(valueCollection.functionalClassValues.collect { case (k, Some(v)) => k -> v }, Some(AutoGeneratedUsername.automaticGeneration), timestamp)
    LinkTypeDao.insertValuesMass(valueCollection.linkTypeValues.collect { case (k, Some(v)) => k -> v }, Some(AutoGeneratedUsername.automaticGeneration), timestamp)
    TrafficDirectionDao.insertValuesMass(valueCollection.trafficDirectionValues.collect { case (k, Some(v)) => k -> v }, Some(AutoGeneratedUsername.automaticGeneration), timestamp)
    AdministrativeClassDao.insertValuesMass(valueCollection.adminClassValues.collect { case (k, Some(v)) => k -> v }, Some(AutoGeneratedUsername.automaticGeneration), timestamp)
