  ...
  def updateLinkProperties(linkProperty: LinkProperties, username: Option[String], municipalityValidation: Int => Unit): Option[RoadLink] = {
    ...
    vvhRoadLink.map { vvhRoadLink =>
      ...
      withDynTransaction {
        ...
        if (vvhRoadLink.administrativeClass != State) setLinkProperty(RoadLinkDAO.AdministrativeClass, linkProperty, username, vvhRoadLink, None, None)
  ...
  protected def enrichRoadLinksFromVVH(vvhRoadLinks: Seq[VVHRoadlink], changes: Seq[ChangeInfo] = Nil): Seq[RoadLink] = {
    def autoGenerateProperties(roadLink: RoadLink): RoadLink = {
      ...
      vvhRoadLink.get.featureClass match {
        ...
        case FeatureClass.RoundAboutRoad => roadLink.copy(linkType = Roundabout, modifiedBy = Some("automatic_generation"), modifiedAt = Some(DateTimePropertyFormat.print(DateTime.now())))
