...
trait RoadLinkService {
  ...
  def getRoadLinksFromVVH(municipality: Int): (Seq[VVHRoadLinkWithProperties], Seq[VVHRoadlink]) = {
    val vvhRoadLinks = fetchVVHRoadlinks(municipality)
    (enrichRoadLinksFromVVH(vvhRoadLinks), vvhRoadLinks)
  }
  ...
  protected def enrichRoadLinksFromVVH(vvhRoadLinks: Seq[VVHRoadlink]): Seq[VVHRoadLinkWithProperties] = {
    ...
    val roadLinkDataByMmlId: Seq[AdjustedRoadLink] = getRoadLinkDataByMmlIds(vvhRoadLinks)
