...
class RoadLinkService(val vvhClient: VVHClient, val eventbus: DigiroadEventBus) {
  ...
  def completeChangedLinks(changes: Seq[ChangeInfo], incompleteLinks: Seq[RoadLink]): (Seq[RoadLink], Seq[RoadLink]) = {
    ...
    val incompleteLinksWithOldLinks = incompleteLinks.map { link =>
      val fakeVhhLinks = changes
        ...
        .map { change => VVHRoadlink(change.oldId.get, link.municipalityCode, link.geometry, link.administrativeClass, link.trafficDirection, FeatureClass.AllOthers)}

      // todo: refactor getRoadLinkDataByLinkIds to take in something nicer than VVHRoadLinks
      (link, getRoadLinkDataByLinkIds(fakeVhhLinks))
    }
  ...
  protected def enrichRoadLinksFromVVH(vvhRoadLinks: Seq[VVHRoadlink], changes: Seq[ChangeInfo] = Nil): Seq[RoadLink] = {
    ...
    val (changedLinks, stillIncompleteLinks) = completeChangedLinks(changes, incompleteOtherLinks)
