...
class RoadLinkService(val vvhClient: VVHClient, val eventbus: DigiroadEventBus) {
  ...
  def completeChangedLinks(incompleteLinks: Seq[RoadLink], changes: Seq[ChangeInfo]): (Seq[RoadLink], Seq[RoadLink]) = {
    val oldRoadLinkData = getOldRoadLinkDataByLinkChanges(changes)

    incompleteLinks.map { link =>
      val oldIds = changes.filter(change => change.newId == Option(link.linkId)).map(_.oldId).flatten
      val oldLinks = oldRoadLinkData.filter(link => oldIds.contains(link.linkId))
  ...
  protected def enrichRoadLinksFromVVH(vvhRoadLinks: Seq[VVHRoadlink], changes: Seq[ChangeInfo] = Nil): Seq[RoadLink] = {
    ...
    val (changedLinks, stillIncompleteLinks) = completeChangedLinks(incompleteOtherLinks, changes)
  ...
  def getOldRoadLinkDataByLinkChanges(changes: Seq[ChangeInfo]): Seq[RoadLink] = {
