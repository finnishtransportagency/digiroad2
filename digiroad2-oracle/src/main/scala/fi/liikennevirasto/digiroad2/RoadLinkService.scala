...
import fi.liikennevirasto.digiroad2.linearasset.{RoadLinkProperties, RoadLink}
...
class RoadLinkService(val vvhClient: VVHClient, val eventbus: DigiroadEventBus) {
  ...
  def fillIncompleteLinksWithPreviousLinkData(incompleteLinks: Seq[RoadLink], changes: Seq[ChangeInfo]): (Seq[RoadLink], Seq[RoadLink]) = {
    ...
    incompleteLinks.map { incompleteLink =>
      ...
      incompleteLink.copy(
        functionalClass   = useValueWhenAllEqual(oldPropertiesForIncompleteLink.map(_.functionalClass)).getOrElse(FunctionalClass.Unknown),
        linkType          = useValueWhenAllEqual(oldPropertiesForIncompleteLink.map(_.linkType)).getOrElse(UnknownLinkType),
  ...
  def isPartlyChanged(roadLink: RoadLink): Boolean = {
    val onlyFunctionalClassChanged = roadLink.functionalClass != FunctionalClass.Unknown && roadLink.linkType.value == UnknownLinkType.value
    val onlyLinkTypeChanged = roadLink.functionalClass == FunctionalClass.Unknown && roadLink.linkType.value != UnknownLinkType.value
    onlyFunctionalClassChanged || onlyLinkTypeChanged
  }
  ...
  protected def enrichRoadLinksFromVVH(vvhRoadLinks: Seq[VVHRoadlink], changes: Seq[ChangeInfo] = Nil): Seq[RoadLink] = {
    ...
    val partlyChangedLinks = stillIncompleteLinks.filter(isPartlyChanged)
    ...
    eventbus.publish("linkProperties:changed",
      RoadLinkChangeSet(autoGeneratedLinks ++ changedLinks ++ partlyChangedLinks, stillIncompleteLinks.map(toIncompleteLink)))
  ...
  def getOldRoadLinkPropertiesForChanges(changes: Seq[ChangeInfo]): Seq[RoadLinkProperties] = {
    ...
    oldLinkIds.map { linkId =>
      // todo: do weed need the modifiedAt and modifiedBy?
