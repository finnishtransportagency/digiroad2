...
trait RoadLinkService {
  ...
  def getRoadLinksFromVVH(mmlIds: Seq[Long]): Seq[VVHRoadLinkWithProperties] = {
    // TODO: Retrieve all vvh road links in one query when indexing is fixed in VVH
    enrichRoadLinksFromVVH(mmlIds.flatMap(fetchVVHRoadlink))
  }
  ...
  protected def enrichRoadLinksFromVVH(vvhRoadLinks: Seq[VVHRoadlink]): Seq[VVHRoadLinkWithProperties] = {
    ...
    autoGeneratedLinks.foreach(updateProperties)
    autoGeneratedLinks.foreach( link => removeIncompleteness(link.mmlId) )
