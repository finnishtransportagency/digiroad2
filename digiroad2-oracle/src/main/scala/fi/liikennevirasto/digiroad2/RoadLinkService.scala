...
class RoadLinkService(val vvhClient: VVHClient, val eventbus: DigiroadEventBus) {
  ...
  def updateProperties(linkId: Long, functionalClass: Int, linkType: LinkType,
    ...
    vvhRoadLink.map { vvhRoadLink =>
      ...
      withDynTransaction {
        setLinkProperty("traffic_direction", "traffic_direction", direction.value, linkId, username, Some(vvhRoadLink.trafficDirection.value), None)
        if (functionalClass != FunctionalClass.Unknown) setLinkProperty("functional_class", "functional_class", functionalClass, linkId, username, None, None)
        if (linkType != UnknownLinkType) setLinkProperty("link_type", "link_type", linkType.value, linkId, username, None, None)
  ...
  protected def setLinkProperty(table: String, column: String, value: Int, linkId: Long, username: Option[String], optionalVVHValue: Option[Int] = None, modifiedAt: Option[String]) = {
  ...
  def updateAutoGeneratedProperties(adjustedRoadLinks: Seq[RoadLink]) {
    def updateProperties(roadLink: RoadLink) = {
      if (roadLink.functionalClass != FunctionalClass.Unknown) setLinkProperty("functional_class", "functional_class", roadLink.functionalClass, roadLink.linkId, roadLink.modifiedBy, None, roadLink.modifiedAt)
      if (roadLink.linkType != UnknownLinkType) setLinkProperty("link_type", "link_type", roadLink.linkType.value, roadLink.linkId, roadLink.modifiedBy, None,roadLink.modifiedAt)
    }
    withDynTransaction {
