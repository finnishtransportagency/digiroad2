...
class RoadLinkService(val vvhClient: VVHClient, val eventbus: DigiroadEventBus) {
  ...
  def fillIncompleteLinksWithPreviousLinkData(incompleteLinks: Seq[RoadLink], changes: Seq[ChangeInfo]): (Seq[RoadLink], Seq[RoadLink]) = {
    val oldRoadLinks = getOldRoadLinksForChanges(changes)
    ...
    incompleteLinks.map { incompleteLink =>
      ...
      val oldRoadLinksForIncompleteLink = oldRoadLinks.filter(oldLink => oldIdsForIncompleteLink.contains(oldLink.linkId))

      incompleteLink.copy(
        functionalClass   = useValueWhenAllEqual(oldRoadLinksForIncompleteLink.map(_.functionalClass)).getOrElse(FunctionalClass.Unknown),
        linkType          = useValueWhenAllEqual(oldRoadLinksForIncompleteLink.map(_.linkType)).getOrElse(UnknownLinkType),
        trafficDirection  = useValueWhenAllEqual(oldRoadLinksForIncompleteLink.map(_.trafficDirection)).getOrElse(incompleteLink.trafficDirection))
  ...
  def getOldRoadLinksForChanges(changes: Seq[ChangeInfo]): Seq[RoadLink] = {
