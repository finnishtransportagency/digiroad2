...
case class AdjustedRoadLinkWrapper(adjustedRoadLinks: Seq[AdjustedRoadLink])
...
trait RoadLinkService {
  ...
  def updateAutoGeneratedProperties(roadLinks: Seq[AdjustedRoadLink]): Unit = {
    ...
    roadLinks.foreach(updateProperties)
    roadLinks.foreach(link => removeIncompleteness(link.mmlId))
  }
  ...
  protected def enrichRoadLinksFromVVH(vvhRoadLinks: Seq[VVHRoadlink]): Seq[VVHRoadLinkWithProperties] = {
    ...
    eventbus.publish("linkProperties:linkPropertiesAutoGenerated", AdjustedRoadLinkWrapper(autoGeneratedLinks))
