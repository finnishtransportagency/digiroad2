...
class RailwayCrossingService(val roadLinkService: RoadLinkService) extends PointAssetOperations {
  ...
  private def adjustmentOperation(persistedAsset: PersistedAsset, adjustment: AssetAdjustment): Long = {
    ...
    OracleRailwayCrossingDao.update(adjustment.assetId, updatedAsset, adjustment.mValue, persistedAsset.municipalityCode, "vvh_generated", Some(adjustment.vvhTimeStamp), LinkGeomSource(persistedAsset.linkSource))
  }
