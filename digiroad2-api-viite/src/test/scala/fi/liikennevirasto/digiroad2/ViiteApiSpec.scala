...
class ViiteApiSpec extends FunSuite with ScalatraSuite with BeforeAndAfter{
  ...
  test("Conversion from JSON to Road Address Project") {
    val str = "{\"id\":0,\"status\":1,\"name\":\"erwgerg\",\"startDate\":\"22.4.2017\",\"additionalInfo\":\"\",\"roadPartList\":[{\"roadPartNumber\":205,\"roadNumber\":5,\"ely\":5,\"roadLength\":6730,\"roadPartId\":30,\"discontinuity\":\"Jatkuva\"}]}"
    val json = parse(str)
