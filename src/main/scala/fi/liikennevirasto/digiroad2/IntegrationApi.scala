...
import fi.liikennevirasto.digiroad2.pointasset.oracle.{PersistedObstacle, PersistedPedestrianCrossing, PersistedRailwayCrossing}
...
class IntegrationApi(val massTransitStopService: MassTransitStopService) extends ScalatraServlet with JacksonJsonSupport with AuthenticationSupport {
  ...
  def pedestrianCrossingsToApi(crossings: Seq[PersistedPedestrianCrossing]): Seq[Map[String, Any]] = {
  ...
  def obstaclesToApi(obstacles: Seq[PersistedObstacle]): Seq[Map[String, Any]] = {
    case class AssetTimeStamps(created: Modification, modified: Modification) extends TimeStamps
