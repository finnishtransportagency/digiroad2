...
object ProjectDAO {
  ...
  def getProjectLinks(projectId: Long): List[ProjectLink] = {
    ...
    Q.queryNA[(Long, Long, Int, Int, Long, Long, Long, Long, Long, Long, Long, String, String, Long, Double, Long, Int)](query).list.map {
      case (projectLinkId, projectId, trackCode, discontinuityType, roadNumber, roadPartNumber, startAddrM, endAddrM, startMValue, endMValue , lrmPositionId, createdBy, modifiedBy, linkId, length, calibrationPoints, status) =>
        ProjectLink(projectLinkId, roadNumber, roadPartNumber, Track.apply(trackCode), Discontinuity.apply(discontinuityType), startAddrM, endAddrM, None, None, None, lrmPositionId, linkId, startMValue, endMValue, SideCode.apply(99), (None, None), false, Seq.empty[Point], projectId, LinkStatus.apply(status))
    }
  }
