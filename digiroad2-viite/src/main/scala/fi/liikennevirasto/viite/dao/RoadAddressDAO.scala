  ...
  def isNewRoadPartUsed(roadNumber: Long, roadPartNumber: Long, projectId: Long) = {
    val query =
      s"""
        ...
        TABLE(SDO_UTIL.GETVERTICES(ra.geometry)) t2
        join lrm_position pos on ra.lrm_position_id = pos.id
        where road_number = $roadNumber AND road_part_number = $roadPartNumber and t.id < t2.id AND (pro.id!=$projectId OR (ra.END_DATE IS NULL OR ra.END_DATE>=pro.START_DATE))
        ORDER BY road_number, road_part_number, track_code, start_addr_m
      """
    queryList(query)
  }
