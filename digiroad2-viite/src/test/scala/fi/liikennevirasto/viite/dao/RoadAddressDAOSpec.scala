...
class RoadAddressDAOSpec extends FunSuite with Matchers {
  ...
  test("Create Road Address") {
    runWithRollback {
      ...
      val ra = Seq(RoadAddress(id, 1943845, 1, Track.Combined, Discontinuous, 0L, 10L, Some(DateTime.parse("1901-01-01")), None, Option("tester"), 12345L, 0.0, 9.8, SideCode.TowardsDigitizing, (None, None), false,
  ...
  test("Create Road Address With Calibration Point") {
    runWithRollback {
      val id = RoadAddressDAO.getNextRoadAddressId
      val ra = Seq(RoadAddress(id, 1943845, 1, Track.Combined, Discontinuous, 0L, 10L, Some(DateTime.parse("1901-01-01")), None, Option("tester"), 12345L, 0.0, 9.8, SideCode.TowardsDigitizing,
  ...
  test("test update for merged Road Addresses") {
    ...
    runWithRollback {
      val id = RoadAddressDAO.getNextRoadAddressId
          val toBeMergedRoadAddresses = Seq(RoadAddress(id, 1943845, 1, Track.Combined, Discontinuous, 0L, 10L, Some(DateTime.parse("1901-01-01")), None, Option("tester"), 6556558L, 0.0, 9.8, SideCode.TowardsDigitizing, (None, None), false,
            Seq(Point(0.0, 0.0), Point(0.0, 9.8))))
