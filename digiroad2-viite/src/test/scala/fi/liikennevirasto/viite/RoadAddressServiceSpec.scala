...
class RoadAddressServiceSpec extends FunSuite with Matchers{
  ...
  test("save road link project and get form info") {
    ...
    runWithRollback{
      ...
      val roadAddressProject = RoadAddressProject(id, RoadAddressProjectState.apply(1), "TestProject", "TestUser", DateTime.now(), "TestUser", DateTime.parse("1901-01-01"), DateTime.now(), "Some additional info", 5, 202, 202)
